...
import {
  ...
  type ProducerReadinessObservation,
...
import {
  ColdProcessScheduler,
  type ColdProcessResult,
  type ColdProcessScope,
} from "../helpers/coldProcessScheduler.js";
...
let packagedProducerReadiness: Promise<ProducerReadinessObservation> | undefined;

function observePackagedProducerReadiness(): Promise<ProducerReadinessObservation> {
  packagedProducerReadiness ??= runProducerReadinessWorkflow(
    path.join(fixture.packageRoot, "dist/bin/agentera.js"),
    path.join(fixture.root, "producer-package"),
  );
  return packagedProducerReadiness;
}
...
const COLD_COMMAND_CONCURRENCY = 4;
let activeColdScope: ColdProcessScope | undefined;
...
function runCold(
  ...
  env = process.env,
  input?: string,
): Promise<ColdProcessResult> {
  if (activeColdScope === undefined) throw new Error("cold package command ran outside its owning scope");
  return activeColdScope.run({ command, args, cwd, env, input });
}

function settleCold<T>(promises: Iterable<PromiseLike<T> | T>): Promise<T[]> {
  if (activeColdScope === undefined) throw new Error("cold package callbacks ran outside their owning scope");
  return activeColdScope.all(promises);
}

async function withColdCommands<T>(callback: () => Promise<T>): Promise<T> {
  if (activeColdScope !== undefined) throw new Error("cold package command scopes cannot nest");
  const scheduler = new ColdProcessScheduler({ concurrency: COLD_COMMAND_CONCURRENCY, timeoutMs: 30_000 });
  const value = await scheduler.own(async (scope) => {
    activeColdScope = scope;
    try {
      return await callback();
    } finally {
      activeColdScope = undefined;
    }
  });
  const snapshot = scheduler.snapshot();
  if (snapshot.active !== 0 || snapshot.slots !== 0 || snapshot.waiters !== 0 || snapshot.timers !== 0) {
    throw new Error(`cold package command scope did not settle: ${JSON.stringify(snapshot)}`);
  }
  return value;
}
...
describe("npm distribution boundary", () => {
  ...
  it("runs the packaged glossary qualification gate through bundled production seams", () => {
    const bin = path.join(fixture.packageRoot, "dist/bin/agentera.js");
    const result = run(
      process.execPath,
      [bin, "check", "verify", "eval", "glossary", "--format", "json"],
      fixture.root,
      isolatedPackageEnv(),
    );
    ...
    expect(payload).toMatchObject({
      status: "pass",
      target: "glossary",
      safety: { mode: "offline-frozen-holdout" },
      glossary_evaluation: {
    ...
    expect(payload.diagnostics.stdout.join("\n")).toContain('"schemaVersion": "agentera.personalGlossaryEvaluation.v1"');
  });

  it("fails a copied package qualification on changed behavior with frozen labels and no effects", () => {
    const packageRoot = path.join(fixture.root, "changed-glossary-package");
    const projectRoot = path.join(fixture.root, "changed-glossary-project");
    fs.cpSync(fixture.packageRoot, packageRoot, { recursive: true });
    fs.mkdirSync(projectRoot);
    try {
      const bundle = path.join(packageRoot, "bundle");
      const corpusPath = path.join(bundle, "references", "analysis", "personal-glossary-evaluation-corpus.yaml");
      const authorityPath = path.join(bundle, "references", "analysis", "personal-glossary-evaluation-authority.yaml");
      const holdoutPath = path.join(bundle, "references", "analysis", "personal-glossary-holdout.yaml");
      const frozenLabels = fs.readFileSync(holdoutPath);
      const corpus = YAML.parse(fs.readFileSync(corpusPath, "utf8"), { maxAliasCount: 256 }) as Record<string, any>;
      for (const record of corpus.explicit_admission.slice(0, 50)) {
        record.text = `Is \`${record.id}\` an automatic glossary admission?`;
      }
      fs.writeFileSync(corpusPath, YAML.stringify(corpus), "utf8");
      const authority = YAML.parse(fs.readFileSync(authorityPath, "utf8")) as Record<string, any>;
      authority.observations.behavior_fixture.fixture_sha256 = sha256(fs.readFileSync(corpusPath));
      fs.writeFileSync(authorityPath, YAML.stringify(authority), "utf8");
      const bundleBefore = treeHashes(bundle);
      const projectBefore = treeHashes(projectRoot);
      const environment = isolatedPackageEnv();
      environment.AGENTERA_HOME = bundle;
      const result = run(
        process.execPath,
        [path.join(packageRoot, "dist/bin/agentera.js"), "check", "verify", "eval", "glossary", "--format", "json"],
        projectRoot,
        environment,
      );
      ...
      expect(fs.readFileSync(holdoutPath)).toEqual(frozenLabels);
      expect(treeHashes(bundle)).toEqual(bundleBefore);
      expect(treeHashes(projectRoot)).toEqual(projectBefore);
    } finally {
  ...
  it("executes source-built and self-contained packaged list help, examples, rejections, and corrections for every authority family", async () => {
    ...
    try {
      ...
      const pair = (args: string[]) => settleCold([
        runCold(process.execPath, [sourceBin, ...args], project, isolatedPackageEnv()),
        runCold(process.execPath, [packagedBin, ...args], project, isolatedPackageEnv()),
      ]);
      const [sourceSchema, packagedSchema] = await pair(["schema", "--format", "json"]);
      ...
      await settleCold(families.map(async (family) => {
        const helpArgs = ["state", ...family.commandTokens, "list", "--help"];
        const [sourceHelp, packagedHelp] = await pair(helpArgs);
        ...
        const [sourceExample, packagedExample] = await pair(exampleArgs);
        ...
        const [sourceMismatch, packagedMismatch] = await pair(mismatchArgs);
        ...
        const [sourceCorrection, packagedCorrection] = await pair(correctionArgs);
        ...
        const [sourceHuman, packagedHuman] = await pair(humanArgs);
        ...
        const [sourceGetHelp, packagedGetHelp] = await pair(getHelpArgs);
        ...
        const [sourceRejectedGet, packagedRejectedGet] = await pair(rejectedGetArgs);
        ...
        const [sourceBare, packagedBare] = await pair(bareArgs);
        ...
        if (family.bareRead === "alias") {
          ...
          const sourceExplicit = await runCold(process.execPath, [sourceBin, ...explicitArgs], project, isolatedPackageEnv());
          ...
          const [sourceMalformedBare, sourceMalformedList, packagedMalformedBare, writer] = await settleCold([
            runCold(process.execPath, [sourceBin, ...malformedBareArgs], project, isolatedPackageEnv()),
            runCold(process.execPath, [sourceBin, ...malformedListArgs], project, isolatedPackageEnv()),
            runCold(process.execPath, [packagedBin, ...malformedBareArgs], project, isolatedPackageEnv()),
            runCold(process.execPath, [packagedBin, "state", ...family.commandTokens, "explain", "--format", "json"], project, isolatedPackageEnv()),
          ]);
          ...
          const [sourceCorrection, packagedCorrection] = await pair(bareCorrection);
          expect(sourceCorrection.status).toBe(0);
          expect(packagedCorrection.status).toBe(0);
        }
      }));
      });
    } finally {
  ...
  it("preserves realistic 100-row TODO cardinality and degradation in source and the extracted package without mutation", async () => {
    const sourceBin = path.join(fixture.constructionRoot, "dist/bin/agentera.js");
    const packagedBin = path.join(fixture.packageRoot, "dist/bin/agentera.js");
    ...
    const env = isolatedPackageEnv();
    await withColdCommands(async () => {
    const invoke = (bin: string, args: string[]) => runCold(process.execPath, [bin, ...args], project, env);
    const parity = async (args: string[], status = 0) => {
      const [source, packaged] = await settleCold([invoke(sourceBin, args), invoke(packagedBin, args)]);
    ...
    const packageOnly = async (args: string[], status = 0) => {
      const packaged = await invoke(packagedBin, args);
      expect(packaged.status, packaged.stderr || packaged.stdout).toBe(status);
      return JSON.parse(packaged.stdout);
    };

    await settleCold([40, 60, 100].map(async (limit) => {
      const result = await parity(["state", "todo", "list", "--ids-only", "--limit", String(limit), "--format", "json"]);
    ...
    const cursorFirst = await parity(["state", "todo", "list", "--ids-only", "--limit", "40", "--format", "json"]);
    ...
    const changedLimit = await packageOnly(["state", "todo", "list", "--ids-only", "--limit", "5", "--cursor", cursorFirst.next_cursor, "--format", "json"], 1);
    ...
    expect((await packageOnly(shellCommandArgs(changedLimit.error.recovery))).entries.map((entry: any) => entry.id)).toEqual(orderedIds.slice(0, 5));
    ...
    const legacy = await packageOnly(["state", "todo", "list", "--ids-only", "--limit", "40", "--cursor", legacyCursor, "--format", "json"], 1);
    ...
    expect((await packageOnly(shellCommandArgs(legacy.error.recovery))).entries.map((entry: any) => entry.id)).toEqual(orderedIds.slice(0, 40));

    const malformed = await packageOnly(["state", "todo", "list", "--ids-only", "--limit", "40", "--cursor", "not-a-cursor", "--format", "json"], 1);
    ...
    expect((await packageOnly(shellCommandArgs(malformed.error.recovery))).entries.map((entry: any) => entry.id)).toEqual(orderedIds.slice(0, 40));

    const cursorPages = [cursorFirst];
    while (cursorPages.at(-1).retrieval.continue) cursorPages.push(await packageOnly(shellCommandArgs(cursorPages.at(-1).retrieval.continue)));
    ...
    expect((await packageOnly(shellCommandArgs(cursorEntries.at(-1).retrieval.get))).entry).toMatchObject({ id: orderedIds.at(-1), artifact: "todo" });

    const normalPages = [await packageOnly(["state", "todo", "list", "--severity", "normal", "--status", "open", "--ids-only", "--limit", "10", "--format", "json"])];
    while (normalPages.at(-1).retrieval.continue) normalPages.push(await packageOnly(shellCommandArgs(normalPages.at(-1).retrieval.continue)));
    ...
    const first = await packageOnly(["state", "todo", "list", "--severity", "critical", "--status", "open", "--ids-only", "--limit", "40", "--format", "json"]);
    const second = await packageOnly(shellCommandArgs(first.retrieval.continue));
    ...
    const selected = await parity(["state", "todo", "list", "--fields", "status,target_version", "--limit", "100", "--format", "json"]);
    ...
    const degraded = await parity(["state", "todo", "list", "--limit", "100", "--format", "json"]);
    ...
    await packageOnly(["state", "todo", "list", "--fields", "title", "--limit", "100", "--format", "json"], 1);

    for (const [index, entry] of [first.entries[0], second.entries.at(-1), degraded.entries.at(-1)].entries()) {
      const exact = index === 0
        ? await parity(shellCommandArgs(entry.retrieval.get))
        : await packageOnly(shellCommandArgs(entry.retrieval.get));
  ...
  it("classifies every extracted-package TODO cursor branch with executable current restart", async () => {
    const packagedBin = path.join(fixture.packageRoot, "dist/bin/agentera.js");
    ...
    const env = isolatedPackageEnv();
    await withColdCommands(async () => {
    const invoke = async (args: string[], status = 0) => {
      const result = await runCold(process.execPath, [packagedBin, ...args], project, env);
      expect(result.status, result.stderr || result.stdout).toBe(status);
      return JSON.parse(result.stdout);
    };
    const authorityPath = path.join(CHECKOUT_ROOT, "references/artifacts/state-storage-authority.yaml");
    const first = await invoke(["state", "todo", "list", "--ids-only", "--limit", "10", "--format", "json"]);
    const payload = decodeListCursor(first.next_cursor, project, authorityPath);
    const assertFailure = async (args: string[], classification: string, message: string, recovery: string, expectedIds = orderedIds.slice(0, 10)): Promise<void> => {
      const failure = await invoke(args, 1);
      expect(failure).not.toHaveProperty("entries");
      expect(failure.error).toMatchObject({ class: classification, message, recovery });
      expect((await invoke(shellCommandArgs(failure.error.recovery))).entries.map((entry: any) => entry.id)).toEqual(expectedIds);
    };
    const failureAssertions: Array<Promise<void>> = [];

    failureAssertions.push(assertFailure(
    ...
    await settleCold(failureAssertions.splice(0));

    const defaultFirst = await invoke(["state", "todo", "list", "--ids-only", "--format", "json"]);
    expect(decodeListCursor(defaultFirst.next_cursor, project, authorityPath).limit).toBe(20);
    expect((await invoke(["state", "todo", "list", "--ids-only", "--limit", "20", "--cursor", defaultFirst.next_cursor, "--format", "json"])).entries.map((entry: any) => entry.id)).toEqual(orderedIds.slice(20, 40));
    const explicitDefault = await invoke(["state", "todo", "list", "--ids-only", "--limit", "20", "--format", "json"]);
    expect((await invoke(["state", "todo", "list", "--ids-only", "--cursor", explicitDefault.next_cursor, "--format", "json"])).entries.map((entry: any) => entry.id)).toEqual(orderedIds.slice(20, 40));
    ...
    for (const cursor of malformedVariants) failureAssertions.push(assertFailure(
    ...
    await settleCold(failureAssertions);
    expect(projectByteSnapshot(project)).toEqual(before);
  ...
  it("executes active documented examples and every concrete startup state recovery in source and the extracted package", async () => {
    const sourceBin = path.join(fixture.constructionRoot, "dist/bin/agentera.js");
    const packagedBin = path.join(fixture.packageRoot, "dist/bin/agentera.js");
    ...
    await withColdCommands(async () => {
    const invoke = (bin: string, args: string[]) => runCold(process.execPath, [bin, ...args], project, env);
    const executeBoth = async (command: string): Promise<void> => {
    ...
    await settleCold([...documented].map(executeBoth));
    ...
    await settleCold(startupArgs.flatMap((args) => [sourceBin, packagedBin].map(async (bin) => {
    ...
    await settleCold([...recoveries].map(executeBoth));
    });
  });

  it("keeps packaged bootstrap authorities and complete served bodies channel-correct", async () => {
    ...
    await withColdCommands(async () => settleCold(["status", "vision", "discuss", "research", "plan", "build", "optimize", "audit", "document", "profile", "design", "orchestrate"].map(async (capability) => {
      const result = await runCold(process.execPath, [bin, "prime", "--context", capability, "--format", "json"], project, isolatedPackageEnv());
  ...
  it("runs packaged personal glossary publication and Profile Full preservation as new processes", async () => {
    expect(fixture.manifest.files.some((entry) => entry.path === "dist/analytics/personalGlossaryProfile.js")).toBe(true);
    const producerReadiness = await observePackagedProducerReadiness();
    const root = fs.mkdtempSync(path.join(fixture.root, "personal-profile-publication-"));
    const bin = path.join(fixture.packageRoot, "dist/bin/agentera.js");
    expect(producerReadiness.personal).toMatchObject({
  ...
  it("disposes, replays, and revalidates private glossary reviews through the constructed package", () => {
    const profile = fs.mkdtempSync(path.join(fixture.root, "packaged-glossary-review-"));
    const environment = isolatedPackageEnv({ AGENTERA_PROFILE_DIR: profile });
    const tiersDir = path.join(profile, "intermediate", "tiers");
    const text = "packaged private glossary review evidence";
    const generation = publishEvidenceTiers([{
      source_id: "packaged-review-tier-source",
      source_kind: "conversation_turn",
      timestamp: "2026-08-10T00:00:00.000Z",
      project_id: "private-package-review",
      runtime: "opencode",
      source_class: "active_runtime",
      source_product: "opencode",
      active_runtime: true,
      ...
      origin_id: originIdentity("packaged-review-tier-source"),
      content_fingerprint: contentFingerprint(text),
      session_id: "packaged-review-tier-session",
      conversation_key: "packaged-review-tier-session",
      author_class: "user",
    }], {
      tiersDir,
    ...
    const bin = path.join(fixture.packageRoot, "dist/bin/agentera.js");
    const profilePath = path.join(profile, "PROFILE.md");
    fs.writeFileSync(profilePath, "# Packaged review profile\n", "utf8");
    try {
      const queued = run(
        process.execPath,
      ...
      const reviewDirectory = path.join(profile, "intermediate", "personal-glossary");
      const reviewPath = path.join(reviewDirectory, "review-records.json");
      const legacyPending = {
      ...
      const legacyRecord = { ...legacyPending, record_sha256: glossaryCanonicalSha256(legacyPending) };
      const legacyStore = {
        schema_version: "agentera.personalGlossaryReviewStore.v1",
        owner: "current_user",
        records: [legacyRecord],
      };
      const legacyBytes = `${canonicalGlossaryJson({ ...legacyStore, store_sha256: glossaryCanonicalSha256(legacyStore) })}\n`;
      fs.writeFileSync(reviewPath, legacyBytes, { encoding: "utf8", mode: 0o600 });
      const legacyRead = run(
        process.execPath,
        [bin, "report", "personal-glossary-reviews", "list", "--status", "pending", "--limit", "20", "--format", "json"],
        fixture.root,
        environment,
      );
      expect(legacyRead.status, legacyRead.stderr).toBe(0);
      expect(JSON.parse(legacyRead.stdout)).toMatchObject({ entries: [{ review_id: record.review_id, scope: null }] });
      expect(fs.readFileSync(reviewPath, "utf8")).toBe(legacyBytes);
      ...
      expect(decision).toMatchObject({ status: "review_required" });

      const keyPair = generateKeyPairSync("ed25519");
      fs.mkdirSync(reviewDirectory, { recursive: true, mode: 0o700 });
      const trustedHost = {
        owner: "current_user",
        public_key_spki_base64url: keyPair.publicKey
          .export({ format: "der", type: "spki" })
          .toString("base64url"),
        schema_version: "agentera.personalGlossaryTrustedLocalHost.v1",
        subject: "user:packaged",
      };
      fs.writeFileSync(
        path.join(reviewDirectory, "trusted-local-host.json"),
        `${JSON.stringify(trustedHost)}\n`,
      ...
      const unsignedApproval = {
        ...
        nonce: "packaged-single-use-review-nonce",
      };
      const approval = {
        ...unsignedApproval,
        signature: sign(
          null,
          Buffer.from(JSON.stringify(unsignedApproval), "utf8"),
          keyPair.privateKey,
        ).toString("base64url"),
      };
      const dispositionInput = JSON.stringify({
      ...
      const disposed = run(
        process.execPath,
        [bin, "report", "personal-glossary-reviews", "disposition", "--input", "-", "--format", "json"],
        fixture.root,
        environment,
        dispositionInput,
      );
      expect(disposed.status, disposed.stderr).toBe(0);
      const disposition = JSON.parse(disposed.stdout);
      expect(disposition).toMatchObject({
        status: "disposed",
        record: { disposition: "accept", status: "terminal" },
        publication_authorization: {
          review_id: record.review_id,
          review_record_sha256: expect.stringMatching(/^[a-f0-9]{64}$/),
        },
      });
      const reviewBytes = fs.readFileSync(reviewPath, "utf8");
      expect(reviewBytes).not.toContain(capsule.term);
      expect(reviewBytes).not.toContain(capsule.meaning);
      expect(reviewBytes).not.toContain(unsignedApproval.nonce);
      ...
      expect(JSON.parse(replay.stdout)).toMatchObject({ status: "unchanged_replay" });
      expect(fs.readFileSync(reviewPath, "utf8")).toBe(reviewBytes);

      const changedUnsignedApproval = { ...unsignedApproval, disposition: "reject" };
      const changedApproval = {
        ...changedUnsignedApproval,
        signature: sign(
          null,
          Buffer.from(JSON.stringify(changedUnsignedApproval), "utf8"),
          keyPair.privateKey,
        ).toString("base64url"),
      };
      const conflict = run(
      ...
      expect(conflict.status, conflict.stderr).toBe(1);
      expect(JSON.parse(conflict.stdout)).toMatchObject({ error: { class: "review_approval_replayed" } });
      expect(conflict.stdout).not.toContain(unsignedApproval.nonce);
      expect(fs.readFileSync(reviewPath, "utf8")).toBe(reviewBytes);
      ...
      const published = run(
        ...
        JSON.stringify({
          ...
          review_authorization: disposition.publication_authorization,
          as_of: "2026-08-10",
        }),
      );
      expect(published.status, published.stderr).toBe(0);
      expect(JSON.parse(published.stdout)).toMatchObject({ status: "changed" });
      const publishedProfile = fs.readFileSync(profilePath, "utf8");
      ...
      const corroborated = createGlossaryEvidenceCapsule({
        ...
        provenance_kind: "personal_inferred_usage",
        evidence: [
          { source_id: "source-package-c", evidence_anchor: "anchor-package-c", source_kind: "instruction_document" },
          { source_id: "source-package-d", evidence_anchor: "anchor-package-d", source_kind: "project_config_signal" },
        ],
        generation: `${capsule.generation}-next`,
        policy_version: capsule.policy_version,
      });
      const corroboratedProjection = projectPersonalGlossaryCandidates({
        generation: corroborated.generation,
        policy_version: corroborated.policy_version,
        retained_at: "2026-08-10T00:00:00.000Z",
        candidates: [{ capsule: corroborated, project_ids: ["project-package-private"] }],
      });
      persistPersonalGlossaryCandidateProjection(corroboratedProjection, { env: { AGENTERA_PROFILE_DIR: profile } });
      const stalePublication = run(
        process.execPath,
        [bin, "report", "personal-glossary-publish", "--input", "-", "--format", "json"],
        fixture.root,
        environment,
      ...
      expect(stalePublication.status, stalePublication.stderr).toBe(1);
      expect(JSON.parse(stalePublication.stdout)).toMatchObject({ error: { class: "publication_not_authorized" } });
      expect(fs.readFileSync(profilePath, "utf8")).toBe(publishedProfile);
    } finally {
  ...
  it("matches source and extracted-package producer readiness", { timeout: 120_000 }, async () => {
    const [source, packaged] = await Promise.all([
      ...
      observePackagedProducerReadiness(),
    ]);
