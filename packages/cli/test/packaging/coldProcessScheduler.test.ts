...
describe("cold process scheduler ownership", () => {
  it("preserves bounded concurrency and deterministic successful result order", async () => {
    ...
    const values = await scheduler.own(({ run, all }) => all([80, 10, 60, 20, 40, 30].map((delay, index) => run(command("setTimeout(() => process.stdout.write(process.argv[1]), Number(process.argv[2]));", String(index), String(delay))))));
