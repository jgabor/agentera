...
function grammarFixture(mutate: (grammar: Record<string, any>) => void = () => undefined): string {
  ...
  const grammar = authority.personal_mining_authority.explicit_discovery.grammar as Record<string, any>;
  mutate(grammar);
...
describe("finite explicit segment grammar authority", () => {
  it("loads one bounded segment class for all ten approved forms", () => {
    ...
    expect([...contract.forms.values()].every((form) => form.segmentClass === contract.segmentClass && form.entry === "outside_approved_cue" && form.termination === "shared_bounded_termination" && form.output === "shared_cue_segment_output")).toBe(true);
  ...
  it("rejects every mutated declared transition", () => {
    const sourceTransitions = grammarAt(productionAuthority).segment_grammar.transitions as Array<Record<string, any>>;
    for (const [index, source] of sourceTransitions.entries()) {
      ...
      expect(validateGlossaryEntryContract(pathname).some((error) => error.includes(`transition ${source.from}:${source.input} does not match its finite rule`))).toBe(true);
    }
  });
  ...
  it("rejects an appended transition outside the canonical finite table", () => {
    ...
    expect(validateGlossaryEntryContract(pathname)).toContain("explicit segment grammar transition outside:prose is not declared by its finite rule");
  });
  ...
  it("declares structural persistence, one finite exit, reference binding, and exact continuation bounds", () => {
    ...
    expect(contract.structuralConfiguration).toEqual({
      ...
      persistInputs: ["comment", "blank_line", "indentation", "nested_marker", "block_scalar", "structural_fragment", "list_boundary"],
      rejectInputs: ["comment", "blank_line", "indentation", "nested_marker", "block_scalar", "structural_fragment", "list_boundary", "approved_cue"],
      exit: {
        input: "prose",
  ...
  it.each([
    ["state transition", (segment: Record<string, any>) => (segment.transitions[0].to = "outside"), "transition"],
    ["rejection reason", (segment: Record<string, any>) => (segment.reasons.unsupported_transition = "wrong"), "reasons.unsupported_transition"],
    ["exit boundary", (segment: Record<string, any>) => (segment.structural_configuration.exit.definition_allowed = "same_line"), "structural_configuration must have one finite prose exit"],
    ["direction", (segment: Record<string, any>) => (segment.reference_binding.following = "nearest_previous_cue"), "reference_binding direction"],
    ["direction", (segment: Record<string, any>) => (segment.reference_binding.this = "nearest_next_cue"), "reference_binding direction"],
    ["direction", (segment: Record<string, any>) => (segment.reference_binding.exact_term = "nearest_previous_cue"), "reference_binding direction"],
    ["duplicate target", (segment: Record<string, any>) => (segment.reference_binding.duplicate_target_reason = "unsupported_transition"), "reference_binding direction"],
    ["missing target", (segment: Record<string, any>) => (segment.reference_binding.missing_target_reason = "unsupported_transition"), "reference_binding direction"],
    ["bound", (segment: Record<string, any>) => (segment.continuation.maximum_utf8_bytes = 4097), "continuation must accept exactly 4096"],
    ["termination bound", (segment: Record<string, any>) => (segment.termination.maximum_meaning_utf8_bytes = 4097), "termination must accept exactly 4096"],
    ["continuation entry", (segment: Record<string, any>) => (segment.continuation.starts_after = "empty_inline_meaning"), "continuation must start after non-empty inline meaning"],
    ["continuation boundary", (segment: Record<string, any>) => (segment.continuation.stops_on = ["blank_line"]), "continuation stops_on is invalid"],
    ["output contract", (segment: Record<string, any>) => (segment.output.span_encoding = "character_offsets"), "output must use UTF-8 byte offsets"],
    ["deduplication conflict", (segment: Record<string, any>) => (segment.deduplication.same_identity_different_meaning = "retain_both"), "deduplication must preserve identity and conflict semantics"],
    ["canonical lexical order", (segment: Record<string, any>) => (segment.canonical_order.direction = "descending"), "canonical_order must remain lexical"],
    ["canonical lexical comparator", (segment: Record<string, any>) => (segment.canonical_order.comparator = "locale_compare"), "canonical_order must remain lexical"],
    ["canonical lexical fields", (segment: Record<string, any>) => (segment.canonical_order.fields = ["meaning", "term", "term_span.start", "meaning_span.start"]), "canonical_order must remain lexical"],
  ...
  it.each([
    ...
    expect(validateGlossaryEntryContract(pathname).some((error) => error.includes("form[0]"))).toBe(true);
  });
  ...
  it("rejects an undeclared structural persistence input and a missing fail-closed state fallback", () => {
    ...
    expect(validateGlossaryEntryContract(persistencePath).some((error) => error.includes("persist_inputs"))).toBe(true);

    const fallbackPath = grammarFixture((grammar) => {
      delete grammar.segment_grammar.fallback_transitions.structural_config;
    });
    expect(validateGlossaryEntryContract(fallbackPath).some((error) => error.includes("fallback_transitions.structural_config"))).toBe(true);
  });
  ...
  it("accepts the production authority and rejects the same mutations through the direct grammar validator", () => {
    ...
    expect(validateExplicitSegmentGrammar(grammarAt(pathname))).toContain("explicit segment grammar termination must accept exactly 4096 UTF-8 bytes");
  });
});
