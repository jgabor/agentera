...
const retainedHistory: GlossaryAdmissionContext = {
  retainedHistory: new Map([
    ["anchor-explicit", { sourceId: "record-explicit", sourceKind: "conversation_turn", signalType: "correction" }],
    [
      "anchor-instruction",
      {
...
const CAPABILITY_FIXTURES = YAML.parse(fs.readFileSync(path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../fixtures/glossary-deferred-capabilities.yaml"), "utf8")).cases as Array<{
  capability: string;
...
describe("shared glossary entry authority", () => {
  it("projects every glossary owner only from its exact code-owned source value", () => {
    ...
    const mutations: Array<{
    ...
    }> = [
      {
        ...
        mutate: (authority) => {
          authority.ownership_contracts.personal.profile_output.command.canonical += " --force";
        },
      ...
      },
      {
        ...
        mutate: (authority) => {
          authority.consumer_boundary.profile_grounding.command += " garbage";
        },
      ...
      },
      {
        ...
        mutate: (authority) => {
          authority.consumer_boundary.profile_grounding.recovery.repair += " changed";
        },
        load: (pathname) => personalProfileGroundingContract(pathname),
      },
      {
        ...
        mutate: (authority) => {
          authority.consumer_boundary.profile_grounding.recovery.absent = `x${authority.consumer_boundary.profile_grounding.recovery.absent}`;
        },
        load: (pathname) => personalProfileGroundingContract(pathname),
      },
      {
        name: "advice command",
        mutate: (authority) => {
          authority.consumer_boundary.advice_resolution.invocation.command = "npx -y agentera@latest report glossary-advice --input REQUEST";
        },
  ...
  it("derives shared shape, active personal output, bounded ownership, confidence, and decay rules from existing authorities", () => {
    ...
    expect(authority.ownership_contracts.personal.implementation).toMatchObject({
      ...
      active: expect.arrayContaining(["public_receipt_construction", "authorized_personal_publication", "profile_section_persistence"]),
      inactive: ["lookup"],
    });
    expect(authority.deferred_capability_contracts.profile).toMatchObject({
      implementation: "active_partial",
      active_behavior: ["ownership_contracts.personal.admission", "personal_mining_authority.profile_full"],
    ...
    expect(personalGlossaryOutputContract()).toEqual({
      ...
      resultFields: ["schema_version", "owner", "candidate_id", "candidate_revision", "candidate_capsule_sha256", "decision_sha256", "review_record_sha256", "generation", "policy_version", "status", "profile_section_sha256", "published_at", "result_sha256"],
  ...
  it("keeps audit read-only while build owns confirmed project publication", () => {
    ...
    expect(confirmedVariantGuardContract().excludedDirectories).toEqual([".agentera", ".agentera-generated", ".cache", ".git", ".next", ".pnpm", ".turbo", ".venv", ".vite", "build", "bundle", "coverage", "dist", "node_modules", "target", "vendor"]);
    expect(authority.ownership_contracts.project.publication.persisted_document.terminology_set_identity).toMatchObject({
  ...
  it.each([
    ...
    expect(() => personalGlossaryProfileFullContract(pathname)).toThrow("personal glossary Profile Full contract is unavailable");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  it("aligns active Build, Discuss, Plan, and prime consumption with mutation-free Audit", () => {
    ...
    const validation = fs.readFileSync(path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../../../skills/agentera/capabilities/audit/schemas/validation.yaml"), "utf8");
    for (const surface of [authority, validation, auditInstructions]) {
      ...
      expect(surface).toMatch(/profile mutation|personal-profile mutation|personal-profile\s+mutation/i);
  ...
  it("loads active bounded acquisition and Build/Discuss/Plan/prime integration", () => {
    expect(glossaryConsumerContract()).toEqual({
      ...
      outcomes: ["invalid_or_unavailable_project", "equivalent_exact_collision", "divergent_exact_collision", "project_only", "proven_project_gap", "inferred_equivalence", "invalid_or_unavailable_personal", "no_applicable_entry"],
      refreshRequired: expect.arrayContaining(["initial_meaning_sensitive_input", "later_user_requirement_change_that_can_change_meaning", "later_user_intent_change_that_can_change_meaning"]),
      refreshNotRequired: expect.arrayContaining(["unrelated_conversation_turn", "background_state_reread"]),
      transientAllowed: ["outcome", "applicable_meaning", "applicable_owner", "review", "tension", "advisory"],
      durableAllowed: ["caveat_id", "event", "capability", "reason", "ownership_state", "transition_id"],
      caveatOwner: "build",
  ...
  it.each([
    ...
    const pathname = malformedAuthority(mutate);
    expect(validateGlossaryEntryContract(pathname)).toContain("consumer_boundary.acquisition must define canonical bounded independent project and personal reads with sanitized term/meaning/owner output");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  it.each([
    ...
    const pathname = malformedAuthority(mutate);
    expect(validateGlossaryEntryContract(pathname)).toContain("consumer_boundary.advice_resolution must define the active bounded read-only runtime, host-review input, transient output, and privacy-safe failure contract");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });

  it("accepts the bounded no-review selected-term transport", () => {
    expect(validateGlossaryEntryContract()).not.toContain("consumer_boundary.selected_term_transport must authorize bounded no-review file or stdin input without argv content, output echo, host review, persistence, or shared Build stdin");
  });
  ...
  it.each([
    [
      "argv term content",
      (authority: Record<string, any>) => {
        authority.consumer_boundary.selected_term_transport.privacy.selected_term_in_argv = "allowed";
      },
    ],
    [
      "term echo",
    ...
    const pathname = malformedAuthority(mutate);
    expect(validateGlossaryEntryContract(pathname)).toContain("consumer_boundary.selected_term_transport must authorize bounded no-review file or stdin input without argv content, output echo, host review, persistence, or shared Build stdin");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });

  it("rejects a primitive with the shared term removed", () => {
    const pathname = malformedAuthority((authority) => {
      authority.shared_primitive.required_fields = authority.shared_primitive.required_fields.filter((field: string) => field !== "term");
      delete authority.shared_primitive.fields.term;
    });
    expect(validateGlossaryEntryContract(pathname)).toEqual(expect.arrayContaining(["shared_primitive.required_fields must define the canonical entry shape once", "shared_primitive.fields.term is required"]));
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  it("rejects explicit-definition evidence count drift", () => {
    ...
    expect(validateGlossaryEntryContract(pathname)).toContain("personal_explicit_definition.evidence_count must be 1");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  it("rejects removal of the bounded personal-history rule", () => {
    ...
    expect(validateGlossaryEntryContract(pathname)).toContain("personal input bounded_rule is required");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });

  it("rejects a confirmed-variant guard detached from the active state validator", () => {
    const pathname = malformedAuthority((authority) => {
      authority.ownership_contracts.project.confirmed_variant_guard.owner = "packages/cli/src/validate/glossaryVariantGuard.ts#scanConfirmedVariantViolations";
    });
    expect(validateGlossaryEntryContract(pathname)).toContain("confirmed variant guard must run through the active state validator and reuse validated project glossary pairs with bounded exclusions");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });

  it("rejects drift from the shared Unicode caseless-exact identity runtime", () => {
    const pathname = malformedAuthority((authority) => {
      authority.consumer_boundary.deterministic_judgments.term_identity_runtime = "packages/cli/src/registries/other.ts#lowercase";
    });
    expect(validateGlossaryEntryContract(pathname)).toContain("consumer_boundary judgments must separate deterministic exact identity and valid gaps from host-reviewed inferred equivalence and invalid project state");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  it("rejects drift from the shared exact-case terminology occurrence runtime", () => {
    ...
    expect(validateGlossaryEntryContract(pathname)).toContain("term_occurrence must define one exact-case escaped literal matcher with ECMAScript identifier-continuation boundaries");
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  it.each([
    ...
    [
      ...
      (authority: Record<string, any>) => {
        authority.consumer_boundary.plan_integration.forbidden_effects = authority.consumer_boundary.plan_integration.forbidden_effects.filter((effect: string) => effect !== "plan_conflict");
      },
    ...
    [
      ...
      (authority: Record<string, any>) => {
        authority.consumer_boundary.plan_integration.mode.signals.unknown_or_ambiguous.result = "autonomous";
      },
      "consumer_boundary.plan_integration must define deterministic mode, ordered review, autonomous abstention, and an emitted event/reason/state writer intent",
    ],
    [
    ...
    [
      ...
      (authority: Record<string, any>) => {
        authority.consumer_boundary.plan_integration.autonomous_handoff_intent.caller_fields.push("caveat_id");
      },
      "consumer_boundary.plan_integration must define deterministic mode, ordered review, autonomous abstention, and an emitted event/reason/state writer intent",
    ],
    [
    ...
    [
      ...
      (authority: Record<string, any>) => {
        authority.consumer_boundary.plan_integration.exact_project_personal_unavailable.autonomous_handoff_intent = "emitted";
      },
    ...
    [
      ...
      (authority: Record<string, any>) => {
        authority.consumer_boundary.disclosure.plan_artifacts.forbidden_content = authority.consumer_boundary.disclosure.plan_artifacts.forbidden_content.filter((field: string) => field !== "personal_glossary_definition");
      },
    ...
    [
      ...
      (authority: Record<string, any>) => {
        authority.consumer_boundary.publication_isolation.project_publication_authority = "consumer_boundary";
      },
    ...
    [
      ...
      (authority: Record<string, any>) => {
        authority.consumer_boundary.downstream_gate.required_sections = authority.consumer_boundary.downstream_gate.required_sections.filter((section: string) => section !== "consumer_boundary.selected_term_transport");
      },
      "consumer_boundary.downstream_gate must block integration with section-specific validation and an actionable local command",
    ],
  ...
  it("rejects overlapping primary outcome predicates", () => {
    ...
    expect(validateGlossaryEntryContract(pathname)).toEqual(expect.arrayContaining([expect.stringMatching(/^consumer_boundary\.primary_selection must be exhaustive and non-overlapping: .*matched \[inferred_equivalence, no_applicable_entry\].*rerun agentera check validate vocabularyAuthority$/)]));
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  it("rejects an unexpectedly unclassified primary input state", () => {
    ...
    expect(validateGlossaryEntryContract(pathname)).toEqual(expect.arrayContaining([expect.stringMatching(/^consumer_boundary\.primary_selection must be exhaustive and non-overlapping: .*matched \[\].*rerun agentera check validate vocabularyAuthority$/)]));
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  ])("rejects contradictory primary outcome %s semantics", (field, outcomeName, contradictory, expected) => {
    const pathname = malformedAuthority((authority) => {
      authority.consumer_boundary.outcome_matrix[outcomeName][field] = contradictory;
    });
    expect(validateGlossaryEntryContract(pathname)).toContain(`consumer_boundary.outcome_matrix.${outcomeName}.${field} must be ${JSON.stringify(expected)} (found ${JSON.stringify(contradictory)}); restore the canonical primary-outcome semantics and rerun agentera check validate vocabularyAuthority`);
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });
  ...
  it.each([
    ...
    expect(validateGlossaryEntryContract(pathname)).toContain(`consumer_boundary.outcome_matrix.equivalent_exact_collision.${field} must be ${JSON.stringify(expected)} (found missing); restore the canonical primary-outcome semantics and rerun agentera check validate vocabularyAuthority`);
    fs.rmSync(path.dirname(pathname), { recursive: true, force: true });
  });

  it.each(CAPABILITY_FIXTURES)("accepts the governed $capability declaration", ({ capability, valid_declaration }) => {
    expect(validateGlossaryCapabilityImplementationClaim(capability, valid_declaration)).toEqual([]);
  });

  it.each(CAPABILITY_FIXTURES)("rejects the false $capability implementation claim", ({ capability, valid_declaration, false_implementation_claim }) => {
    expect(validateGlossaryCapabilityImplementationClaim(capability, false_implementation_claim)).toEqual([`${capability} glossary behavior is ${valid_declaration}; implemented is a false implementation claim`]);
  });
});
...
describe("persisted glossary entry boundary", () => {
  ...
  it("rejects every consumer-only persisted field declared by the authority", () => {
    for (const field of ["precedence", "collision", "review"]) {
      ...
      expect(validateGlossaryEntry(candidate, "project")).toContain(`entry contains forbidden persisted fields: ${field}`);
    }
  });
  ...
  it("rejects a date-shaped value that is not a calendar date", () => {
    ...
    expect(validateGlossaryEntry(candidate, "project")).toContain("temporal.observed_at must be an ISO date");
  });
});
...
describe("personal explicit-definition provenance", () => {
  ...
  it("rejects an explicit definition whose anchor does not resolve to its claimed record", () => {
    ...
    expect(errors).toContain("provenance.evidence[0].evidence_anchor must resolve to its retained source_id");
  });
});
...
describe("personal inferred-usage provenance", () => {
  ...
  it("rejects duplicate anchors even when two evidence rows are supplied", () => {
    ...
    expect(errors).toContain("inferred personal usage requires two distinct retained identities and anchors");
  });
});
...
describe("ownership and shared semantics", () => {
  ...
  it("rejects project-only fields attached to otherwise valid personal evidence", () => {
    ...
    expect(errors).toContain("provenance.evidence[0] contains fields forbidden for personal_explicit_definition: source_path, source_record_sha256");
  });
  ...
  it("keeps permanence independent while confidence remains an integer", () => {
    ...
    expect(validateGlossaryEntry(candidate, "personal", retainedHistory)).toContain("confidence must be an integer from protocol CS1-CS5");
