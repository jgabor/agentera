...
export async function runProducerReadinessWorkflow(executable: string, root: string): Promise<ProducerReadinessObservation> {
  ...
  const admitted = admission.admitPersonalGlossaryEvidence({ tiersDir, requestedTerms: ["signal braid"] });
  ...
  const proposal = audit.assessTerminologyDrift({
    ...
    concepts: [{
      ...
      severity: "warning",
      terms: [
        { term: "ProjectCanonical", evidence: [{ source_path: "src/canonical.ts", line: 1 }, { source_path: "src/canonical-extra.ts", line: 1 }] },
  ...
  const request = {
    ...
    proposal,
    confirmation: { proposal_digest: proposal.proposal_digest, confirmed_by: "user", confirmed_at: "2026-07-26T14:00:00Z" },
  };
  ...
  const confirmedVariantViolations = guard.scanPost30CruftViolations(project).filter((message: string) => message.includes("ProjectVariant")).length;
  ...
  const observedFiles = [profile.profilePath, path.join(project, ".agentera/glossary.yaml"), path.join(project, ".agentera/consumer-trap.yaml"), path.join(project, "profile-data/consumer-trap.txt")];
  ...
  for (const capability of startupCapabilities) {
    ...
    const grounding = served.profile?.grounding;
    assert.equal(grounding?.command, "agentera report profile-grounding --format json");
    assert.equal(grounding?.raw_profile_read, "forbidden");
    assert(served.instructions.includes(grounding.command), `${capability} instructions omit grounding command`);
    ...
    const command = grounding.command.split(" ").slice(1);
  ...
  const malformedProfiles = [
    (() => {
      ...
      return { family: "unowned", traps, profile: `# Profile\n\n## Glossary\n${trappedDocument(traps)}\n` };
    })(),
    (() => {
      const traps = groundingTraps("unmatched-marker");
      return { family: "unmatched-marker", traps, profile: `# Profile\n\n${PROFILE_GLOSSARY_START}\n## Glossary\n${trappedDocument(traps)}\n` };
    })(),
    (() => {
      const traps = groundingTraps("duplicate-marker");
      return { family: "duplicate-marker", traps, profile: `${ownedProfile(trappedDocument(traps))}${PROFILE_GLOSSARY_START}\n${traps.raw}\n` };
    })(),
    (() => {
      const traps = groundingTraps("invalid-json");
      return { family: "invalid-json", traps, profile: ownedProfile(`{${trapValues(traps).map((trap) => JSON.stringify(trap)).join(",")}`) };
    })(),
    (() => {
      const traps = groundingTraps("yaml-body");
      return { family: "yaml-body", traps, profile: ownedProfile(YAML.stringify(trappedEntry(traps)).trimEnd()) };
    })(),
    (() => {
    ...
    (() => {
      ...
      return { family: "duplicate-term", traps, profile: ownedProfile(trappedDocument(traps, entries)) };
    })(),
    (() => {
  ...
  for (const { family, traps, profile: malformed } of malformedProfiles) {
    ...
    assert(fs.readFileSync(profile.profilePath).equals(before), `${family} profile bytes changed after rejection`);
  ...
  const unavailable = invoke(executable, ["report", "profile-grounding", "--format", "json"], project, { ...env, AGENTERA_PROFILE_DIR: trappedProfileDir });
  ...
  const unreadable = invoke(executable, ["report", "profile-grounding", "--format", "json"], project, { ...env, NODE_OPTIONS: nodeOptions });
  assert.notEqual(unreadable.status, 0, "profile read failure was accepted");
  assertTrapsAbsent(unreadable, readTraps, "read");
  assert(fs.readFileSync(profile.profilePath).equals(beforeReadFailure), "profile bytes changed after read failure");
