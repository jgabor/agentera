...
export async function collectGarbageThenReadBaseline(request: InspectorRequest): Promise<{ usedSize: number }> {
  await request("HeapProfiler.collectGarbage");
  return (await request("Runtime.getHeapUsage")) as { usedSize: number };
}

export function measureColdCli(options: { args: string[]; project: string; home: string; repoRoot: string }): Promise<ColdMeasurement> {
  return measureColdCliOperation(options, "");
}

export function measureColdStateList(options: { project: string; repoRoot: string }): Promise<ColdMeasurement> {
...
export function measureColdCliWithRetainedAllocation(options: { args: string[]; project: string; home: string; repoRoot: string; elements: number }): Promise<ColdMeasurement> {
  ...
  return measureColdCliOperation(options, `globalThis.__coldMeasurementRetained = new Array(${options.elements}).fill(1);`);
}

function measureColdCliOperation(options: { args: string[]; project: string; home: string; repoRoot: string }, beforeOperation: string): Promise<ColdMeasurement> {
...
export function coldCliFailureEvidence(options: { operation: string; stdout: string; stderr: string; childArgs: string[]; env: NodeJS.ProcessEnv; exitCode?: number | null }): string {
...
function measureColdProcess(options: { operation: string; runner: string; cwd: string; env: NodeJS.ProcessEnv }): Promise<ColdMeasurement> {
  ...
  if (nodeOptionsUtf8Bytes > EFFECTIVE_NODE_OPTIONS_UTF8_LIMIT) {
    throw new Error(`effective NODE_OPTIONS is ${nodeOptionsUtf8Bytes} UTF-8 bytes; evidence limit ${EFFECTIVE_NODE_OPTIONS_UTF8_LIMIT}`);
  }
  ...
  return new Promise((resolve, reject) => {
    ...
    let settled = false;
    const timeout = setTimeout(() => fail(new Error("cold CLI did not complete serialized output")), 30_000);
    ...
    const fail = (error: Error, exitCode?: number | null): void => {
      ...
      reject(
        new Error(
          `${error.message}; evidence: ${coldCliFailureEvidence({
            operation,
            stdout,
            stderr,
            childArgs,
      ...
      socket.onopen = async () => {
        try {
          ...
          await request("Runtime.evaluate", {
            expression: "globalThis.__agenteraColdCliContinue(); delete globalThis.__agenteraColdCliContinue",
          });
