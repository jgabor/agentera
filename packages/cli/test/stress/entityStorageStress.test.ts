...
describe("entity publication stress", () => {
  it("names the repetition and violated invariant in bounded diagnostics", () => {
    expect(() => assertRaceInvariant(17, "exactly one publisher", false, { publishers: 2 })).toThrow(
  ...
  it("preserves atomicity and cleanup across 100 simultaneous stale-lock races", async () => {
    for (let repetition = 1; repetition <= 100; repetition += 1) {
      try {
        ...
        const { results } = await concurrentPublication(root, `-stale-${repetition}`, { repetition });
        ...
        assertRaceInvariant(repetition, "canonical writer lock cleanup", !fs.existsSync(lockPath), { lockPath });
