...
function planRecord(status: string, created: string, title = "Canonical plan"): Record<string, unknown> {
  return {
    header: { level: "light", created, status, title },
    what: "test",
    why: "test",
...
function cutoverProject(): string {
  ...
  entity(root, "progress", "progress_cycle", "aaaaaaaaaa", {
    ...
    type: "fix",
    phase: "build",
    what: "canonical progress",
    context: { intent: "test" },
  });
  entity(root, "decisions", "decision", "bbbbbbbbbb", {
    date: "2026-07-17",
    question: "Canonical?",
    context: "test",
  ...
  entity(root, "health", "health_audit", "cccccccccc", {
    date: "2026-07-17",
  ...
  entity(root, "plan", "plan_task", "eeeeeeeeee", {
    plan: "dddddddddd",
    name: "Canonical task",
    status: "pending",
    depends_on: [],
  ...
  entity(root, "todo", "todo_item", "ffffffffff", {
    severity: "critical",
    status: "open",
    description: "Canonical TODO",
  });
  entity(root, "docs", "documentation_inventory_entry", "gggggggggg", {
    document: "Canonical docs",
...
describe("final lifecycle protocol", () => {
  it("fails unknown marker-absent commands read-only with one exact preview recovery", () => {
    ...
    for (const args of [
      ...
      expect(JSON.parse(result.out).error).toEqual(
        expect.objectContaining({
          class: "migration_required",
          recovery: commandText(["npx", "-y", "agentera@next", "upgrade", "--channel", "development", "--project", root, "--dry-run"]),
        }),
      );
  ...
  it("rejects removed mutation operations without touching marker-absent state", () => {
    const root = project();
    const before = treeDigest(root);
    const recovery = commandText(["npx", "-y", "agentera@next", "upgrade", "--channel", "development", "--project", root, "--yes"]);
    for (const args of [
  ...
  it("uses canonical entities for startup and compact while ignoring hostile aggregates", () => {
    ...
    expect(prime.rc, JSON.stringify(prime)).toBe(0);
    expect(prime.out).not.toContain("HOSTILE_AGGREGATE");
    expect(prime.out).toContain("aaaaaaaaaa");
    expect(prime.out).toContain('"artifact": "progress"');
    ...
    expect(plan.first_pending).toEqual(
      expect.objectContaining({
        id: "eeeeeeeeee",
        artifact: "plan",
    ...
    for (const capability of CAPABILITY_NAMES) {
      const payload = capability === "status" ? buildStatusCapabilityContextPayload(state) : buildPrimeCapabilityContextPayload(state, capability);
    ...
    expect(compact.rc, compact.err).toBe(0);
    expect(compact.out).toContain("canonical entity state");
    expect(compact.out).not.toContain("HOSTILE_AGGREGATE");
    expect(treeDigest(root)).toBe(before);
  });
  ...
  it("selects eligible plans and objectives older than twenty ineligible records", () => {
    ...
    entity(root, "plan", "plan_task", "zzzzzzzzzb", {
      plan: "zzzzzzzzza",
      name: "Older pending task",
      status: "pending",
      depends_on: [],
      acceptance: ["pass"],
    });
    ...
    expect(orientation).toMatchObject({
      plan: {
        id: "zzzzzzzzza",
        artifact: "plan",
        first_pending: { id: "zzzzzzzzzb", artifact: "plan" },
      },
      state_presence: { active: { objective: true } },
    });
    expect(orientation.next_action.alternatives).toContainEqual(
      expect.objectContaining({
        id: "zzzzzzzzzc",
        artifact: "objective",
        capability: "optimize",
  ...
  it.each([["plan", "plan", "plan", "open", planRecord] as const, ["objective", "objective", "objective", "active", objectiveRecord] as const])("fails actionably instead of selecting the first competing %s", (_name, artifact, boundary, status, makeRecord) => {
    const root = cutoverProject();
      ...
      expect(error).toMatchObject({
        ...
        details: {
          open_plan_candidates: {
            total: 2,
            sample_ids: ["yyyyyyyyya", "yyyyyyyyyb"],
            omitted_count: 0,
          },
        },
      });
  ...
  it("fails startup on dangling canonical relationships without consulting hostile aggregates", () => {
    ...
    entity(root, "plan", "plan_task", "xxxxxxxxxx", {
      plan: "dddddddddd",
      name: "Dangling",
      status: "pending",
      depends_on: ["wwwwwwwwww"],
      acceptance: ["pass"],
    });
    const result = capture(root, ["prime", "--format", "json"]);
    expect(result.rc).not.toBe(0);
