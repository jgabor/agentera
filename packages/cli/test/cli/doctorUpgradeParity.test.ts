...
import { APP_MANUAL_REVIEW_NEEDED, APP_OUTDATED, APP_REPAIR_NEEDED, APP_UP_TO_DATE, buildDoctorStatus, doctorParityJsonEnvelope, DOCTOR_PARITY_JSON_KEYS } from "../../src/upgrade/doctor.js";
import { STATUS_APPLIED, STATUS_MANUAL_REVIEW_NEEDED, STATUS_NO_CHANGES_NEEDED, STATUS_READY_TO_APPLY } from "../../src/upgrade/compatibility.js";
...
const ORACLE = JSON.parse(fs.readFileSync(path.join(__dirname, "fixtures/oracle/parity-remaining-families.json"), "utf8")) as {
  families: {
...
const FORBIDDEN_REPAIR_PHRASES = ["bundle freshness", "bundle refresh", "app refresh required", "bundle freshness gap"];

const CANONICAL_DOCTOR_STATUSES = new Set([APP_UP_TO_DATE, APP_OUTDATED, APP_REPAIR_NEEDED, APP_MANUAL_REVIEW_NEEDED]);

const CANONICAL_UPGRADE_LIFECYCLE = new Set([STATUS_READY_TO_APPLY, STATUS_APPLIED, STATUS_NO_CHANGES_NEEDED, STATUS_MANUAL_REVIEW_NEEDED]);
...
describe("doctor upgrade safety parity (D56 T4)", () => {
  ...
  it("doctorParityJsonEnvelope emits oracle structural keys only", () => {
    ...
    const cls = classifyDrift(normalizeEnvelope(envelope) as Record<string, unknown>, requiredKeys, literalPins, doctorSpec.forbiddenSubstrings);
  ...
  const lifecycleTransitions: Array<{
  ...
  }> = [
    {
      ...
      pass: () => {
        ...
        return {
          ...
          serialized: JSON.stringify(doctorParityJsonEnvelope(status)),
        };
      },
    },
    {
    ...
    },
    {
      ...
      pass: () => {
        ...
        fs.cpSync(path.join(__dirname, "../upgrade/fixtures/v2-yaml-project"), project, {
          recursive: true,
        });
  ...
  for (const transition of lifecycleTransitions) {
    it(`emits canonical ${transition.name} lifecycle metadata (pass)`, () => {
      ...
      if (transition.name === "outdated" || transition.name === "repair_needed" || transition.name === "manual_review_needed") {
        ...
        expect(status).toBe(transition.name === "outdated" ? APP_OUTDATED : transition.name === "repair_needed" ? APP_REPAIR_NEEDED : APP_MANUAL_REVIEW_NEEDED);
      } else {
        expect(CANONICAL_UPGRADE_LIFECYCLE.has(status)).toBe(true);
        expect(status).toBe(transition.name === "ready_to_apply" ? STATUS_READY_TO_APPLY : STATUS_NO_CHANGES_NEEDED);
      }
