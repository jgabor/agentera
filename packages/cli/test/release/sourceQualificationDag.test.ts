...
function overlapEvidence(buildRoot = "/tmp/private-build/generation-a") {
  return {
    ...
    participants: {
      ...
      build: {
        command: [...gate("build").command, "--", "--output-root", buildRoot],
        elapsedMs: 30,
        status: "pass",
      },
    },
...
function result(name: string, specification?: { environment?: NodeJS.ProcessEnv }) {
  const overlapRoot = specification?.environment?.AGENTERA_GENERATED_OVERLAP_ROOT;
  const buildRoot = overlapRoot ? path.join(overlapRoot, "private-build", "generation-a") : undefined;
  return {
    name,
    elapsedMs: 10,
    stdout: name === "generated-overlap" ? JSON.stringify(overlapEvidence(buildRoot)) : name === "performance" ? performanceStdout() : `${name} passed\n`,
    stderr: "",
  };
}
...
function createOverlapPackageRoot(workRoot: string) {
  const repo = path.join(workRoot, "source");
  ...
  for (const args of [
    ["init", "--quiet"],
    ["add", "."],
    ["-c", "user.name=Agentera Tests", "-c", "user.email=tests@agentera.invalid", "-c", "commit.gpgsign=false", "commit", "--quiet", "-m", "fixture"],
  ]) {
...
describe("source qualification DAG", () => {
  it("reserves workers for the long source participant without widening its peers", () => {
    ...
    expect(generatedOverlapParticipantEnvironment("source", environment)).toEqual({
      VITEST_MAX_WORKERS: "3",
    });
    expect(generatedOverlapParticipantEnvironment("package", environment)).toEqual({});
    expect(() =>
      generatedOverlapParticipantEnvironment("source", {
  ...
  it("runs one-worker performance alone, then capacity serially, before the parallel reader barrier", async () => {
    ...
    expect(qualification.gates.find((entry: any) => entry.name === "source").observation).toMatchObject({
      command: gate("source").command,
      files: 10,
      tests: 40,
      pending: [],
    });
    expect(qualification.gates.find((entry: any) => entry.name === "performance").observation).toMatchObject({
      inventoryFiles: 3,
      evidence: {
        status: "pass",
        samples: 1,
    ...
    expect(qualification.gates.find((entry: any) => entry.name === "capacity")).toMatchObject({
      ...
      observation: { inventoryFiles: 2 },
    });
    expect(qualification.gates.filter((entry: any) => entry.phase === "barrier-b").map((entry: any) => entry.name)).toEqual(["compact", "capability-contract", "activation-conjunction"]);
  ...
  it("keeps the parent-owned overlap root through barrier B and removes it after the DAG", async () => {
    ...
    let overlapRoot = "";
    let generatedOwnerRoot = "";
    let barrierObserved = false;
    const marker = () => path.join(overlapRoot, "private-build", "generation-a", "alive");

    await runSourceQualificationDag({
      repo: REPO_ROOT,
      gates: GATES,
      clock: () => 0,
      wallClock: () => 1_000,
      createState: (name: string) => {
        const root = fs.mkdtempSync(path.join(os.tmpdir(), `agentera-owner-lifetime-${name}-`));
        ownerRoots.push(root);
        if (name === "generated-overlap") generatedOwnerRoot = root;
        return {
          root,
          environment: {},
      ...
      startOwner: (specification: any) => {
        if (specification.name === "generated-overlap") {
          overlapRoot = specification.environment.AGENTERA_GENERATED_OVERLAP_ROOT;
          fs.mkdirSync(path.dirname(marker()), { recursive: true });
          fs.writeFileSync(marker(), "alive\n");
        }
        if (["compact", "capability-contract", "activation-conjunction"].includes(specification.name)) {
          expect(fs.existsSync(generatedOwnerRoot)).toBe(false);
    ...
    expect(fs.existsSync(overlapRoot)).toBe(false);
    expect(ownerRoots.every((root) => !fs.existsSync(root))).toBe(true);
  });
    ...
    await expect(
      runSourceQualificationDag({
        ...
        startOwner: (specification: any) => {
          ...
          if (specification.name === "generated-overlap") {
            overlapRoot = specification.environment.AGENTERA_GENERATED_OVERLAP_ROOT;
            fs.writeFileSync(path.join(overlapRoot, "owned"), "owned\n");
          }
          return {
            name: specification.name,
            cancellable: specification.cancellable,
      ...
      failures: expect.arrayContaining([expect.objectContaining({ name: "stress", status: "failed" }), expect.objectContaining({ name: "generated-overlap", status: "failed" }), expect.objectContaining({ name: "typecheck", status: "cancelled" })]),
      completed: [],
    });
    expect(overlapSettled).toBe(true);
    expect(cancelled).toContain("typecheck");
    expect(cancelled).not.toContain("generated-overlap");
    expect(started).not.toEqual(expect.arrayContaining(["performance", "capacity", "compact", "capability-contract"]));
    expect(fs.existsSync(overlapRoot)).toBe(false);
  });
      ...
      runSourceQualificationDag({
        ...
        startOwner: (specification: any) => {
          ...
          return {
            name: specification.name,
            cancellable: specification.cancellable,
            promise: specification.name === "performance" ? Promise.reject(ownerError("performance", "performance exceeded its remaining source deadline")) : Promise.resolve(result(specification.name, specification)),
            cancel: () => undefined,
          };
        },
        readGeneratedState: () => {
  ...
  it("blocks reader checks when the serial capacity barrier fails", async () => {
    const started: string[] = [];
    await expect(
      runSourceQualificationDag({
        repo: REPO_ROOT,
        ...
        startOwner: (specification: any) => {
          ...
          return {
            name: specification.name,
            cancellable: specification.cancellable,
            promise: specification.name === "capacity" ? Promise.reject(ownerError("capacity", "capacity fixture failed")) : Promise.resolve(result(specification.name, specification)),
            cancel: () => undefined,
          };
        },
  ...
  it("blocks receipt completion when a post-build reader fails and cancels its peer", async () => {
    ...
    await expect(
          ...
          const promise =
            specification.name === "compact"
              ...
              : specification.name === "capability-contract"
                ...
                : Promise.resolve(result(specification.name, specification));
          return {
            name: specification.name,
            cancellable: specification.cancellable,
            promise,
      ...
      failures: expect.arrayContaining([expect.objectContaining({ name: "compact", status: "failed" }), expect.objectContaining({ name: "capability-contract", status: "cancelled" })]),
    });
  ...
  it("rejects a reader barrier that changes generation or leaves a lease", async () => {
    ...
    await expect(
      runSourceQualificationDag({
        ...
        readGeneratedState: () => (reads++ < 3 ? { generation: "generation-a", leases: [] } : { generation: "generation-b", leases: ["reader.lease"] }),
      }),
    ).rejects.toMatchObject({
  ...
  it.each([
    ...
    const runConcurrent = async (specifications: any[]) => {
      ...
      const values = Object.fromEntries(specifications.map((specification) => [specification.name, { ...result(specification.name, specification), timeoutMs: specification.timeoutMs }]));
  ...
  it("uses a fake deadline to stop starting overlap work and settle owned children before handoff", async () => {
    let now = 1_000;
  ...
  it("returns complete overlap evidence through the cooperative runner success path", async () => {
    ...
    try {
        ...
        startParticipant: (name: string, command: string[]) => {
          ...
          return {
            ...
            promise: Promise.resolve({
              command,
              elapsedMs: 10,
              stdout: name === "invocation" ? "3.0.0-dev.41" : "",
            }),
            cancel: () => undefined,
          };
        },
      ...
      const privateRoot = started.find(({ name }) => name === "build")!.command.at(-1)!;
      ...
      expect(started).toEqual([
        ...
        {
          name: "build",
          command: ["pnpm", "-C", "packages/cli", "build", "--", "--output-root", privateRoot],
        },
        { name: "package", command: ["pnpm", "-C", "packages/cli", "run", "verify:package"] },
        {
          name: "invocation",
          command: [process.execPath, path.join(privateRoot, "dist/bin/agentera.js"), "--version"],
        },
      ]);
      expect(fs.existsSync(workRoot)).toBe(false);
    } finally {
  ...
  it("cancels the complete Windows child process tree", () => {
    ...
    expect(invocations).toEqual([["taskkill", ["/PID", "4321", "/T"], { windowsHide: true, stdio: "ignore" }]]);
  ...
  it("keeps generated-overlap policy aligned with release qualification", () => {
    ...
    expect(RELEASE_CONTRACT.qualification.source.performanceEvidenceSchema).toBe(policy.owners.performance.evidence.schema_version);
