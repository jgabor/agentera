...
import {
  ...
  validateRetainedReferenceAuthority,
} from "../../src/validate/retainedReferenceAuthority.js";
...
function fixture(): { root: string; authority: Record<string, any> } {
  ...
  write(root, "references/runbook.md", [
    "# Runbook",
    "",
    ...
    inventory: [
      {
        ...
        consumers: [{ kind: "runtime", module: "packages/cli/src/current.ts", symbol: "loadCurrent", consumption: "loads" }],
      },
      {
        ...
        consumers: [{ kind: "runtime", module: "packages/cli/src/migration.ts", symbol: "loadMigration", consumption: "loads" }],
      },
      {
      ...
      },
      {
        ...
        consumers: [{ kind: "validator", module: "packages/cli/src/validator.ts", symbol: "validateManifest", consumption: "loads" }],
      },
...
describe("retained reference authority", () => {
  ...
  it("rejects a current reference supported only by a test module", () => {
    ...
    expect(validateRetainedReferenceAuthority(root)).toContain(
      "references/current.yaml: consumers[0].module must name a contained production packages/cli src or scripts module",
    );
  });

  it("rejects an emitted path and an unrelated read as consumption evidence", () => {
    const { root } = fixture();
    write(root, "packages/cli/src/current.ts", [
      "export function loadCurrent() {",
      '  console.log("references/current.yaml");',
      '  return fs.readFileSync("references/other.yaml");',
      "}",
      "",
    ].join("\n"));
    expect(validateRetainedReferenceAuthority(root)).toContain(
      "references/current.yaml: consumers[0] must read or parse the exact reference; unrelated reads and emitted strings do not count",
    );
  });
  ...
  it("rejects reference literals outside the reader path argument", () => {
    ...
    write(root, "packages/cli/src/current.ts", [
      "export function loadCurrent() {",
      '  return fs.readFileSync("references/other.yaml", ["references", "current.yaml"] as any);',
      "}",
      "",
    ].join("\n"));
    expect(validateRetainedReferenceAuthority(root)).toContain(
  ...
  it("rejects a no-op loader consumer", () => {
    ...
    rewrite(root, authority);
    expect(validateRetainedReferenceAuthority(root)).toContain(
      "references/meta/retained-reference-authority.yaml: consumers[0] must read or parse the exact reference; unrelated reads and emitted strings do not count",
    );
  });

  it("rejects a validator-like symbol that only reads data under the removed validates mode", () => {
    const { root, authority } = fixture();
    write(root, "packages/cli/src/validator.ts", [
      "export function validateManifest() {",
      '  fs.readFileSync("references/meta/retained-reference-authority.yaml");',
      "  return [];",
      "}",
      "",
    ].join("\n"));
    ...
    rewrite(root, authority);
    expect(validateRetainedReferenceAuthority(root)).toContain(
      "references/meta/retained-reference-authority.yaml: consumers[0].consumption must be loads",
    );
  });
  ...
  it("rejects a dead production export", () => {
    ...
    authority.inventory[0].production_owner = { module: "packages/cli/src/dead.ts", symbol: "loadDead" };
    authority.inventory[0].consumers[0] = { kind: "runtime", module: "packages/cli/src/dead.ts", symbol: "loadDead", consumption: "loads" };
    rewrite(root, authority);
    expect(validateRetainedReferenceAuthority(root)).toContain(
      "references/current.yaml: consumers[0].symbol is not reachable from a production CLI or package-script entrypoint",
    );
  ...
  it("rejects a dead export from an otherwise reachable production module", () => {
    const { root, authority } = fixture();
    write(root, "packages/cli/src/current.ts", [
      'export function loadCurrent() { return fs.readFileSync("references/current.yaml"); }',
      'export function loadDead() { return fs.readFileSync("references/current.yaml"); }',
      "",
    ].join("\n"));
    authority.inventory[0].production_owner = { module: "packages/cli/src/current.ts", symbol: "loadDead" };
    authority.inventory[0].consumers[0] = { kind: "runtime", module: "packages/cli/src/current.ts", symbol: "loadDead", consumption: "loads" };
    rewrite(root, authority);
    expect(validateRetainedReferenceAuthority(root)).toContain(
      "references/current.yaml: consumers[0].symbol is not reachable from a production CLI or package-script entrypoint",
    );
  ...
  it("rejects a symlinked production module", () => {
    ...
    rewrite(root, authority);
    expect(validateRetainedReferenceAuthority(root)).toContain(
      "references/current.yaml: consumers[0].module must be a contained regular production file, not a symlink",
    );
  });
  ...
  it("rejects a migration-only path outside its allowed migration roots", () => {
    ...
    rewrite(root, authority);
    expect(validateRetainedReferenceAuthority(root)).toContain(
      "references/analysis/migration.yaml: migration-only references must remain in references/adapters or references/cli",
    );
  });
