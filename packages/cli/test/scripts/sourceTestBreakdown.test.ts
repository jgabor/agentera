...
describe("source test timing breakdown", () => {
    ...
    const summary = summarizeSourceReport({
      ...
      testResults: [
        ...
        },
        {
          ...
          endTime: 3_000,
          assertionResults: [
            { fullName: "failed CLI test", status: "failed", duration: 800 },
          ],
        },
        {
          ...
          endTime: 4_500,
          assertionResults: [
            { fullName: "fast state test", status: "passed", duration: 500 },
          ],
        },
      ],
    }, { testRoot });

    expect(summary.counts).toEqual({ files: 3, tests: 4, passed: 2, failed: 1, skipped: 1, todo: 0 });
  ...
  it("rejects malformed timing evidence", () => {
    ...
    }, { testRoot })).toThrow("ends before it starts");
  });
});
