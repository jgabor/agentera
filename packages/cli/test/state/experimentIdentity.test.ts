...
describe("objective-scoped experiment identity", () => {
  it("scopes experiment zero and later numbers to stable objective identity", () => {
    const first = inspectExperimentIdentities(objectiveId, {
      experiments: [{ number: 0 }, { number: 2 }],
    });
    const second = inspectExperimentIdentities(otherObjectiveId, { experiments: [{ number: 0 }] });

    expect(first.entries.map((entry) => entry.stableId)).toEqual([`${objectiveId}/experiment:0`, `${objectiveId}/experiment:2`]);
  ...
  it("keeps persisted objective identity through title and canonical path rename", () => {
    ...
    fs.renameSync(path.join(root, ".agentera", "optimize", "old-title"), path.join(root, ".agentera", "optimize", "new-title"));
  ...
  it("retains missing and duplicate legacy entries with explicit caveats", () => {
    ...
    expect(projection.caveats).toEqual(expect.arrayContaining([expect.stringContaining("duplicate experiment number 0"), expect.stringContaining("missing experiment number")]));
  });
  ...
  it("fails ambiguous or duplicate publication identity before effects", () => {
    ...
    expect(() => validateExperimentPublicationIdentity(discovery, projection, objectiveId, 0)).toThrowError(expect.objectContaining<Partial<ExperimentIdentityError>>({ className: "ambiguous" }));
    expect(() => validateExperimentPublicationIdentity(discovery, projection, objectiveId, -1)).toThrowError(expect.objectContaining<Partial<ExperimentIdentityError>>({ className: "invalid_request" }));
