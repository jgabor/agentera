...
process.chdir(root);
try {
  const rc = main(["node", "agentera", "state", "experiments", "publish", "--objective", objective, "--input", "-", "--format", "json"], {
    out: (text) => {
      out += text;
    },
    err: (text) => {
  ...
  const body = out.trim().startsWith("{") ? JSON.parse(out) : null;
  fs.writeFileSync(result, JSON.stringify({ ok: rc === 0, error: body?.error?.message ?? err }));
} finally {
  process.chdir(cwd);
}
