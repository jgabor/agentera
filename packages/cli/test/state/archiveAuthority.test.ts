...
function authorityErrors(value: Record<string, any>): string[] {
  ...
  if (value.api?.direct_get?.command !== "npx -y agentera@next state <artifact-id> get --id ID") errors.push("direct_get.command");
  if (value.api?.list?.command !== "npx -y agentera@next state <artifact-id> list [--limit N] [--cursor TOKEN] [--ids-only | --fields FIELDS]") errors.push("list.command");
...
describe("state storage authority", () => {
  ...
  it("publishes exact get/list syntax, ordering, cursor snapshots, and limits", () => {
    ...
    expect(Object.keys(api)).not.toEqual(expect.arrayContaining(["backfill", "migrate"]));
  });
