...
function project(entity = true): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agentera-objective-entities-"));
  roots.push(root);
  if (entity) {
...
function objective(title: string, status = "open"): Record<string, unknown> {
  return {
    ...
    objective: {
      description: `Reduce ${title}`,
      why: "Users wait",
      measurement: "Locked p95 harness",
      constraints: [],
    },
...
function experiment(label: string, status: "baseline" | "kept" = "kept", date = "2026-07-17 09:00"): Record<string, unknown> {
  return {
    ...
    method: "Run locked harness",
    change: "Cache keys",
    metric: { primary_value: "80 ms", delta_vs_baseline: "-20 ms" },
    regression: "pnpm test passed",
    status,
    conclusion: "Measured result",
    provenance: { command: "locked-harness", revision: "abc123" },
  };
}
function capture(root: string, args: string[], input?: Record<string, unknown>): { rc: number; out: string; err: string; json: any } {
  ...
  try {
    const rc = main(["node", "agentera", ...args], {
      ...
      stdin: input ? () => dumpYamlMapping(input) : undefined,
    });
...
function createObjective(root: string, title: string, status = "open"): any {
  const result = capture(root, ["state", "objective", "create", "--input", "-", "--format", "json"], objective(title, status));
...
function publish(root: string, objectiveId: string, record: Record<string, unknown>): any {
  const result = capture(root, ["state", "experiments", "publish", "--objective", objectiveId, "--input", "-", "--format", "json"], record);
  expect(result.rc, result.err || result.out).toBe(0);
  return result.json;
}
  ...
  const children = results.map(
    (result, index) =>
      new Promise<void>((resolve, reject) => {
        const child = spawn(process.execPath, [baselineWorker], {
          cwd: path.resolve(import.meta.dirname, "../.."),
          env: {
            ...sourceSubprocessEnv(),
            AGENTERA_BOOTSTRAP_SOURCE_ROOT: path.resolve(import.meta.dirname, "../../../.."),
            AGENTERA_BASELINE_ROOT: root,
            AGENTERA_BASELINE_OBJECTIVE: objectiveId,
            AGENTERA_BASELINE_READY: ready[index],
            AGENTERA_BASELINE_START: start,
            AGENTERA_BASELINE_RESULT: result,
          },
          stdio: "pipe",
        });
        let stderr = "";
        ...
        child.on("exit", (code) => (code === 0 ? resolve() : reject(new Error(`baseline worker exited ${code}: ${stderr}`))));
      }),
  );
  const deadline = Date.now() + 10_000;
  while (!ready.every((file) => fs.existsSync(file))) {
    if (Date.now() > deadline) throw new Error("baseline workers did not become ready");
...
describe("objective and experiment entity authority", () => {
  it("creates and replaces independent objective entities with bare project-wide IDs", () => {
    ...
    expect(created).toMatchObject({
      ...
      record: { header: { title: "latency" } },
    });
    expect(created.record.header.id).toBeUndefined();
    const before = path.join(root, `.agentera/entities/objective/objective/${created.id}.yaml`);
    ...
    expect(result.json).toMatchObject({
      id: created.id,
      record: { header: { title: "latency v2" } },
    });
  ...
  it("rejects every canonical identity alias on objective create and update before effects", () => {
    ...
    const file = path.join(root, `.agentera/entities/objective/objective/${owner.id}.yaml`);
    ...
    for (const alias of ["stable_id", "artifact_id", "entry_number", "number", "task_number", "experiment_number", "plan_id", "objective_id", "type_prefixed_id", "id", "artifact"]) {
      ...
      for (const [verb, selector] of [
        ["create", []],
        ["update", ["--id", owner.id]],
      ] as const) {
        const result = capture(root, ["state", "objective", verb, ...selector, "--input", "-", "--format", "json"], input);
        expect(result.rc, alias).toBe(2);
        expect(result.json.error.class).toBe("schema_violation");
        expect(result.json.error.violations.join(" ")).toContain(alias);
        expect(fs.readFileSync(file)).toEqual(before);
      }
    }
    const nested = objective("invalid");
    (nested.header as Record<string, unknown>).id = "forbidden";
    expect(capture(root, ["state", "objective", "create", "--input", "-", "--format", "json"], nested).rc).toBe(2);
  ...
  it("publishes immutable full experiments related to exactly one objective and replays only exact content", () => {
    ...
    expect(baseline).toMatchObject({
      ...
      record: {
        objective: owner.id,
        status: "baseline",
        provenance: { command: "locked-harness" },
      },
    });
    expect(baseline.record.number).toBeUndefined();
    const file = path.join(root, `.agentera/entities/experiments/experiment/${baseline.id}.yaml`);
  ...
  it("requires one semantic baseline and rejects missing, duplicate, or conflicting ownership before effects", () => {
    ...
    expect(missing.json.error.class).toBe("conflict");
    expect(fs.existsSync(path.join(root, ".agentera/entities/experiments"))).toBe(false);
    const baseline = publish(root, owner.id, experiment("baseline", "baseline"));
    ...
    expect(duplicate.rc).not.toBe(0);
    expect(duplicate.json.error.class).toBe("conflict");
    expect(fs.readdirSync(path.dirname(path.join(root, `.agentera/entities/experiments/experiment/${baseline.id}.yaml`)))).toHaveLength(1);
    const other = createObjective(root, "throughput");
    const envelope = loadYamlMapping(fs.readFileSync(path.join(root, `.agentera/entities/experiments/experiment/${baseline.id}.yaml`), "utf8"));
    (envelope.record as Record<string, unknown>).objective = other.id;
    fs.mkdirSync(path.join(root, ".agentera/entities/experiments/other"), { recursive: true });
    fs.writeFileSync(path.join(root, `.agentera/entities/experiments/other/${baseline.id}.yaml`), dumpYamlMapping(envelope));
    ...
    expect(blocked.json.error.class).toBe("conflict");
    expect(fs.readdirSync(path.join(root, ".agentera/entities/experiments/experiment"))).toEqual(before);
  });

  it("serializes concurrent baseline publication so one process wins without residue", async () => {
    const root = project();
    const owner = createObjective(root, "latency");
    const results = await concurrentBaselines(root, owner.id);
    expect(results.filter(({ ok }) => ok)).toHaveLength(1);
    expect(results.filter(({ ok }) => !ok)).toEqual([expect.objectContaining({ error: expect.stringMatching(/exactly one|baseline|conflict/i) })]);
    const directory = path.join(root, ".agentera/entities/experiments/experiment");
    expect(fs.readdirSync(directory).filter((name) => name.endsWith(".yaml"))).toHaveLength(1);
  ...
  it("gets and lists full records in declared temporal order with bounded exact recovery and snapshot cursors", () => {
    const root = project();
    const owner = createObjective(root, "latency");
    const baseline = publish(root, owner.id, experiment("baseline", "baseline", "2026-07-16 09:00"));
    const candidate = publish(root, owner.id, experiment("candidate", "kept", "2026-07-17 09:00"));
    const exact = capture(root, ["state", "experiments", "get", "--id", baseline.id, "--format", "json"]);
    expect(exact.rc).toBe(0);
    expect(exact.json.entry.record).toEqual(baseline.record);
    expect(exact.json.entry.provenance.path).toContain(baseline.id);
    const first = capture(root, ["state", "experiments", "list", "--objective", owner.id, "--limit", "1", "--format", "json"]);
    expect(first.rc).toBe(0);
    expect(first.json.entries[0].id).toBe(candidate.id);
    expect(first.json.next_cursor).toBeTruthy();
    expect(first.json.retrieval.get).toContain("--id ID");
    ...
    const second = capture(root, ["state", "experiments", "list", "--objective", owner.id, "--limit", "1", "--cursor", first.json.next_cursor, "--format", "json"]);
    expect(second.rc).toBe(0);
    expect(second.json.entries[0].id).toBe(baseline.id);
    publish(root, owner.id, experiment("later", "kept", "2026-07-18 09:00"));
    const stale = capture(root, ["state", "experiments", "list", "--objective", owner.id, "--limit", "1", "--cursor", first.json.next_cursor, "--format", "json"]);
  ...
  it("bounds Unicode experiment pages and executes objective-bound omission recovery", () => {
    ...
    expect(exact.json.entry).toMatchObject({
      ...
      record: {
        objective: owner.id,
        label: published[0].record.label,
        hypothesis: published[0].record.hypothesis,
      },
    });
  });
  ...
  it("preserves objective, topic, and status in deterministic filtered continuations", () => {
    const root = project();
    const owner = createObjective(root, "filtered experiments");
    ...
    expect(page).toMatchObject({
      filters: { objective: owner.id, topic: "Cache helps", status: "kept" },
      counts: { candidate: 2, returned: 1, continuation: 1 },
    });
  ...
  it("lists objectives and infers only one active objective for the compatibility query", () => {
    const root = project();
    const first = createObjective(root, "first");
    createObjective(root, "closed", "closed");
    expect(capture(root, ["state", "objective", "get", "--id", first.id, "--format", "json"]).json.entry.record.header.title).toBe("first");
    const page = capture(root, ["state", "objective", "list", "--limit", "1", "--format", "json"]);
    ...
    const help = capture(root, ["state", "experiments", "--help"]);
    expect(help.out).toContain("get --id ID");
    expect(help.out).not.toContain("get --objective OBJECTIVE_ID --number");
    const bare = capture(root, ["state", "objective", "--format", "json"]);
    expect(bare.rc).toBe(2);
    expect(bare.json.error).toMatchObject({
      class: "invalid_request",
      recovery: expect.stringContaining("state objective list"),
    });
    createObjective(root, "second");
    const stale = capture(root, ["state", "objective", "list", "--limit", "1", "--cursor", page.json.next_cursor, "--format", "json"]);
    expect(stale.rc).toBe(1);
    expect(stale.json.error.class).toBe("cursor_snapshot_unavailable");
  });
  ...
  it("keeps legacy optimize publication and files unchanged while entity mode rejects legacy selectors and paths", () => {
    ...
    const directory = path.join(legacy, ".agentera/optimize/latency");
    fs.mkdirSync(directory, { recursive: true });
    const legacyObjective = objective("legacy");
    (legacyObjective.header as Record<string, unknown>).id = "objective:123e4567-e89b-42d3-a456-426614174000";
    fs.writeFileSync(path.join(directory, "objective.yaml"), dumpYamlMapping(legacyObjective));
    const result = capture(legacy, ["state", "experiments", "publish", "--objective", String((legacyObjective.header as any).id), "--number", "0", "--input", "-", "--format", "json"], experiment("baseline", "baseline"));
    expect(result.rc).toBe(1);
    expect(result.json.error.class).toBe("migration_required");
    expect(fs.existsSync(path.join(directory, "experiments.yaml"))).toBe(false);
    expect(fs.existsSync(path.join(legacy, ".agentera/entities"))).toBe(false);
    const entity = project();
    const owner = createObjective(entity, "entity");
    expect(capture(entity, ["state", "experiments", "publish", "--objective", owner.id, "--number", "0", "--input", "-", "--format", "json"], experiment("baseline", "baseline")).rc).toBe(2);
    expect(fs.existsSync(path.join(entity, ".agentera/optimize"))).toBe(false);
  });

  it("renders final explain contracts before and after cutover", () => {
    const entity = project();
    const objectiveCreate = capture(entity, ["state", "objective", "explain", "--format", "json"]);
    const objectiveUpdate = capture(entity, ["state", "objective", "explain", "--verb", "update", "--format", "json"]);
    const experiments = capture(entity, ["state", "experiments", "explain", "--format", "json"]);
    const rendered = JSON.stringify([objectiveCreate.json, objectiveUpdate.json, experiments.json]);
    expect(objectiveCreate.json.fields).toEqual([]);
    expect(objectiveCreate.json.example).toBe("agentera state objective create --input objective.yaml");
    expect(objectiveUpdate.json.fields).toEqual([
      expect.objectContaining({
        flag: "--id",
        field: "id",
        description: expect.stringContaining("Bare ten-letter"),
      }),
    ]);
    expect(objectiveUpdate.json.example).toContain("objective update --id qjtrmnpvka");
    expect(experiments.json.fields.map((field: any) => field.flag)).toEqual(["--objective", "--id"]);
    expect(experiments.json.input_schema.cli_owned_fields).toEqual(["id", "artifact", "objective"]);
    expect(experiments.json.example).toContain("--objective qjtrmnpvka");
    expect(rendered).not.toMatch(/--number|OBJECTIVE_ID|objective:123e|RFC 9562|state plan|number is assigned by the CLI/i);
    const legacy = project(false);
    const legacyExperiment = capture(legacy, ["state", "experiments", "explain", "--format", "json"]);
    const legacyObjective = capture(legacy, ["state", "objective", "explain", "--verb", "update", "--format", "json"]);
    expect(legacyExperiment.json.example).toBe("agentera state experiments publish --objective qjtrmnpvka --input experiment.yaml");
    expect(legacyExperiment.json.fields.map((field: any) => field.flag)).toEqual(["--objective", "--id"]);
    expect(legacyObjective.json.example).toContain("objective update --id qjtrmnpvka");
  });

  it("lets Git merge unrelated entities, conflicts on one objective update, and validates distinct-path duplicate ownership", () => {
    const root = project();
    const first = createObjective(root, "first");
    const second = createObjective(root, "second");
    ...
    createObjective(left, "left");
    publish(left, first.id, experiment("left baseline", "baseline"));
    createObjective(right, "right");
    publish(right, second.id, experiment("right baseline", "baseline"));
    ...
    for (const [checkout, title] of [
      [a, "A"],
      [b, "B"],
    ]) {
      const update = capture(checkout, ["state", "objective", "update", "--id", first.id, "--input", "-", "--format", "json"], objective(title));
      expect(update.rc).toBe(0);
      git(checkout, "add", ".agentera/entities");
      git(checkout, "commit", "-m", title);
    }
    git(root, "merge", "--no-edit", "a");
    expect(() => git(root, "merge", "--no-edit", "b")).toThrow();
    git(root, "merge", "--abort");
    const source = path.join(root, `.agentera/entities/objective/objective/${first.id}.yaml`);
    const duplicate = path.join(root, `.agentera/entities/experiments/experiment/${first.id}.yaml`);
  ...
  it("rolls back experiment publication on marker substitution through the shared context", () => {
    const root = project();
    const owner = createObjective(root, "marker");
    ...
    const req: StateWriteRequest = {
      ...
      dryRun: false,
      force: false,
      values: { objective: owner.id },
      callerPayload: experiment("baseline", "baseline"),
      input: experiment("baseline", "baseline"),
    };
    const original = binding.publicationContext.publishImmutable.bind(binding.publicationContext);
    vi.spyOn(binding.publicationContext, "publishImmutable").mockImplementation((relative, bytes) => {
      const result = original(relative, bytes);
      fs.writeFileSync(path.join(root, ".agentera/state-mode.yaml"), MARKER + "# changed\n");
      return result;
    });
    expect(() =>
      publishExperimentEntity(req, {
  ...
  it("restores exact objective bytes when whole-state postvalidation unexpectedly fails", () => {
    for (const verb of ["create", "update"] as const) {
      const root = project();
      const owner = createObjective(root, verb);
      const ownerPath = path.join(root, `.agentera/entities/objective/objective/${owner.id}.yaml`);
      ...
      const inject = () => {
        if (injected) return;
        injected = true;
        const bad = path.join(root, ".agentera/entities/experiments/experiment/bbbbbbbbbb.yaml");
        fs.mkdirSync(path.dirname(bad), { recursive: true });
        fs.writeFileSync(bad, "id: bbbbbbbbbb\nartifact: experiments\nrecord: {}\n");
      };
      if (verb === "create") {
        const original = binding.publicationContext.publishImmutable.bind(binding.publicationContext);
        vi.spyOn(binding.publicationContext, "publishImmutable").mockImplementation((...args) => {
          const result = original(...args);
          if (String(args[0]).includes("/objective/")) inject();
          return result;
        });
      } else {
      ...
      const spec = operationSpec("objective", verb)!;
      const req: StateWriteRequest = {
        artifact: "objective",
        spec,
        projectRoot: root,
        dryRun: false,
        force: false,
        values: verb === "update" ? { id: owner.id } : {},
        callerPayload: objective("changed"),
        input: objective("changed"),
      };
      expect(() => mutateObjectiveEntity(req, { publicationContext: binding.publicationContext })).toThrow(/invalid|conflict/i);
      binding.publicationContext.close();
      expect(fs.readFileSync(ownerPath)).toEqual(before);
      if (verb === "create") expect(fs.readdirSync(path.dirname(ownerPath)).filter((name) => name.endsWith(".yaml"))).toHaveLength(1);
    }
  });
});
