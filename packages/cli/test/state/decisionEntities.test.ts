...
function project(entity = true): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "agentera-decision-entities-"));
  roots.push(root);
  if (entity) {
...
function base(root: string, id = "aaaaaaaaaa", choice = "Entity files"): Record<string, any> {
  return appendDecisionEntity(
    request(root, "append", {
      date: "2026-07-17",
      question: "Where should authority live?",
      context: "Parallel branches must merge.",
      alternatives: { chosen: "Entity files", rejected: ["Aggregate"] },
      choice,
...
describe("decision entity authority", () => {
  it("appends one immutable base and never writes legacy projections or archives", () => {
    const root = project();
    const written = executeStateWrite(
      request(root, "append", {
        date: "2026-07-17",
        question: "Q?",
        context: "C",
  ...
  it("gets effective base, revision, and satisfaction detail with bare identity and provenance", () => {
    ...
    updateDecisionSatisfactionEntity(
      request(root, "update", {
        id: "aaaaaaaaaa",
        satisfaction: { state: "provisionally_satisfied", evidence: "tests pass" },
      }),
      { id: "bbbbbbbbbb" },
    );
    ...
    amendDecisionEntity(
      request(root, "amend", {
        id: "aaaaaaaaaa",
        base_sha256: first.entry.effective_sha256,
    ...
    expect(result.entry).toMatchObject({
      ...
      provenance: {
        base: { id: "aaaaaaaaaa", artifact: "decisions" },
        revisions: [{ id: "cccccccccc", artifact: "decisions" }],
        satisfaction: { id: "bbbbbbbbbb", artifact: "decisions" },
      },
    });
  ...
  it("rejects malformed decision revision value types during entity validation", () => {
    ...
    fs.writeFileSync(
      path.join(directory, "bbbbbbbbbb.yaml"),
      dumpYamlMapping({
        id: "bbbbbbbbbb",
        artifact: "decisions",
        record: {
    ...
    expect(validation.issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          message: expect.stringContaining("choice must be a non-empty string"),
        }),
      ]),
    );
  });
  ...
  it.each([
    [
      "mixed alternatives",
      {
        alternatives: [{ name: "Replacement", status: "chosen" }],
        "alternatives.chosen": "Granular",
      },
      "must not mix migration-only alternatives replacement",
    ],
    ...
    expect(validateEntityState(root).issues).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          boundary: "decision_revision",
  ...
  it.each(["high", "medium", "low"])("reports migration-proven inherited confidence %s in exact and list reads", (confidence) => {
    ...
    expect(() =>
      canonicalEntityEnvelope(fs.readFileSync(entityPath, "utf8"), {
        artifact: "decisions",
        boundary: "decision",
        id: "aaaaaaaaaa",
      }),
  ...
  it.each([
    ...
    [
      "arbitrary unsupported with fabricated provenance",
      "certain",
      {
        kind: "inherited_decision_confidence",
        source: "current_projection",
    ...
    [
      "current confidence with provenance",
      "firm",
      {
        kind: "inherited_decision_confidence",
        source: "current_projection",
    ...
    [
      ...
      "high",
      {
        ...
        source_path: ".agentera/archive/decisions/1.yaml",
  ...
  it.each([
    ...
    [
      "fabricated projection digest",
      (root: string, source: Record<string, any>) => {
        fs.writeFileSync(path.join(root, ".agentera/decisions.yaml"), dumpYamlMapping({ decisions: [source] }));
        return {
          source: "current_projection",
          source_path: ".agentera/decisions.yaml",
          digest: "b".repeat(64),
    ...
    [
      ...
      (root: string, source: Record<string, any>) => {
        source.choice = "Different source choice";
        fs.writeFileSync(path.join(root, ".agentera/decisions.yaml"), dumpYamlMapping({ decisions: [source] }));
        return {
          source: "current_projection",
          source_path: ".agentera/decisions.yaml",
          digest: createHash("sha256").update(canonicalRecordJson(source)).digest("hex"),
        };
      },
    ],
    [
      ...
      (root: string, source: Record<string, any>) => {
        ...
        return {
          source: "current_projection",
          source_path: ".agentera/not-decisions.yaml",
          digest: createHash("sha256").update(canonicalRecordJson(source)).digest("hex"),
        };
      },
    ],
    [
      ...
      (root: string, source: Record<string, any>) => {
        fs.writeFileSync(path.join(root, ".agentera/real-decisions.yaml"), dumpYamlMapping({ decisions: [source] }));
        fs.symlinkSync("real-decisions.yaml", path.join(root, ".agentera/decisions.yaml"));
        return {
          source: "current_projection",
          source_path: ".agentera/decisions.yaml",
          digest: createHash("sha256").update(canonicalRecordJson(source)).digest("hex"),
    ...
    [
      "malformed archive",
      (root: string, source: Record<string, any>) => {
        const dir = path.join(root, ".agentera/archive/decisions");
        fs.mkdirSync(dir, { recursive: true });
        fs.writeFileSync(path.join(dir, "1.yaml"), "not: an archive\n");
        return {
          source: "verified_archive",
          source_path: ".agentera/archive/decisions/1.yaml",
          digest: createHash("sha256").update(canonicalRecordJson(source)).digest("hex"),
    ...
    [
      ...
      (root: string, source: Record<string, any>) => ({
        source: "verified_archive",
        source_path: ".agentera/archive/decisions/not-a-number.yaml",
        digest: createHash("sha256").update(canonicalRecordJson(source)).digest("hex"),
      }),
    ],
    [
    ...
    [
      ...
      (root: string, source: Record<string, any>) => {
        ...
        fs.symlinkSync("real.yaml", path.join(dir, "1.yaml"));
        return {
          source: "verified_archive",
          source_path: ".agentera/archive/decisions/1.yaml",
          digest,
    ...
    entity.migration_provenance = {
      ...
      source_record_sha256: binding.digest,
      confidence: "high",
    };
    fs.writeFileSync(entityPath, dumpYamlMapping(entity));
    expect(validateEntityState(root).valid).toBe(false);
  ...
  it("keeps inherited provenance out of amendments and caveats only the effective inherited label", () => {
    ...
    fs.writeFileSync(
      path.join(archiveDir, "1.yaml"),
      dumpYamlMapping({
        schemaVersion: "agentera.stateArchiveEntry.v1",
        artifact_id: "decisions",
        entry_number: 1,
        record: sourceRecord,
        record_sha256: sourceRecordSha256,
      }),
    );
    entity.migration_provenance = {
      kind: "inherited_decision_confidence",
    ...
    amendDecisionEntity(
      request(root, "amend", {
        id: "aaaaaaaaaa",
        base_sha256: inherited.effective_sha256,
        choice: "Still inherited",
      }),
      { id: "bbbbbbbbbb" },
    );
    ...
    amendDecisionEntity(
      request(root, "amend", {
        id: "aaaaaaaaaa",
        base_sha256: unchangedConfidence.effective_sha256,
        confidence: "firm",
      }),
      { id: "cccccccccc" },
    );
  ...
  it("routes exact get/list through bare IDs and rejects numeric selectors in entity mode", () => {
    ...
    expect(JSON.parse(out).entries[0].id).toBe("aaaaaaaaaa");
    out = "";
    expect(
      runStateGet(
        "decisions",
  ...
  it("preserves and shell-quotes topic filters in executable continuations", () => {
    ...
    expect(
      runStateList(
        "decisions",
        ["--topic", topic, "--ids-only", "--limit", "1", "--format", "json"],
        {
          out: (text) => {
            out += text;
          },
  ...
  it("rejects near-prefix selector flag typos", () => {
    ...
    for (const typo of ["--fieldsx", "--ids-onlyx"]) {
      ...
      expect(JSON.parse(out).error).toMatchObject({
        ...
        message: `unrecognized argument '${typo}'`,
      });
    }
  });
  ...
  it("enforces satisfaction evidence, user confirmation, transitions, replacement, and replay", () => {
    ...
    const provisional = updateDecisionSatisfactionEntity(
      request(root, "update", {
        id: "aaaaaaaaaa",
        satisfaction: { state: "provisionally_satisfied", evidence: "green" },
      }),
      { id: "bbbbbbbbbb" },
    ) as any;
    ...
    const confirmed = {
      ...
      satisfaction: {
        ...
        user_confirmation: { confirmed_by: "user", confirmed_at: "2026-07-17T12:00:00Z" },
      },
    };
    expect(() =>
      updateDecisionSatisfactionEntity(
  ...
  it("validates amendment base hashes and safe paths, converges replays, and rejects divergent IDs", () => {
    const root = project();
    base(root);
    const hash = (getDecisionEntity(root, "aaaaaaaaaa") as any).entry.effective_sha256;
    ...
    ).toThrow(/not amendable/);
    amendDecisionEntity(request(root, "amend", { id: "aaaaaaaaaa", base_sha256: hash, choice: "new" }), {
      id: "bbbbbbbbbb",
    });
  ...
  it("paginates whole effective entries and invalidates a cursor after mutation", () => {
    const root = project();
    base(root, "aaaaaaaaaa", "A");
    base(root, "bbbbbbbbbb", "B");
    base(root, "cccccccccc", "C");
    const first = listDecisionEntities(root, 1) as any;
    expect(first.entries).toHaveLength(1);
    expect(first.next_cursor).toBeTruthy();
    const second = listDecisionEntities(root, 1, undefined, first.next_cursor) as any;
    expect(second.entries).toHaveLength(1);
    expect(second.entries[0].id).not.toBe(first.entries[0].id);
    base(root, "dddddddddd", "D");
  ...
  it("binds cursors to every revision and satisfaction input while stable snapshots continue", () => {
    const mutations: Array<[string, (root: string) => void, (root: string) => void]> = [
      [
        ...
        () => {},
        (root) => {
          updateDecisionSatisfactionEntity(request(root, "update", { id: "aaaaaaaaaa", satisfaction: { state: "open" } }), { id: "dddddddddd" });
        },
      ],
      [
        "satisfaction mutate",
        ...
        (root) => {
          const file = path.join(root, ".agentera/entities/decisions/decision_satisfaction/dddddddddd.yaml");
          fs.writeFileSync(file, fs.readFileSync(file, "utf8").replace("state: open", "state: provisionally_satisfied\n  evidence: changed"));
        },
      ],
      [
        "satisfaction remove",
        ...
        (root) => {
          fs.rmSync(path.join(root, ".agentera/entities/decisions/decision_satisfaction/dddddddddd.yaml"));
        },
      ],
      [
        "satisfaction ownership conflict",
        (root) => {
          updateDecisionSatisfactionEntity(request(root, "update", { id: "aaaaaaaaaa", satisfaction: { state: "open" } }), { id: "dddddddddd" });
        },
        (root) => {
          const dir = path.join(root, ".agentera/entities/decisions/decision_satisfaction");
          fs.writeFileSync(
            path.join(dir, "eeeeeeeeee.yaml"),
            dumpYamlMapping({
              id: "eeeeeeeeee",
              artifact: "decisions",
              record: { decision: "aaaaaaaaaa", state: "open" },
            }),
          );
        },
      ],
      [
        ...
        (root) => {
          const hash = (getDecisionEntity(root, "aaaaaaaaaa") as any).entry.effective_sha256;
          amendDecisionEntity(request(root, "amend", { id: "aaaaaaaaaa", base_sha256: hash, choice: "added" }), {
            id: "dddddddddd",
          });
        },
      ],
      [
        ...
        addRevision,
        (root) => {
          const file = path.join(root, ".agentera/entities/decisions/decision_revision/dddddddddd.yaml");
          fs.writeFileSync(file, fs.readFileSync(file, "utf8").replace("choice: revised", "choice: changed"));
        },
      ],
      [
        "revision remove",
        addRevision,
        (root) => {
          fs.rmSync(path.join(root, ".agentera/entities/decisions/decision_revision/dddddddddd.yaml"));
        },
      ],
      [
        "revision ownership conflict",
        addRevision,
        (root) => {
          const file = path.join(root, ".agentera/entities/decisions/decision_revision/dddddddddd.yaml");
          const entity = YAML.parse(fs.readFileSync(file, "utf8"));
          entity.id = "eeeeeeeeee";
          fs.writeFileSync(path.join(path.dirname(file), "eeeeeeeeee.yaml"), dumpYamlMapping(entity));
        },
      ],
    ];
    function addRevision(root: string): void {
      const hash = (getDecisionEntity(root, "aaaaaaaaaa") as any).entry.effective_sha256;
      amendDecisionEntity(request(root, "amend", { id: "aaaaaaaaaa", base_sha256: hash, choice: "revised" }), {
        id: "dddddddddd",
      });
    }
  ...
  it("validates canonical revision schema, authority paths, provenance, hashes, and relations", () => {
    const invalid: Array<[string, Record<string, unknown>]> = [
      ...
      [
        "identity overlap",
        {
          ...
          provenance: "historical_revision",
          base_sha256: "0".repeat(64),
          changes: { decision: "bbbbbbbbbb", choice: "new" },
        },
      ],
      [
        "temporal overlap",
        {
          ...
          provenance: "historical_revision",
          base_sha256: "0".repeat(64),
          changes: { date: "2026-07-18", choice: "new" },
        },
      ],
      [
        "satisfaction overlap",
        {
          ...
          provenance: "historical_revision",
          base_sha256: "0".repeat(64),
          changes: { satisfaction: { state: "open" }, choice: "new" },
        },
      ],
      [
        "relation",
    ...
    for (const [label, record] of invalid) {
      const root = project();
      base(root);
      const dir = path.join(root, ".agentera/entities/decisions/decision_revision");
      fs.mkdirSync(dir, { recursive: true });
    ...
    base(root);
    const hash = (getDecisionEntity(root, "aaaaaaaaaa") as any).entry.effective_sha256;
    amendDecisionEntity(request(root, "amend", { id: "aaaaaaaaaa", base_sha256: hash, choice: "valid" }), {
      id: "bbbbbbbbbb",
    });
  ...
  it("fails effective reads and state validation on ambiguous satisfaction or revision ownership", () => {
    ...
    for (const [id, state] of [
      ["bbbbbbbbbb", "open"],
      ["cccccccccc", "provisionally_satisfied"],
    ]) {
      const dir = path.join(root, ".agentera/entities/decisions/decision_satisfaction");
      fs.mkdirSync(dir, { recursive: true });
      fs.writeFileSync(
        ...
        dumpYamlMapping({
          ...
          record: {
            decision: "aaaaaaaaaa",
            state,
            ...(state === "provisionally_satisfied" ? { evidence: "x" } : {}),
          },
        }),
      );
    }
    ...
    fs.rmSync(path.join(root, ".agentera/entities/decisions/decision_satisfaction"), {
      recursive: true,
    });
    ...
    expect(() =>
      executeStateWrite(
        request(root, "append", {
          question: "Legacy?",
          context: "No marker",
          alternatives: { chosen: "Legacy" },
          choice: "Legacy",
          reasoning: "Cutover is explicit",
          confidence: "firm",
        }),
      ),
    ).toThrow(/durable entity-state marker/);
    expect(fs.existsSync(path.join(root, ".agentera/decisions.yaml"))).toBe(false);
  ...
  it("lets Git merge unrelated bases and exposes competing same-decision ownership without data loss", () => {
    ...
    base(left, "aaaaaaaaaa", "left");
    base(right, "bbbbbbbbbb", "right");
