...
function writePlan(
  target: string,
  status: "active" | "completed" | "open" | "complete",
  created: string,
  taskStatus: string,
  evidence: string,
): void {
...
describe("plan lifecycle migration", () => {
  it("previews and applies docs-mapped current and archived plans without changing evidence", () => {
    ...
    const originals = new Map(
      [current, completeArchive, openArchive, collisionArchive].map((target) => [target, {
    ...
    const preview = JSON.parse(stdout) as { phases: Array<{ name: string; items: Array<Record<string, unknown>> }> };
    const artifactPhase = preview.phases.find((phase) => phase.name === "artifacts");
    expect(artifactPhase?.items).toEqual(expect.arrayContaining([
      expect.objectContaining({ action: "normalize-plan-lifecycle", source: "state/current-plan.yaml", status: "pending" }),
      expect.objectContaining({ action: "normalize-plan-lifecycle", source: "state/archive/PLAN-2026-07-09-collision.yaml", collisions: [expect.stringContaining("unfinished task evidence")] }),
    ]));
