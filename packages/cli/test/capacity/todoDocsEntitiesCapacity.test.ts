...
describe("TODO entity capacity", () => {
  it("preserves 40, 60, and 100 realistic TODO rows across byte pressure, filters, and continuation without mutation", () => {
    ...
    expect(second.json).toMatchObject({
      counts: { total: 70, candidate: 70, returned: 30, remaining: 0, omitted: 0, continuation: 0 },
    });
    ...
    expect(selected.json.projection).toMatchObject({
      selector: "fields",
      fields: ["status", "target_version"],
    ...
    expect(degraded.json).toMatchObject({
      ...
      degradation: {
        ...
        omitted_fields: expect.arrayContaining(["record", "provenance"]),
      },
    });
    ...
    expect(rejected.json).toMatchObject({
      ...
      error: {
        ...
        message: expect.stringContaining("selected fields cannot fit"),
      },
    });
