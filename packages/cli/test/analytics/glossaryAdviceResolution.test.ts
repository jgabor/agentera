...
function source(owner: "project" | "personal", availability: GlossaryAvailability, entries: ConsumerGlossaryEntry[] = []): GlossaryInputAvailability {
...
function entry(owner: "project" | "personal", term: string, meaning: string): ConsumerGlossaryEntry {
  return { owner, term, meaning };
}

function acquired(project: GlossaryInputAvailability, personal: GlossaryInputAvailability): AcquiredGlossaryInputs {
...
function personalCandidate(): GlossaryInputAvailability {
  return source("personal", "valid_present", [entry("personal", "Ready to sail", "A potentially equivalent personal meaning")]);
}
...
describe("shared glossary advice resolution", () => {
  it("loads the active runtime, host-review vocabulary, output schema, and failure classes from authority", () => {
    ...
    expect(contract).toMatchObject({
      ...
      command: "agentera report glossary-advice --input REQUEST",
      ...
      outputFields: ["outcome", "applicable_meaning", "applicable_owner", "review", "tension", "advisory"],
      failureClasses: ["invalid_request", "invalid_acquisition", "invalid_host_review"],
    });
    expect(contract.maxRequestedTermUtf8Bytes).toBe(glossaryAcquisitionContract().maxSourceUtf8Bytes);
  });
  ...
  it("keeps exact project authority while reporting each bounded orthogonal advisory", () => {
    expect(resolveGlossaryAdvice(REQUESTED, acquired(projectExact(), source("personal", "unreadable")))).toEqual({
      outcome: "project_only",
      applicable_meaning: "Project meaning",
      applicable_owner: "project",
      review: "none_for_primary",
    ...
    expect(resolveGlossaryAdvice(REQUESTED, acquired(projectExact(), personalCandidate()), inferredReview)).toEqual({
      outcome: "project_only",
      applicable_meaning: "Project meaning",
      applicable_owner: "project",
  ...
  it("uses case-insensitive exact term identity and exact-string meaning equality", () => {
    const equal = acquired(source("project", "valid_present", [entry("project", "SHIP SHAPE", "Same")]), source("personal", "valid_present", [entry("personal", "ship shape", "Same")]));
  ...
  it("uses Unicode caseless-exact identity for Greek authority, collisions, duplicates, and host candidates", () => {
    const greekProject = source("project", "valid_present", [entry("project", "ΟΣ", "Project Greek meaning")]);
    expect(resolveGlossaryAdvice("οσ", acquired(greekProject, source("personal", "valid_empty")))).toMatchObject({
      outcome: "project_only",
      applicable_meaning: "Project Greek meaning",
    });
    expect(resolveGlossaryAdvice("οσ", acquired(greekProject, source("personal", "valid_present", [entry("personal", "ος", "Divergent personal meaning")])))).toMatchObject({
      outcome: "divergent_exact_collision",
      applicable_owner: "project",
      tension: "divergent_exact_collision",
    });

    expect(() => resolveGlossaryAdvice("other", acquired(source("project", "valid_present", [entry("project", "ΟΣ", "One"), entry("project", "οσ", "Two")]), source("personal", "absent")))).toThrow(GlossaryAdviceInputError);

    expect(
      resolveGlossaryAdvice("requested", acquired(source("project", "valid_empty"), source("personal", "valid_present", [entry("personal", "ΟΣ", "Candidate")])), {
        relation: "inferred_equivalence",
        candidate_owner: "personal",
        candidate_term: "οσ",
      }),
  ...
  it("does not normalize canonically distinct terms during lookup", () => {
    expect(resolveGlossaryAdvice("e\u0301", acquired(source("project", "valid_present", [entry("project", "é", "Project precomposed")]), source("personal", "valid_present", [entry("personal", "e\u0301", "Personal decomposed")])))).toMatchObject({
      outcome: "proven_project_gap",
  ...
  it("covers absent, empty, nonmatching, and every invalid availability without unsafe fallback", () => {
    for (const availability of ["malformed", "unreadable", "ambiguous", "over_bound"] as const) {
      expect(resolveGlossaryAdvice(REQUESTED, acquired(source("project", availability), personalExact()))).toMatchObject({
        outcome: "invalid_or_unavailable_project",
        applicable_meaning: null,
        applicable_owner: null,
      });
      expect(resolveGlossaryAdvice(REQUESTED, acquired(projectExact(), source("personal", availability)))).toMatchObject({
        outcome: "project_only",
        applicable_owner: "project",
    ...
    for (const project of [source("project", "absent"), source("project", "valid_empty"), projectGap()]) {
    ...
    for (const [personal, outcome] of [
      ...
      expect(resolveGlossaryAdvice(REQUESTED, acquired(source("project", "valid_empty"), personal))).toMatchObject({
        outcome,
        applicable_meaning: null,
      });
    }
  ...
  it("derives every lawful tuple's first matching row and row semantics from the loaded authority", () => {
    ...
    for (const projectInput of contract.dimensions.project_input!) {
      for (const personalInput of contract.dimensions.personal_input!) {
        ...
        const inferredCandidates = personalInput === "valid_without_exact" ? ["absent", "present"] : ["absent"];
        for (const exactMeaning of exactMeanings) {
          for (const inferredCandidate of inferredCandidates) {
            ...
            const personal = personalInput === "valid_exact" ? personalStates.valid_exact(exactMeaning === "equivalent" ? "Same" : "Different") : personalStates[personalInput as "invalid" | "valid_without_exact"]();
            ...
            const matching = contract.rows.filter((row) => Object.entries(state).every(([dimension, value]) => row.match[dimension]!.includes(value)));
            const advisory = contract.advisories.find((candidate) => candidate.primaryOutcome === matching[0]?.name && Object.entries(state).every(([dimension, value]) => candidate.match[dimension]?.includes(value)));
            expect(matching, JSON.stringify(state)).toHaveLength(1);
            expect(result).toMatchObject({
              outcome: matching[0]!.name,
              applicable_owner: matching[0]!.selectedOwner === "project" || matching[0]!.selectedOwner === "personal" ? matching[0]!.selectedOwner : null,
  ...
  it("fails once and privacy-safely for duplicate, malformed, over-bound, and unknown review input", () => {
    ...
    const duplicate = acquired(source("project", "valid_present", [entry("project", REQUESTED, privateTrap), entry("project", "ship shape", privateTrap)]), source("personal", "absent"));
    const overBound = "x".repeat(glossaryAcquisitionContract().maxSourceUtf8Bytes + 1);
    const tooManyEntries = acquired(
      source(
        "project",
        "valid_present",
        Array.from({ length: glossaryAcquisitionContract().maxEntries + 1 }, (_, index) => entry("project", `Term ${index}`, `Meaning ${index}`)),
      ),
    ...
    const attempts: Array<() => unknown> = [
      ...
      () => resolveGlossaryAdvice(REQUESTED, acquired(projectExact(), source("personal", "valid_present", []))),
    ...
    for (const attempt of attempts) {
      ...
      expect(["invalid_request", "invalid_acquisition", "invalid_host_review"]).toContain((failure as GlossaryAdviceInputError).code);
    }
  });
  ...
  it("returns only selected transient content and is deterministic, read-only, and unrelated-entry safe", () => {
    ...
    const inputs = acquired(source("project", "valid_present", [entry("project", REQUESTED, "Selected project meaning"), entry("project", privateTrap, privateTrap)]), source("personal", "valid_present", [entry("personal", privateTrap, privateTrap)]));
