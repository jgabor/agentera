...
import { verificationPolicyErrors, verificationPendingContract } from "../src/registries/verificationPolicy.ts";
...
function validateConsumedPolicy(policy) {
  const issues = verificationPolicyErrors(policy);
  if (issues.length > 0) {
...
function parsePendingAuthority(overlapAuthority) {
  const { issues, authority } = verificationPendingContract(overlapAuthority, render);
  if (issues.length > 0) {
