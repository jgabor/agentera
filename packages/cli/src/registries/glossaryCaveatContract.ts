...
function fixedValues(value: unknown): Record<string, string | null> {
  ...
  if (!source) return {};
  return Object.fromEntries(Object.entries(source).filter((entry): entry is [string, string | null] => typeof entry[1] === "string" || entry[1] === null));
}
...
function allowedPairs(value: unknown): Array<{ reason: string; ownershipState: string }> {
  ...
  return value.flatMap((candidate) => {
    ...
    return typeof pair?.reason === "string" && typeof pair.ownership_state === "string" ? [{ reason: pair.reason, ownershipState: pair.ownership_state }] : [];
  });
}

export function glossaryCaveatPairAllowed(contract: GlossaryCaveatContract, reason: string, ownershipState: string): boolean {
  return contract.allowedCurrentPairs.some((pair) => pair.reason === reason && pair.ownershipState === ownershipState);
}

export function glossaryCaveatContract(pathname: string = glossaryEntryAuthorityPath()): GlossaryCaveatContract {
  return glossaryCaveatContractFromDocument(loadYamlMappingFile(pathname));
}

export function glossaryCaveatContractFromDocument(authority: Record<string, unknown>): GlossaryCaveatContract {
  ...
  return {
    ...
    currentAppendCallerFixedValues: Object.fromEntries(Object.entries(fixedValues(currentAppend?.caller_fixed_values)).filter((entry): entry is [string, string] => typeof entry[1] === "string")),
    ...
    maxStringUtf8Bytes: typeof envelope?.max_string_utf8_bytes === "number" ? envelope.max_string_utf8_bytes : 0,
    primeAttentionText: typeof primeProjection?.attention_text === "string" ? primeProjection.attention_text : "",
    ...
    primePublicAttentionLimit: typeof primeCapacity?.public_attention_limit === "number" ? primeCapacity.public_attention_limit : 0,
    primeReservedGlossarySlots: typeof primeCapacity?.reserved_glossary_slots === "number" ? primeCapacity.reserved_glossary_slots : 0,
  };
}
