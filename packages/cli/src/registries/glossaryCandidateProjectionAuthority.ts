...
export function validatePersonalCandidateProjectionAuthority(authority: Mapping): string[] {
  ...
  if (
    ...
    refreshProduction?.runtime !== "packages/cli/src/analytics/personalGlossaryRefreshProjection.ts#produceCurrentPersonalGlossaryProjection" ||
  ...
  if (
    ...
    refreshConcurrency?.lock_lifetime !== "before_evidence_publication_through_projection_binding_verification" ||
  ...
  if (
    ...
    !sameStrings(safeReplacement?.reject, ["symlink", "non_regular_file", "configured_path_mismatch", "path_escape"]) ||
    safeReplacement?.filesystem_exception !== "personal_mining_authority.privacy.storage.filesystem.malformed_candidate_projection_replacement_exception" ||
    !sameStrings(safeReplacement?.replaceable_state, ["candidate_projection"]) ||
    !sameStrings(safeReplacement?.preserved_state, ["current_evidence_generation", "review_records", "personal_profile", "project_state", "project_glossary"])
  ) {
  ...
  if (
    malformedProjectionReplacementException?.scope !== "malformed_candidate_projection_replacement_only" ||
    malformedProjectionReplacementException?.authorization !== "explicit_consented_refresh_only" ||
    malformedProjectionReplacementException?.precondition !== "existing_owned_regular_file_is_malformed" ||
    malformedProjectionReplacementException?.target !== "exact_configured_candidate_projection_file" ||
    malformedProjectionReplacementException?.inspection !== "lstat_exact_target_without_following_symlinks" ||
    malformedProjectionReplacementException?.required_type !== "regular_file" ||
    !sameStrings(malformedProjectionReplacementException?.reject, ["symlink", "non_regular_file", "configured_path_mismatch", "path_escape"]) ||
  ...
  if (
    ...
    !sameStrings(miningSummary?.family_fields, ["candidate_count", "abstention_count", "abstentions_by_reason"]) ||
    ...
    summaryReconciliation?.total_candidates !== "total_equals_explicit_plus_recurring_candidate_counts" ||
    summaryReconciliation?.total_abstentions !== "total_equals_explicit_plus_recurring_abstention_counts" ||
    summaryReconciliation?.projection_input !== "report_input_count_equals_total_candidate_count" ||
    !sameStrings(miningSummary?.forbidden_content, ["terms", "source_ids", "evidence_anchors", "project_ids", "project_keys", "paths", "excerpts"]) ||
  ...
  if (
    ...
    projection?.runtime !== "packages/cli/src/analytics/personalGlossaryCandidateProjection.ts#projectPersonalGlossaryCandidates" ||
    ...
    !sameStrings(sourceFamilies?.recurring, ["personal_inferred_usage", "personal_inferred_conversation"]) ||
    ...
    !sameStrings(excerpts?.omission_reasons, ["no_excerpt", "unrelated_context", "source_bound_exceeded", "unsafe_tool_arguments", "unsafe_content"]) ||
    ...
    !sameStrings(persistence?.fields, ["schema_version", "owner", "generation", "policy_version", "retained_at", "candidates", "report", "projection_sha256"]) ||
    ...
    retrieval?.runtime !== "packages/cli/src/cli/commands/personalGlossaryCandidateReads.ts#runPersonalGlossaryCandidateReadsCommand" ||
    retrieval?.input !== "current_validated_user_local_candidate_projection_bound_to_current_tier_generation" ||
    ...
    !sameStrings(cursor?.binding, ["collection", "generation", "policy_version", "filters", "limit", "order", "snapshot"]) ||
    ...
    !sameStrings(exact?.required_bindings, ["candidate_id", "candidate_revision", "generation", "policy_version"]) ||
    ...
    !sameStrings(occurrence?.allowed_fields, ["occurrence_id", "source_kind", "signal_type", "author_class"]) ||
    !nonEmpty(exact?.rule) ||
