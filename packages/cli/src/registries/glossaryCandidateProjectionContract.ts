...
export function personalGlossaryCandidateProjectionContract(pathname: string = glossaryEntryAuthorityPath()): PersonalGlossaryCandidateProjectionContract {
  ...
  const sourceFamilies = Object.fromEntries(Object.entries(families ?? {}).flatMap(([family, values]) => (Array.isArray(values) && values.every((value) => typeof value === "string") ? [[family, [...values] as string[]]] : [])));
  ...
  return {
    ...
    projectIdsMaxPerCandidate: typeof selection?.project_ids_max_per_candidate === "number" ? selection.project_ids_max_per_candidate : 0,
    ...
    candidateSecretReason: typeof contentExclusion?.candidate_reason === "string" ? contentExclusion.candidate_reason : "",
    ...
    candidateReadProvenanceKinds: [...new Set(Object.values(sourceFamilies).flat())].sort(),
    candidateReadScopes: strings(filters?.scope),
    candidateReadMaxSerializedUtf8Bytes: typeof list?.max_serialized_utf8_bytes === "number" ? list.max_serialized_utf8_bytes : 0,
    ...
    candidateReadCursorUnavailableBehavior: typeof cursor?.unavailable_behavior === "string" ? cursor.unavailable_behavior : "",
    ...
    candidateReadSafeContextMaxUtf8Bytes: typeof exact?.safe_context_max_utf8_bytes === "number" ? exact.safe_context_max_utf8_bytes : 0,
    candidateReadExactMaxSerializedUtf8Bytes: typeof exact?.max_serialized_utf8_bytes === "number" ? exact.max_serialized_utf8_bytes : 0,
    ...
    candidateReadSafeContextRetentionDays: typeof safeContextView?.retention_days === "number" ? safeContextView.retention_days : 0,
    ...
    candidateReadCurrentGenerationProjectionBinding: typeof currentGeneration?.projection_generation === "string" ? currentGeneration.projection_generation : "",
    candidateReadCurrentGenerationUnavailableBehavior: typeof currentGeneration?.unavailable_behavior === "string" ? currentGeneration.unavailable_behavior : "",
    candidateReadCurrentGenerationStaleProjectionBehavior: typeof currentGeneration?.stale_projection_behavior === "string" ? currentGeneration.stale_projection_behavior : "",
  };
}
