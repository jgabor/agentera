...
export function printUpgradeHelp(): string {
  return [
    ...
    "  --shared-skill        Isolated one-file install/refresh or approved dedicated-directory replacement; default preview",
    "                       No project/channel/migration/reset/cleanup/force/verify flags; symlink targets stay untouched.",
    ...
    "  prime and doctor return shared_skill.upgrade_offer with one Yes/No question and the complete apply_command.",
    "  Only explicit Yes runs that unchanged command. No/no answer does nothing; current installs have no offer.",
    "  Legacy conversion requires --authorization TOKEN supplied by the CLI, not historical per-file ownership.",
    "  Apply requires Linux /proc/self/fd publication; legacy retention must be on the same filesystem.",
    "  Conversion supports at most 4096 entries including the root; the CLI retains the old link/tree and creates bookkeeping.",
    "  No pre-existing journal or per-file approval is required. Changed scope or unsafe paths fail without broadening consent.",
    "  Never prune a symlink target or invent manual repair commands. Native cleanup ownership rules remain unchanged.",
