...
export function printStateHelp(sub?: string): string {
  ...
  if (sub === "plan") {
    return [
      ...
      "Plan task list/get reads the active file-based plan.",
  ...
  if (sub === "experiments") {
    return [
      ...
      "List merges retained projection and immutable archive identities newest-first with bounded snapshot cursors.",
      "Get verifies archive detail first, then reports retained full, legacy summary-only, or unavailable detail truthfully.",
