...
export function printStateHelp(sub?: string): string {
  ...
  if (sub === "todo") {
    return [
      ...
      '       agentera state todo create --severity LEVEL --description TEXT [READINESS FLAGS] --format json',
      "       agentera state todo update --id ID [--severity LEVEL] [--description TEXT] [READINESS FLAGS] --format json",
