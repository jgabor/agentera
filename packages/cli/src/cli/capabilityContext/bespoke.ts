...
export function bespokeCapabilityContexts(capabilityName: string | null, state: JsonObject, buildRequest: BuildExecutionRequest | null = null): JsonObject {
  ...
  const history = state.history && typeof state.history === "object" && !Array.isArray(state.history) ? (state.history as JsonObject) : {};
  const decisionHistory = history.decisions && typeof history.decisions === "object" && !Array.isArray(history.decisions) ? (history.decisions as JsonObject) : {};
  return {
