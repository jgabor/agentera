...
export const BENCHMARK_CONTEXT_CMD = preCutoverCommand("prime --context optimize");
...
export function optimizeBenchmarkContext(capability: string | null): JsonObject | null {
  ...
  const fallbackCommands = [STATE_FAMILY_FALLBACK_COMMANDS.docs, preCutoverCommand("state query --list-artifacts")];
  return {
    capability: "optimize",
    benchmark_source: {
