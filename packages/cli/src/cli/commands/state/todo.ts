...
export function queryTodo(args: StateArgs, _schemas: Record<string, SchemaInfo>, io: Io, openOnly = false): number {
  const output = out(io);
  ...
  try {
    ...
    if (format === "json" || format === "yaml") emitStructured(response, format, output);
      ...
      for (const entry of response.entries as Array<{
        id: string;
        record: Record<string, unknown> & { severity: string; status: string };
      }>) {
