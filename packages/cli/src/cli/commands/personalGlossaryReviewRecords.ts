...
function parseQueue(argv: string[], value: ReviewRecordsCommandContract): { input: string } | InvalidInputErrorBody {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (name !== QUEUE_STRUCTURED_INPUT_SPEC.option && name !== "--format") {
      return {
        ...
        message: "unrecognized review-record argument",
        syntax: queueSyntax(value),
      };
    }
    ...
    if (name === "--format") {
      if (format)
        return {
          ...
          message: "--format may only be supplied once",
          syntax: queueSyntax(value),
        };
      if (option !== "json")
        return {
          class: "invalid_choice",
          message: "personal-glossary-reviews requires",
          valid_values: ["json"],
        };
      format = true;
      continue;
    ...
    if (input !== undefined)
      return {
        class: "mutually_exclusive",
        message: `${QUEUE_STRUCTURED_INPUT_SPEC.option} may only be supplied once`,
        syntax: queueSyntax(value),
      };
    input = option;
  }
  return input
    ...
    : {
        class: "missing_argument",
        message: `${QUEUE_STRUCTURED_INPUT_SPEC.option} is required`,
        syntax: queueSyntax(value),
      };
}
...
function parseDisposition(argv: string[], value: ReviewRecordsCommandContract): { input: string } | InvalidInputErrorBody {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (name !== DISPOSITION_STRUCTURED_INPUT_SPEC.option && name !== "--format") {
      return {
        class: "unrecognized_argument",
        message: "unrecognized review disposition argument",
        syntax: dispositionSyntax(value),
      };
    }
    const option = inline ?? argv[++index];
    if (!option || option.startsWith("--")) {
    ...
    if (input !== undefined)
      return {
        class: "mutually_exclusive",
        message: `${DISPOSITION_STRUCTURED_INPUT_SPEC.option} may only be supplied once`,
        syntax: dispositionSyntax(value),
      };
    input = option;
  }
  return input
    ...
    : {
        class: "missing_argument",
        message: `${DISPOSITION_STRUCTURED_INPUT_SPEC.option} is required`,
        syntax: dispositionSyntax(value),
      };
}
...
function parseList(argv: string[], value: ReviewRecordsCommandContract): ListOptions | InvalidInputErrorBody {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    const { name, inline } = argvPart(argv[index]!);
    if (!["--format", "--limit", "--cursor", "--status"].includes(name)) {
      return {
        class: "unrecognized_argument",
        message: "unrecognized review-record argument",
    ...
    if (name === "--limit") {
      ...
      if (parsed > value.maximumLimit)
        return {
          class: "invalid_choice",
          message: `--limit must be from 1 to ${value.maximumLimit}`,
    ...
    if (name === "--cursor") {
      if (!boundedText(option, MAX_CURSOR_UTF8_BYTES))
        return {
          ...
          message: `--cursor must be a non-empty value within ${MAX_CURSOR_UTF8_BYTES} UTF-8 bytes`,
          syntax: listSyntax(value),
        };
      cursor = option;
      continue;
    ...
    if (!value.statuses.includes(option as ReviewStatus)) {
      return {
        class: "invalid_choice",
        message: "--status must be pending or terminal",
        valid_values: value.statuses,
        syntax: listSyntax(value),
      };
    }
...
function parseExact(argv: string[], value: ReviewRecordsCommandContract): ExactOptions | InvalidInputErrorBody {
  ...
  for (const [flag, field] of [
    ...
    if (!SHA256.test(fields[field]!)) {
      return {
        class: "invalid_request",
        message: `${flag} must be a lowercase SHA-256 identity`,
...
function validateQueueRequest(request: Mapping, value: ReviewRecordsCommandContract): { receipt: Mapping } | { error: InvalidInputErrorBody } {
  const keys = Object.keys(request);
  if (keys.some((key) => !value.queueRequestFields.includes(key)) || value.queueRequestFields.some((field) => !(field in request))) {
    return {
      error: {
        class: "schema_violation",
        message: "personal glossary review queue request fields are invalid",
        valid_values: value.queueRequestFields,
      },
    };
  }
  if (request.schema_version !== value.queueRequestSchemaVersion || !mapping(request.receipt)) {
    return {
      error: {
        class: "schema_violation",
        message: `request requires ${value.queueRequestSchemaVersion} and one receipt mapping`,
        valid_values: [value.queueRequestSchemaVersion, "receipt"],
      },
    };
  }
...
function validateDispositionRequest(request: Mapping, value: ReviewRecordsCommandContract): { reviewId: string; receipt: Mapping; approval: Mapping } | { error: InvalidInputErrorBody } {
  const keys = Object.keys(request);
  if (keys.some((key) => !value.dispositionRequestFields.includes(key)) || value.dispositionRequestFields.some((field) => !(field in request))) {
    return {
      error: {
        class: "schema_violation",
  ...
  if (request.schema_version !== value.dispositionRequestSchemaVersion || typeof request.review_id !== "string" || !SHA256.test(request.review_id) || !mapping(request.receipt) || !mapping(request.approval)) {
    return {
      error: {
        class: "schema_violation",
...
function emitQueueSuccess(io: Io, value: ReviewRecordsCommandContract, result: PersonalGlossaryReviewQueueResult): number {
  const response: Mapping = {
    ...
    effects: {
      review_metadata: result.status === "queued" || result.status === "reopened" ? "created" : result.status === "suppressed" ? "unchanged_suppressed" : "unchanged_replay",
  ...
  if (serializedBytes(response) > value.queueResultMaxUtf8Bytes) {
    return failure(io, value.queueResultSchemaVersion, `${value.command} ${QUEUE_STRUCTURED_INPUT_SPEC.action}`, queueSyntax(value), queueSyntax(value), {
...
function queueReview(io: Io, input: string, value: ReviewRecordsCommandContract): number {
  ...
  try {
    ...
    return invalid(io, {
      ...
      message: `${QUEUE_STRUCTURED_INPUT_SPEC.option} must be one readable bounded UTF-8 YAML or JSON mapping`,
    });
  }
  ...
  return failure(io, value.queueResultSchemaVersion, `${value.command} ${QUEUE_STRUCTURED_INPUT_SPEC.action}`, queueSyntax(value), queueSyntax(value), failures[result.status as Exclude<PersonalGlossaryReviewQueueResult["status"], "queued" | "unchanged_replay" | "suppressed" | "reopened">]);
}

function emitDispositionSuccess(io: Io, value: ReviewRecordsCommandContract, result: PersonalGlossaryReviewDispositionResult): number {
  const response: Mapping = {
  ...
  if (serializedBytes(response) > value.dispositionResultMaxUtf8Bytes) {
    return failure(io, value.dispositionResultSchemaVersion, `${value.command} ${DISPOSITION_STRUCTURED_INPUT_SPEC.action}`, dispositionSyntax(value), dispositionSyntax(value), {
      class: "output_bound_exceeded",
...
function dispositionReview(io: Io, input: string, value: ReviewRecordsCommandContract): number {
  ...
  try {
    ...
    return invalid(io, {
      class: "invalid_format",
      message: `${DISPOSITION_STRUCTURED_INPUT_SPEC.option} must be one readable bounded UTF-8 YAML or JSON mapping`,
    });
  }
  ...
  return failure(io, value.dispositionResultSchemaVersion, `${value.command} ${DISPOSITION_STRUCTURED_INPUT_SPEC.action}`, dispositionSyntax(value), dispositionSyntax(value), failures[result.status]);
}
...
export function runPersonalGlossaryReviewRecordsCommand(argv: string[], io: Io): number {
  ...
  try {
    ...
    return failure(io, "agentera.personalGlossaryReviewRetrieval.v1", "agentera report personal-glossary-reviews", "agentera report personal-glossary-reviews {queue,disposition,list,get}", "agentera report personal-glossary-reviews list --limit 20", {
  ...
  if (structuredInputSpec === QUEUE_STRUCTURED_INPUT_SPEC) {
    ...
    if ("class" in parsed)
      return invalid(io, {
        ...parsed,
        recovery: "Correct the bounded queue request and retry; no review metadata was changed.",
      });
  ...
  if (structuredInputSpec === DISPOSITION_STRUCTURED_INPUT_SPEC) {
    ...
    if ("class" in parsed)
      return invalid(io, {
        ...parsed,
        recovery: "Correct the bounded disposition request and retry; no review metadata was changed.",
      });
  ...
  if (operation === "list") {
    ...
    if ("class" in parsed)
      return invalid(io, {
        ...parsed,
        recovery: "Correct the bounded list request and retry; no review metadata was changed.",
      });
  ...
  if ("class" in parsed)
    return invalid(io, {
      ...parsed,
      recovery: "Correct the exact review binding and retry; no review metadata was changed.",
    });
