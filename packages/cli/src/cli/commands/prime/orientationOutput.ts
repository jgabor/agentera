...
const OMITTABLE_DEFAULT_CONDITIONAL_TOP_FIELDS: readonly string[] = ["docs", "objective"];
...
function omitInactiveConditionalDefaults(payload: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [key, value] of Object.entries(payload)) {
    if (OMITTABLE_DEFAULT_CONDITIONAL_TOP_FIELDS.includes(key) && !isConditionalFieldPresent(key, payload)) {
      continue;
    }
    out[key] = value;
  }
...
function capabilityContextPointer(requiredBeforeRendering = true): JsonObject {
  return {
    ...
    note: requiredBeforeRendering ? "Dashboard rendering instructions (template, field rules, exit marker) are owned by the status capability. Run the fetch_command before rendering." : "Includes dashboard instructions and bounded state; no second prime call.",
  };
}

export function buildOrientationJsonPayload(state: OrientationState, command: string, options: { capabilityContextRequiredBeforeRendering?: boolean } = {}): Record<string, unknown> {
  ...
  const render = command === "status" ? "caller-owned README-style prime orientation dashboard" : "caller-owned README-style prime orientation dashboard";
  const access = command === "status" ? "single installed CLI call; app/reset/profile safety included; no preflight glob/read/import/doctor calls during normal prime" : "single installed CLI call; app/reset/profile safety included; no preflight glob/read/import/doctor calls during normal prime";
  return {
    command,
...
export function buildStatusContextState(state: OrientationState, _command = "prime", _options: { budgetBytes?: number; degradedMode?: "minimal" | "status_routing" } = {}): Record<string, unknown> {
  const todoReconciliation = (state.todo_reconciliation as unknown as JsonObject | null | undefined) ?? null;
  const startup = startupAggregation(capabilityContext("status") ?? {}, state.health as unknown as JsonObject, state.state_cutover as unknown as JsonObject, todoReconciliation);
  const plan = state.plan;
  const firstPending = plan.first_pending && typeof plan.first_pending === "object" && !Array.isArray(plan.first_pending) ? (plan.first_pending as JsonObject) : null;
  ...
  const redactRecovery = (action: NextAction): NextAction => (recovery !== null && action.reason === recovery ? { ...action, reason: "See capability_context.startup.todo_reconciliation for recovery." } : action);
  const nextAction = {
    recommended: redactRecovery(state.next_action.recommended),
    alternatives: state.next_action.alternatives.filter((action) => !unsafeDetailOmitted || action.artifact !== "todo").map(redactRecovery),
  };
  return {
    outcome: startup.outcome,
    mode: state.mode,
...
export function emitPrime(command: string, payload: Record<string, unknown>, format: string, fieldsArg: string | null | undefined, out: (t: string) => void, err: (t: string) => void, options: { bareBrief?: boolean; briefBudgetBytes?: number; maxUtf8Bytes?: number } = {}): number {
  ...
  const effectivePayload = requested.length === 0 && options.bareBrief ? briefOrientationPayload(conditional, { budgetBytes: options.briefBudgetBytes }) : conditional;
...
export function rejectRetiredPrimeFields(command: string, format: string, fieldsArg: string | null | undefined, out: (t: string) => void, err: (t: string) => void): number | null {
  const requested = requestedFields(fieldsArg);
...
export function printOrientationTextBriefing(state: OrientationState, command: string, out: (t: string) => void): void {
  ...
  out(`project_integration: recommendation=${projectIntegration.recommendation} | ` + `channel=${projectIntegration.update_channel} | pending_artifacts=${projectIntegration.pending_artifacts}\n`);
  ...
  if (health.exists && health.id) {
    ...
    out(`health: id=${health.id} | artifact=${health.artifact ?? "health"} | grade=${health.grade || "unknown"} | ` + `trajectory=${health.trajectory || "unknown"} | worst=${worstText}\n`);
  }
  if (health.degraded_history) {
    const degraded = health.degraded_history as Record<string, unknown>;
    out(`health: degraded_history | summaries=${String(degraded.summary_count ?? 0)} | returned=${String(degraded.returned_count ?? 0)} | omitted=${String(degraded.omitted_count ?? 0)} | ` + `detail=summary-only | recovery=${preCutoverCommand("state health list --limit 20")}\n`);
  }
  const latestProgress = state.progress.latest as Record<string, unknown> | undefined;
  if (latestProgress) {
    out(`progress: id=${String(latestProgress.id ?? "unknown")} | artifact=${String(latestProgress.artifact ?? "progress")} | ` + `what=${String(latestProgress.what ?? "unknown")}\n`);
  }
  if (state.progress.degraded_history) {
    const degraded = state.progress.degraded_history as Record<string, unknown>;
    out(`progress: degraded_history | summaries=${String(degraded.summary_count ?? 0)} | returned=${String(degraded.returned_count ?? 0)} | omitted=${String(degraded.omitted_count ?? 0)} | ` + `detail=summary-only | recovery=${preCutoverCommand("state progress list --limit 20")}\n`);
  }
  ...
  if (decisionHistory) {
    out(`decisions: degraded_history | summaries=${String(decisionHistory.summary_count ?? 0)} | returned=${String(decisionHistory.returned_count ?? 0)} | omitted=${String(decisionHistory.omitted_count ?? 0)} | ` + `detail=summary-only | recovery=${preCutoverCommand("state decisions list --limit 20")}\n`);
  }
  ...
  const deferred = (startup.availability as JsonObject[]).filter((entry) => entry.availability === "deferred").map((entry) => String(entry.family));
