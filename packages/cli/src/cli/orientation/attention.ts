...
export function buildOrientationAttention(state: OrientationState): string[] {
  ...
  if (todoReconciliation?.status === "action_required") {
    ...
    attention.push(
      todoReconciliation.state === "unsafe_inactive" ? `action-required: TODO reconciliation is ${label}; ${todoReconciliation.recovery_command}` : `action-required: TODO reconciliation is ${label}; preview \`${todoReconciliation.preview_command ?? "n/a"}\`; apply \`${todoReconciliation.apply_command ?? "n/a"}\``,
    );
  }
  ...
  if (health.stale) {
    ...
    let attentionText = `normal: audit stale (${daysSince} days since health audit ${auditId}; ` + `threshold days=${thresholdDays}, cycles=${thresholdCycles}`;
  ...
  if (health.degrading) {
    ...
    attention.push(worst ? `critical: health needs attention (${worst[0]}:${worst[1]})` : "critical: health is degrading");
  }
