...
function countStatus(artifact: string, p: string, activeCount: number, archiveCount: number, protectedOverflowCount = 0, budgetActiveCount?: number, projectionRecovery?: ProjectionRecoveryReport, projectionState?: "within_defaults" | "over_defaults"): CompactionStatus {
  ...
  return {
    ...
    reason: projectionState ? (projectionState === "over_defaults" ? "lossless projection exceeds display defaults; numbered archives remain authoritative" : "lossless projection is within display defaults") : protectedOverflowCount ? "protected-overflow review pressure" : "uniform_10_40_50",
...
function projectionRecoveryFor(projectRoot: string, artifact: string, p: string): ProjectionRecoveryReport | undefined {
  try {
...
export function computeCompactionStatus(projectRoot: string): CompactionStatus[] {
    ...
    return [
      {
        ...
        over_limit_count: 0,
        reason: issueCount ? `${issueCount} canonical entity validation issue(s); run agentera check validate state` : "canonical entity state is independently stored and is not compacted",
        protected_overflow_count: 0,
        exists: true,
      },
    ];
  ...
  if (fs.existsSync(todoPath)) {
    ...
    if (headingCount > 1) {
      ...
      statuses.push({
        ...
        total_count: null,
        over_limit_count: null,
        reason: `TODO.md has ${headingCount} '## ✓ Resolved' sections; merge into exactly one before compacting. ` + `Compaction drops oldest resolved entries and is destructive (no lossless archive); ` + TODO_DROPPED_RECOVERY_GUIDANCE,
        protected_overflow_count: 0,
        exists: true,
      });
  ...
  for (const [artifact, [activeKey, archiveKey]] of Object.entries(COMPACTABLE_YAML_ARTIFACTS)) {
    ...
    if (fs.existsSync(p)) {
      ...
      const protectedOverflowCount = artifact === "decisions" ? decisionProtectedOverflowCount(hydrateDecisionRecords(active, projectRoot), hydrateDecisionRecords(archive, projectRoot)) : 0;
      const budgetActiveCount = artifact === "decisions" ? decisionSatisfiedActiveCount(hydrateDecisionRecords(active, projectRoot)) : undefined;
      statuses.push(
        countStatus(
          artifact,
          p,
          ...
          active.length > projectionPolicy.activeEntries || archive.length > projectionPolicy.summaryEntries || active.length + archive.length > projectionPolicy.totalEntries ? "over_defaults" : "within_defaults",
        ),
      );
    } else {
...
function operationForStatus(status: CompactionStatus, mode: string): CompactionOperation {
  ...
  if (!status.over_limit_count && !status.pending_summarization_count) {
    return base("ok", status.projection_state === "within_defaults" ? "within configured lossless projection defaults" : "within uniform_10_40_50 limits");
  }
...
function fixCompactionUnlocked(projectRoot: string, transaction: StateMutationTransaction): CompactionOperation[] {
  ...
  for (const status of computeCompactionStatus(projectRoot)) {
    ...
    if (baseline.action !== "pending_fix" && baseline.action !== "pending_projection" && baseline.action !== "pending_formatting") {
    ...
    try {
      if (status.artifact === "todo#Resolved") {
        ...
        operations.push({
          ...
          changed: false,
          result: null,
          message: `no fixer registered for ${status.artifact}`,
        });
        continue;
      }
    } catch (exc) {
    ...
    operations.push({
      ...
      result,
      message: `full ${result.full_before}->${result.full_after}; ` + `archive ${result.oneline_before}->${result.oneline_after}; ` + `omitted ${result.omitted_count ?? 0}; dropped ${result.dropped}`,
    });
  }
  return operations;
}

export function fixCompaction(projectRoot: string, options: StateMutationOptions = {}): CompactionOperation[] {
  return withStateMutation(projectRoot, (transaction) => fixCompactionUnlocked(projectRoot, transaction), options);
}

export function runCompaction(projectRoot: string, mode = "check", options: StateMutationOptions = {}): CompactionOperation[] {
