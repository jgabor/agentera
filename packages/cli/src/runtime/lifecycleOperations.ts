...
export const LIFECYCLE_OPERATION_CONTRACT_RELATIVE_PATH = "references/adapters/runtime-lifecycle-operation-contract.yaml";
...
export const LIFECYCLE_RESOURCE_STATES = ["exact", "modified", "missing", "legacy", "symlinked", "wrong_type", "partial_managed", "unowned", "unsafe_path", "ambiguous_ownership"] as const;
export const LIFECYCLE_PLAN_ACTIONS = ["create", "update", "remove", "finalize_ownership", "noop", "blocked_unowned", "action_required"] as const;
export const LIFECYCLE_APPLY_STATUSES = ["applied", "noop", "failed", "blocked_unowned", "skipped_dependency", "action_required"] as const;

export const LIFECYCLE_MANUAL_REVIEW_GUIDANCE = "The destination is not ledger-owned; review the collision manually. Agentera will not adopt it by name or equality.";
...
export type LifecycleOwnership = "managed" | "legacy" | "claimable" | "unowned" | "partial" | "ambiguous" | "undeclared";
...
export function lifecycleOperationFingerprint(spec: LifecycleOperationSpec): string | null {
  ...
  return spec.content === undefined ? null : fingerprintBytes(Buffer.isBuffer(spec.content) ? spec.content : Buffer.from(spec.content));
}

export function createLifecycleOwnershipManifest(operations: LifecycleOperationSpec[]): LifecycleOwnershipManifest {
  return {
...
export function validateLifecycleOwnershipLedger(value: unknown): string[] {
  ...
  for (const [index, record] of value.records.entries()) {
    ...
    if (typeof record.destination !== "string" || !path.isAbsolute(record.destination) || path.resolve(record.destination) !== record.destination) {
    ...
    if (record.identity !== null && (!isObject(record.identity) || typeof record.identity.device !== "string" || !/^\d+$/.test(record.identity.device as string) || typeof record.identity.inode !== "string" || !/^\d+$/.test(record.identity.inode as string))) {
...
export function writeLifecycleOwnershipLedgerAtomic(ledgerPath: string, ledger: LifecycleOwnershipLedger): void {
...
function actionRequired(spec: LifecycleOperationSpec, state: LifecycleResourceState, ownership: LifecycleOwnership, reason: string): PlannedLifecycleOperation {
  return planned(spec, state, ownership, "action_required", reason);
}

function planned(spec: LifecycleOperationSpec, state: LifecycleResourceState, ownership: LifecycleOwnership, action: LifecyclePlanAction, reason: string): PlannedLifecycleOperation {
  return {
    id: spec.id,
    destination: path.resolve(spec.destination),
    state,
...
function matchingDeclarations(manifest: LifecycleOwnershipManifest, spec: LifecycleOperationSpec): LifecycleOwnershipDeclaration[] {
  return manifest.resources.filter((resource) => resource.resourceId === spec.id);
}

function matchingRecords(ledger: LifecycleOwnershipLedger, spec: LifecycleOperationSpec): LifecycleOwnershipRecord[] {
  return ledger.records.filter((record) => record.resourceId === spec.id);
}

function planOne(spec: LifecycleOperationSpec, manifest: LifecycleOwnershipManifest, ledger: LifecycleOwnershipLedger, allowedRoots: string[], suppliedObservation?: LifecyclePathObservation): PlannedLifecycleOperation {
  const declarations = matchingDeclarations(manifest, spec);
  if (declarations.length !== 1) {
    return actionRequired(spec, declarations.length === 0 ? "ambiguous_ownership" : "ambiguous_ownership", declarations.length === 0 ? "undeclared" : "ambiguous", declarations.length === 0 ? "no Agentera ownership declaration matches the resource" : "multiple Agentera ownership declarations match the resource");
  }
  const declaration = declarations[0];
  const expectedFingerprint = lifecycleOperationFingerprint(spec);
  if (path.resolve(declaration.destination) !== path.resolve(spec.destination) || declaration.kind !== spec.kind || declaration.intent !== spec.intent || declaration.expectedFingerprint !== expectedFingerprint) {
    return actionRequired(spec, "ambiguous_ownership", "undeclared", "operation does not match its Agentera ownership declaration");
  }
  ...
  if (records.length > 1) {
    return actionRequired(spec, "ambiguous_ownership", "ambiguous", "multiple ownership ledger records match the resource");
  }
  const record = records[0];
  if (record && (path.resolve(record.destination) !== path.resolve(spec.destination) || record.kind !== spec.kind)) {
    return actionRequired(spec, "ambiguous_ownership", "ambiguous", "ownership ledger record does not match the declared destination and kind");
  }
  if (record?.scope === "partial") {
    return actionRequired(spec, "partial_managed", "partial", "partial ownership cannot authorize a whole-resource mutation");
  }

  if (!record && observation.kind !== "missing") {
    return planned(spec, "unowned", "unowned", "blocked_unowned", "pre-existing resource has no matching Agentera ownership ledger record");
  }
  if (observation.kind === "missing") {
    if (spec.intent === "remove") {
      return record ? planned(spec, "missing", record.status === "legacy" ? "legacy" : "managed", "finalize_ownership", "resource is absent; stale ownership record will be removed") : planned(spec, "missing", "claimable", "noop", "resource is already absent");
    }
    return planned(spec, "missing", record?.status === "legacy" ? "legacy" : record ? "managed" : "claimable", "create", "declared resource is absent and can be created safely");
  }

  if (observation.kind === "symlink" && spec.kind !== "symlink") {
    return actionRequired(spec, "symlinked", record?.status === "legacy" ? "legacy" : "managed", "resource is a symlink where a non-symlink was declared");
  }
  if (observation.kind !== spec.kind) {
    return actionRequired(spec, "wrong_type", record?.status === "legacy" ? "legacy" : "managed", `resource kind ${observation.kind} does not match declared kind ${spec.kind}`);
  }
  if (record?.status === "pending_create" && !record.identity) {
    if (observation.fingerprint === expectedFingerprint) {
      return planned(spec, "exact", "managed", "finalize_ownership", "interrupted create published exact bytes before its identity record; ownership can be recovered from the observed resource");
    }
    return actionRequired(spec, "partial_managed", "partial", "interrupted create has no recorded publication identity and the observed resource does not match exactly");
  }
  if (!record?.identity) {
    return actionRequired(spec, record?.status === "pending_create" ? "partial_managed" : "ambiguous_ownership", record?.status === "pending_create" ? "partial" : "ambiguous", "ownership record has no published resource identity");
  }
  if (!sameLifecycleIdentity(record.identity, observation.identity)) {
    return actionRequired(spec, "ambiguous_ownership", "ambiguous", "resource identity no longer matches the ownership ledger");
  }
  if (record?.status === "pending_create") {
    if (observation.fingerprint === expectedFingerprint) {
      return planned(spec, "exact", "managed", "finalize_ownership", "interrupted create completed; ownership ledger can be finalized");
    }
    return spec.kind === "file" && spec.intent === "ensure"
      ? planned(spec, "modified", "managed", "update", "interrupted Agentera-created file will resume through its recorded identity")
      : actionRequired(spec, "partial_managed", "partial", "interrupted create differs and has no safe conditional replacement primitive");
  }
  if (spec.intent === "remove") {
    ...
    if (!secureLifecycleRemovalAvailable()) {
      return actionRequired(spec, state, ownership, "safe removal requires Linux /proc/self/fd pinned-parent access");
    }
    if (!record?.fingerprint || record.fingerprint !== observation.fingerprint) {
      return actionRequired(spec, state, ownership, "ownership ledger fingerprint must match the observed resource before removal");
    }
    if (spec.kind === "directory" && fs.readdirSync(spec.destination).length > 0) {
      return actionRequired(spec, "partial_managed", "partial", "non-empty directories require separately declared child removals");
    }
    return planned(spec, state, ownership, "remove", "owned resource identity and fingerprint match for pinned-parent removal");
  }
  ...
  if (observation.fingerprint === expectedFingerprint) {
    if (record.fingerprint !== expectedFingerprint) {
      return planned(spec, "exact", "managed", "finalize_ownership", "resource is exact; ownership ledger fingerprint will be refreshed");
    }
    return planned(spec, "exact", "managed", "noop", "owned resource already matches");
  }
  if (spec.kind !== "file") {
    return actionRequired(spec, "modified", "managed", "safe conditional replacement is unavailable for symlinks and directories");
  }
...
function validateRequest(request: LifecycleOperationRequest): void {
  ...
  const manifestErrors = validateLifecycleOwnershipManifest(request.manifest).filter((error) => !error.includes("duplicates"));
  if (manifestErrors.length > 0) throw new LifecycleOperationError(manifestErrors.join("; "));
  const ledgerErrors = validateLifecycleOwnershipLedger(request.ledger ?? emptyLifecycleOwnershipLedger()).filter((error) => !error.includes("duplicates"));
...
function executionOrder(operations: LifecycleOperationSpec[]): LifecycleOperationSpec[] {
  ...
  while (pending.length > 0) {
    const index = pending.findIndex((operation) => (operation.dependsOn ?? []).every((dependency) => emitted.has(dependency)));
    if (index < 0) {
...
export function planLifecycleOperations(request: LifecycleOperationRequest): LifecycleOperationPlan {
  ...
  return {
    schemaVersion: "agentera.lifecycleOperationPlan.v1",
    mode: "preview",
    operations: ordered.map((operation) => planOne(operation, request.manifest, ledger, request.allowedRoots)),
    request,
  };
}

function nextManagedRecord(spec: LifecycleOperationSpec, identity: LifecycleResourceIdentity): LifecycleOwnershipRecord {
  return {
    resourceId: spec.id,
    destination: path.resolve(spec.destination),
    kind: spec.kind,
...
function withRecord(ledger: LifecycleOwnershipLedger, record: LifecycleOwnershipRecord | null, resourceId: string): LifecycleOwnershipLedger {
  const records = ledger.records.filter((candidate) => candidate.resourceId !== resourceId);
  if (record) records.push(record);
  return { ...ledger, records };
}

function publishLedger(next: LifecycleOwnershipLedger, persist: LifecycleApplyOptions["persistLedger"]): LifecycleOwnershipLedger {
...
export function applyLifecycleOperations(plan: LifecycleOperationPlan, options: LifecycleApplyOptions = {}): LifecycleApplyResult {
  ...
  for (const spec of executionOrder(request.operations)) {
    ...
    try {
      if (current.action === "finalize_ownership") {
        const identity = verifyLifecycleResourceAtPublication(spec, observation, options.beforePublication);
        const next = spec.intent === "remove" ? withRecord(ledger, null, spec.id) : withRecord(ledger, nextManagedRecord(spec, identity as LifecycleResourceIdentity), spec.id);
        ...
        if (current.action === "create" && publishedIdentity) {
          ...
          ledger = publishLedger(withRecord(ledger, publishedPending, spec.id), options.persistLedger);
        }
        if (current.action !== "remove") {
          const next = withRecord(ledger, nextManagedRecord(spec, publishedIdentity as LifecycleResourceIdentity), spec.id);
          ledger = publishLedger(next, options.persistLedger);
        }
      }
      const result: AppliedLifecycleOperation = {
      ...
      const createdIdentity = error instanceof LifecyclePublicationError ? error.createdIdentity : undefined;
  ...
  const requiredUnmet = results.filter((result) => result.required && result.status !== "applied" && result.status !== "noop").map((result) => result.id);
  return {
    schemaVersion: "agentera.lifecycleApplyResult.v1",
