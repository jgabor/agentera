...
export const LIFECYCLE_AUTHORITY_RELATIVE_PATH = "references/adapters/runtime-lifecycle-authority.yaml";
export const LIFECYCLE_ADAPTER_CONTRACT_RELATIVE_PATH = "references/adapters/runtime-lifecycle-adapters.yaml";
export const NATIVE_RESOURCE_CLEANUP_CONTRACT_RELATIVE_PATH = "references/adapters/runtime-retired-resources.yaml";

const EXPECTED_EVIDENCE_FIELDS = ["host_present", "installed", "enabled", "trusted"] as const;
export const LIFECYCLE_APPLICABILITY_VALUES = ["required", "conditional", "not_applicable"] as const;
export const LIFECYCLE_ACTION_CLASS_VALUES = ["repairable_owned", "manual_verification", "unobservable_gap"] as const;
export const LIFECYCLE_COMMAND_ELIGIBILITY_VALUES = ["preview", "apply", "manual", "diagnostic"] as const;
const EVIDENCE_VALUES = new Set<LifecycleEvidenceValue>([true, false, "unknown", "denied", "not_applicable"]);
const INVENTORY_DECLARATIONS = [/^\s*["']?(?:active_runtimes|active_runtime_ids|active_runtime_order)["']?\s*:/, /^\s*(?:export\s+)?const\s+(?:activeRuntimeIds|ACTIVE_RUNTIME_IDS)\s*=/];
...
export type LifecycleSupportFloorBlockerCode = "canonical_skill_unknown" | "canonical_skill_not_detected" | "diagnosis_incomplete" | "mandatory_evidence_missing" | "mandatory_evidence_unknown" | "mandatory_evidence_denied" | "mandatory_trust_denied" | "mandatory_evidence_not_applicable";
...
export function validateLifecycleAuthorityData(data: unknown, sourcePath = LIFECYCLE_AUTHORITY_RELATIVE_PATH): string[] {
  if (!isMapping(data)) return [sourceError(sourcePath, "1", "authority must be a YAML object")];
  const errors: string[] = [];
  if (data.status !== "migration_only_authority") {
    errors.push(sourceError(sourcePath, "status", "must be migration_only_authority"));
  }
  ...
  if (data.adapter_contract !== LIFECYCLE_ADAPTER_CONTRACT_RELATIVE_PATH) {
    errors.push(sourceError(sourcePath, "adapter_contract", `must point to ${LIFECYCLE_ADAPTER_CONTRACT_RELATIVE_PATH}`));
  }
  if (data.native_resource_cleanup_contract !== NATIVE_RESOURCE_CLEANUP_CONTRACT_RELATIVE_PATH) {
    errors.push(sourceError(sourcePath, "native_resource_cleanup_contract", `must point to ${NATIVE_RESOURCE_CLEANUP_CONTRACT_RELATIVE_PATH}`));
  }

  const canonicalSkill = data.canonical_skill;
  if (!isMapping(canonicalSkill) || canonicalSkill.path !== "~/.agents/skills/agentera") {
    errors.push(sourceError(sourcePath, "canonical_skill.path", "must be ~/.agents/skills/agentera"));
  }
  if (!isMapping(canonicalSkill) || canonicalSkill.required_for_support_floor !== true) {
    errors.push(sourceError(sourcePath, "canonical_skill.required_for_support_floor", "must be true"));
  }

  const projection = data.projection_contract;
  if (!isMapping(projection) || projection.schema_version !== "agentera.runtimeLifecycleProjection.v1") {
    errors.push(sourceError(sourcePath, "projection_contract.schema_version", "must be agentera.runtimeLifecycleProjection.v1"));
  }
  if (!isMapping(projection) || projection.snapshot_identity !== "deterministic_sha256") {
    errors.push(sourceError(sourcePath, "projection_contract.snapshot_identity", "must be deterministic_sha256"));
  }
  if (!isMapping(projection) || JSON.stringify(projection.applicability) !== JSON.stringify(LIFECYCLE_APPLICABILITY_VALUES)) {
    errors.push(sourceError(sourcePath, "projection_contract.applicability", `must be ${LIFECYCLE_APPLICABILITY_VALUES.join(", ")}`));
  }
  if (!isMapping(projection) || JSON.stringify(projection.action_classes) !== JSON.stringify(LIFECYCLE_ACTION_CLASS_VALUES)) {
    errors.push(sourceError(sourcePath, "projection_contract.action_classes", `must be ${LIFECYCLE_ACTION_CLASS_VALUES.join(", ")}`));
  }
  if (!isMapping(projection) || JSON.stringify(projection.command_eligibility) !== JSON.stringify(LIFECYCLE_COMMAND_ELIGIBILITY_VALUES)) {
    errors.push(sourceError(sourcePath, "projection_contract.command_eligibility", `must be ${LIFECYCLE_COMMAND_ELIGIBILITY_VALUES.join(", ")}`));
  }
  if (!isMapping(projection) || projection.shared_resource_rule !== "selected_and_required_by_at_least_one_selected_runtime") {
    errors.push(sourceError(sourcePath, "projection_contract.shared_resource_rule", "must require selection and at least one selected runtime"));
  }
  ...
  if (JSON.stringify(evidenceFields) !== JSON.stringify(EXPECTED_EVIDENCE_FIELDS)) {
    errors.push(sourceError(sourcePath, "evidence_contract.mandatory_fields", `must be ${EXPECTED_EVIDENCE_FIELDS.join(", ")}`));
  }
  ...
  if (!isMapping(supportFloor) || JSON.stringify(supportFloor.requirements) !== JSON.stringify(["canonical_shared_skill_detected", "diagnosis_complete", "mandatory_evidence_resolved"])) {
    errors.push(sourceError(sourcePath, "support_floor.requirements", "must require canonical skill detection, complete diagnosis, and resolved mandatory evidence"));
  }
  const evidenceRules = isMapping(supportFloor) && isMapping(supportFloor.evidence_rules) ? supportFloor.evidence_rules : {};
  ...
  if (JSON.stringify(evidenceRules.known_false_diagnoses_degraded) !== JSON.stringify(["host_present", "installed", "enabled"])) {
    errors.push(sourceError(sourcePath, "support_floor.evidence_rules.known_false_diagnoses_degraded", "must be host_present, installed, enabled"));
  }
  if (evidenceRules.not_applicable_scope !== "unobserved_conditional_surface_only") {
    errors.push(sourceError(sourcePath, "support_floor.evidence_rules.not_applicable_scope", "must be unobserved_conditional_surface_only"));
  }
  ...
  if (runtimes.length !== 0) {
    errors.push(sourceError(sourcePath, "active_runtimes", "must be empty because repository-native runtime integrations are retired"));
  }
  const historicalInputs = data.historical_inputs;
  if (!Array.isArray(historicalInputs) || historicalInputs.length !== 1 || !isMapping(historicalInputs[0]) || historicalInputs[0].id !== "claude" || JSON.stringify(historicalInputs[0].purposes) !== JSON.stringify(["consent_gated_historical_import"])) {
    errors.push(sourceError(sourcePath, "historical_inputs", "must contain only claude for consent-gated historical import"));
  }
  const aliases = data.migration_aliases;
  const cursorAgent = isMapping(aliases) ? aliases["cursor-agent"] : null;
  if (!isMapping(cursorAgent) || cursorAgent.runtime_id !== "cursor" || cursorAgent.surface_id !== "cli") {
    errors.push(sourceError(sourcePath, "migration_aliases.cursor-agent", "must route to runtime cursor surface cli"));
  }
  return errors;
}

export function loadLifecycleAuthority(authorityPath = path.join(resolveSourceRoot(), LIFECYCLE_AUTHORITY_RELATIVE_PATH)): RuntimeLifecycleAuthority {
...
export function validateLifecycleAuthorityRoot(root = resolveSourceRoot()): string[] {
  ...
  for (const location of inventoryDeclarations(root, path.resolve(authorityPath))) {
    errors.push(`${location}: duplicate active runtime inventory; authority is ${LIFECYCLE_AUTHORITY_RELATIVE_PATH}:active_runtimes`);
  }
  return errors;
}

function observedSurfaceExpected(definition: LifecycleSurfaceDefinition, observation: LifecycleSurfaceObservation | undefined, authority: RuntimeLifecycleAuthority): boolean {
  ...
  return authority.supportFloorPolicy.notApplicableScope === "unobserved_conditional_surface_only" && observation !== undefined && hostPresent === true;
}

function mandatoryEvidenceViolation(field: LifecycleEvidenceField, value: LifecycleEvidenceValue | undefined, surfaceId: string, authority: RuntimeLifecycleAuthority): LifecycleSupportFloorViolation | null {
  ...
  if (field === "trusted" && authority.supportFloorPolicy.deniedMandatoryTrustBlocks && (value === "denied" || value === false)) {
    return {
...
function surfaceState(definition: LifecycleSurfaceDefinition, observation: LifecycleSurfaceObservation | undefined, authority: RuntimeLifecycleAuthority): LifecycleSurfaceState {
  const evidence = observation?.evidence ?? {};
  const expected = observedSurfaceExpected(definition, observation, authority);
  const applicability: LifecycleApplicability = definition.presence === "required" ? "required" : expected ? "conditional" : "not_applicable";
  ...
  else if (supportFloorViolations.some((violation) => violation.code === "mandatory_trust_denied" || violation.code === "mandatory_evidence_denied" || violation.code === "mandatory_evidence_not_applicable")) status = "blocked";
...
export function buildRuntimeLifecycleState(authority: RuntimeLifecycleAuthority, observations: LifecycleRuntimeObservation[]): RuntimeLifecycleState {
  const duplicateObservationIds = observations.map((observation) => observation.runtimeId).filter((id, index, ids) => ids.indexOf(id) !== index);
  if (duplicateObservationIds.length > 0) {
    throw new LifecycleAuthorityError(`duplicate lifecycle observations for runtime ${[...new Set(duplicateObservationIds)].join(", ")}`);
  }
  ...
  if (unknown) {
    throw new LifecycleAuthorityError(`unknown active runtime observation ${unknown.runtimeId}; expected ${[...activeIds].join(", ")}`);
  }
  ...
  const runtimes = authority.runtimes.map((runtime): LifecycleRuntimeState => {
    ...
    const duplicateSurfaceIds = observedSurfaces.map((surface) => surface.id).filter((id, index, ids) => ids.indexOf(id) !== index);
    if (duplicateSurfaceIds.length > 0) {
      throw new LifecycleAuthorityError(`${runtime.id}: duplicate lifecycle observations for surface ${[...new Set(duplicateSurfaceIds)].join(", ")}`);
    }
    const unknownSurface = observedSurfaces.find((surface) => !runtime.surfaces.some((definition) => definition.id === surface.id));
    if (unknownSurface) {
      throw new LifecycleAuthorityError(`${runtime.id}: unknown lifecycle surface ${unknownSurface.id}; expected ${runtime.surfaces.map((surface) => surface.id).join(", ")}`);
    }
    ...
    const uniqueViolations = violations.filter((violation, index, all) => all.findIndex((candidate) => candidate.code === violation.code && candidate.surfaceId === violation.surfaceId && candidate.field === violation.field) === index);
    const unmet = uniqueViolations.map((violation) => [violation.code, violation.surfaceId, violation.field].filter(Boolean).join(":"));
