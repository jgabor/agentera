...
function forbiddenDestinationSegment(destination: string, forbidden: string[]): string | null {
  const segments = destination.toLowerCase().split(/[\\/]+/).filter(Boolean);
...
export function validateRuntimeLifecycleAdapterContractData(
  ...
  if (!sameList(data.categories, RUNTIME_ADAPTER_CATEGORIES)) {
    errors.push(
      sourceError(sourcePath, "categories", `must be exactly ${RUNTIME_ADAPTER_CATEGORIES.join(", ")}`),
    );
  }
  ...
  sharedResources.forEach((resource, index) =>
    validateResource(resource, `shared_resources[${index}]`, sourcePath, forbidden, runtimeIds, errors));
  managedResources.forEach((resource, index) =>
    validateResource(resource, `managed_resources[${index}]`, sourcePath, forbidden, runtimeIds, errors));
  ...
  adapters.forEach((adapter, adapterIndex) => {
    ...
    for (const [actionIndex, action] of nativeActions.entries()) {
      ...
      const commandValid = typeof action.command === "string"
    ...
    return {
      ...
      skillLocations: (adapter.skill_locations as JsonObject[]).map((location) => ({
        ...
        surfaces: Array.isArray(location.surfaces)
          ...
          : authority.runtimes.find((runtime) => runtime.id === adapter.runtime_id)?.surfaces.map((surface) => surface.id) ?? [],
      })),
      categories,
