...
import {
  ...
  type GlossaryConversationEvidenceExpectation,
...
export type RecurringReviewReason =
  | typeof RECURRING_REASONS.conflictingMeaning
  | typeof RECURRING_REASONS.uncertainScope
  | typeof RECURRING_REASONS.inferredUsageRequiresReview;
...
function semanticValues(record: JsonObject): string[] {
  ...
  if (record.source_kind === "project_config_signal") {
    return Array.isArray(data.signals)
      ? data.signals.filter((value): value is string => typeof value === "string")
      : [];
  }
  if (record.source_kind === "conversation_turn") {
    return ["text", "prompt", "content"].flatMap((field) =>
...
function resolvedSignal(signal: SignalRecord, record: JsonObject): ResolvedSignal | null {
  if (record.source_id !== signal.source_id || record.source_kind !== signal.source_kind)
    return null;
  ...
  return {
    ...
    provenanceValid:
      validDigest(firstString(signal.origin_id, record.origin_id)) &&
      validDigest(firstString(signal.content_fingerprint, record.content_fingerprint)),
  };
}
...
function hasCaselessTerm(text: string, term: string): boolean {
  ...
  const prefix =
    characters[0] && new RegExp(`^${IDENTIFIER_CONTINUATION}$`, "u").test(characters[0])
      ? `(?<!${IDENTIFIER_CONTINUATION})`
      : "";
  const last = characters.at(-1) ?? "";
  const suffix = new RegExp(`^${IDENTIFIER_CONTINUATION}$`, "u").test(last)
    ? `(?!${IDENTIFIER_CONTINUATION})`
    : "";
...
function scalarBefore(text: string, index: number): string {
  ...
  const start =
    lastUnit >= 0xdc00 && lastUnit <= 0xdfff && previousUnit >= 0xd800 && previousUnit <= 0xdbff
      ? index - 2
      : index - 1;
...
function touchesMalformedCombiningSequence(text: string, start: number, end: number): boolean {
  ...
  return (
    INTERNAL_SEPARATOR_RE.test(next) && COMBINING_MARK_RE.test(scalarAt(text, end + next.length))
  );
}
...
function isNoiseToken(value: string, text: string, start: number, end: number): boolean {
  ...
  if (/^\d+$/.test(value) || classifyRecurringLexicalToken(value, text, start, end) !== null)
    return true;
...
function meaningCues(term: string, values: string[]): Set<string> {
  ...
  const patterns = [
    new RegExp(`${escaped}\\s+(?:means|refers to|is)\\s+([^.!?\\n]+)`, "giu"),
    new RegExp(`(?:means|refers to)\\s+${escaped}\\s+([^.!?\\n]+)`, "giu"),
  ];
  for (const value of values) {
...
function scopeMarkers(term: string, values: string[]): { project: boolean; personal: boolean } {
  ...
  const projectPattern = new RegExp(
    `(?:project[- ]only|repository[- ]only|for this project|project\\s+(?:term|meaning|definition)\\s*[:=]?\\s*${escaped})`,
    "iu",
  );
  const personalPattern = new RegExp(
    `(?:personal[- ]only|for me|my\\s+(?:term|meaning|definition)\\s*[:=]?\\s*${escaped})`,
    "iu",
  );
  return {
...
function isProjectOnly(
  records: readonly ResolvedSignal[],
  occurrences: readonly TermOccurrence[],
  route: "instruction" | "conversation",
): boolean {
  if (records.length === 0) return false;
  const dataScopes = records.map((item) => String(dataObject(item.record).scope ?? ""));
  const explicitProjectScope = dataScopes.every(
    (scope) => scope === "project" || scope === "project_only",
  );
  const projectConfigOnly = records.every(
    (item) => item.signal.source_kind === "project_config_signal",
  );
  const oneProject = new Set(records.map((item) => item.projectId)).size === 1;
  const markedProject =
    occurrences.length > 0 && occurrences.every((item) => item.projectScope && !item.personalScope);
  if (route === "conversation") return explicitProjectScope || markedProject;
  return (
    (projectConfigOnly && oneProject) ||
    (explicitProjectScope && projectConfigOnly) ||
    markedProject
  );
}

function groupOccurrences(
  resolved: readonly ResolvedSignal[],
  allowedSourceKinds: ReadonlySet<string>,
  allowedSignalTypes: ReadonlySet<string>,
  includeAgents: boolean,
): Map<string, TermGroup> {
  const groups = new Map<string, TermGroup>();
  for (const source of resolved) {
    if (
      !allowedSourceKinds.has(source.signal.source_kind) ||
      !allowedSignalTypes.has(source.signal.signal_type)
    )
      continue;
...
function orderedSignals(signals: readonly ResolvedSignal[]): ResolvedSignal[] {
  return [...signals].sort(
    (left, right) =>
      compareText(left.signal.source_id, right.signal.source_id) ||
      compareText(left.signal.evidence_anchor, right.signal.evidence_anchor),
  );
}
...
function independentEvidence(records: readonly ResolvedSignal[]): IndependentEvidence {
  ...
  for (const record of orderedSignals(records)) {
    ...
    const duplicate =
      sourceIds.has(record.signal.source_id) ||
      anchors.has(record.signal.evidence_anchor) ||
      (record.originId !== null && origins.has(record.originId)) ||
      (record.contentFingerprint !== null && fingerprints.has(record.contentFingerprint));
    if (duplicate) {
      copied = true;
      continue;
    }
...
function expectationFor(
  term: string,
  identity: string,
  input: RecurringGlossaryMiningInput,
): GlossaryConversationEvidenceExpectation | undefined {
  return (
    input.conversationEvidenceByTerm?.get(identity) ??
    input.conversationEvidenceByTerm?.get(term) ??
    input.conversationEvidence
  );
}

function conversationFailureReason(
  records: readonly ResolvedSignal[],
  agentRecords: readonly ResolvedSignal[],
  expectation: GlossaryConversationEvidenceExpectation | undefined,
  generation: string,
  evidenceMax: number,
): RecurringAbstentionReason | null {
  const expectedAnchors = expectation?.qualifyingEvidenceAnchors;
  if (
    evidenceMax === 0 ||
    records.length > evidenceMax ||
    (Array.isArray(expectedAnchors) && expectedAnchors.length > evidenceMax)
  ) {
    return RECURRING_REASONS.conversationEvidenceBoundExceeded;
  }
  if (records.length === 0) {
    return agentRecords.length > 0
      ? RECURRING_REASONS.agentOnlyEvidence
      : RECURRING_REASONS.insufficientIndependentOrigins;
  }
...
function validateConversationEvidence(
  records: readonly ResolvedSignal[],
  expectation: GlossaryConversationEvidenceExpectation,
  generation: string,
): boolean {
  try {
    ...
    const retainedHistory = new Map<string, RetainedEvidence>(
      records.map((record) => [record.signal.evidence_anchor, retainedEvidence(record)]),
    );
...
function addAbstention(
  output: Map<string, RecurringGlossaryAbstention>,
  term: string,
  reason: RecurringAbstentionReason,
): void {
...
export function mineRecurringGlossaryCandidates(
  ...
  const selected = tier.records
    .filter(
      (signal) =>
        authority.inferredSignalTypes.includes(signal.signal_type) ||
        authority.conversationSignalTypes.includes(signal.signal_type),
    )
  ...
  const conversationGroups = groupOccurrences(
    resolved,
    conversationKinds,
    conversationTypes,
    false,
  );
  const agentConversationGroups = groupOccurrences(
    resolved,
    conversationKinds,
    conversationTypes,
    true,
  ...
  for (const identity of [...identities].sort(compareText)) {
    ...
    const term = canonicalTerm(
      group ?? { identity, terms: new Set(requestedTerm ? [requestedTerm] : []), occurrences: [] },
      requestedTerm,
    );
    if (!term) continue;
    ...
    if (conversationGroup) {
      ...
      const agentRecords = agentGroup
        ? agentGroup.occurrences
            .map((occurrence) => occurrence.source)
            .filter((record) => record.authorClass !== "user")
        : [];
      ...
      const failure = conversationFailureReason(
        userRecords,
        agentRecords,
        expected,
      ...
      if (
        failure === null &&
        expected &&
        validateConversationEvidence(userRecords, expected, tier.manifest.generation)
      ) {
  ...
  return {
    ...
    abstentions: [...abstentions.entries()]
      .sort(([left], [right]) => compareText(left, right))
      .map(([, result]) => result),
    recovery: null,
  };
