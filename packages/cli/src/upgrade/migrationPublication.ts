...
import {
  ...
  withPinnedLifecycleDirectory,
  type LifecyclePathObservation,
  type LifecyclePublicationBoundaryHook,
  type LifecyclePublicationKind,
} from "../runtime/lifecyclePublication.js";
...
export function bindMigrationResource(
...
export function verifyBoundMigrationResource(item: MigrationPhaseItem, role: EvidenceRole): void {
  ...
  verifyLifecycleResourceAtPublication(
    { id: `${item.action}.${role}`, destination: resource.destination, kind: resource.kind },
    resource.observation,
  );
}

export function withBoundMigrationDirectory<T>(
  item: MigrationPhaseItem,
  role: EvidenceRole,
  ...
  return withPinnedLifecycleDirectory(
    { id: `${item.action}.${role}`, destination: resource.destination, kind: "directory" },
    resource.observation,
    callback,
    hook,
  );
...
export function updateBoundMigrationFile(item: MigrationPhaseItem, role: EvidenceRole, content: string): void {
  ...
  publishLifecycleResource(
    { id: `${item.action}.${role}`, destination: resource.destination, kind: "file", content },
    "update",
    resource.observation,
  );
}
...
export function bindMigrationTree(
  item: MigrationPhaseItem,
  root: string,
  allowedRoots: string[],
): string | null {
...
export function removeBoundMigrationTree(item: MigrationPhaseItem): void {
  ...
  for (const entry of tree) {
    verifyLifecycleResourceAtPublication(
      { id: `${item.action}.verify`, destination: entry.destination, kind: entry.kind },
      entry.observation,
    );
  }
  for (const entry of [...tree].reverse()) {
    removeLifecycleResource(
      { id: `${item.action}.remove`, destination: entry.destination, kind: entry.kind },
      entry.observation,
    );
  }
}
