...
export function planDeclaredMarkerManagedFileItem(resource: RetiredResourceDiagnostic, source: string, roots: string[]): MigrationPhaseItem {
  ...
  try {
    ...
    return { resourceId: resource.id, status: "blocked", action: REVIEW_DECLARED_RESOURCE_ACTION, source, message: `preserved ${source}: expected marker on a readable regular declared file was not proven` };
  }
  ...
  if (resource.markerKind === "body_first_line_exact") {
    const frontmatter = /^---\r?\n[\s\S]*?\r?\n---(?:\r?\n|$)/.exec(body);
    marked = body.slice(frontmatter?.[0].length ?? 0).split(/\r?\n/).find((line) => line.trim() !== "") === expectedMarker;
  }
  ...
  if (!marked) return { resourceId: resource.id, status: "blocked", action: REVIEW_DECLARED_RESOURCE_ACTION, source, message: `preserved ${source}: expected managed marker is absent or misplaced; remove the marker to opt out` };
  const item: MigrationPhaseItem = {
    resourceId: resource.id, status: "pending", action: RETIRE_DECLARED_RESOURCE_ACTION, source,
    ownership: { kind: "managed-marker-file", identity: `${stat.dev}:${stat.ino}`, fingerprint: `sha256:${createHash("sha256").update(body).digest("hex")}` },
  ...
  if (error) return { ...item, status: "blocked", action: REVIEW_DECLARED_RESOURCE_ACTION, message: `preserved ${source}: ${error}` };
  return item;
}
...
function lifecycleItem(
  ...
  const operation = "plan" in result.nativeResourceCleanup
    ? result.nativeResourceCleanup.plan.operations[0]
    : null;
  const status = operation?.action === "remove"
    ? "pending"
  ...
  return {
    ...
    message: status === "pending"
      ? `will remove ownership-proven retired resource ${resourceId} at ${source}`
      : status === "noop"
        ? `retired resource ${resourceId} is already absent at ${source}`
        : `preserved ${source}: ${operation?.reason ?? "matching ownership evidence is unavailable"}`,
  };
}

export function planDeclaredRetiredResourceCleanupItems(
  ctx: MigrationContext,
  representedPaths: ReadonlySet<string> = new Set(),
  selectedResourceId: string | null = null,
  includeDefaultPlugin = false,
): MigrationPhaseItem[] {
  ...
  return diagnosis.resources.flatMap((resource) => resource.evidence.paths.flatMap((source) => {
    ...
    return [lifecycleItem(
      ...
      resource.id,
      source,
      resource.ownershipMode === "bounded_fingerprint_and_ledger",
    )];
  }));
}

export function applyDeclaredRetiredResourceCleanupItems(
  ...
  for (const item of items) {
    ...
    if (item.status !== "pending" || item.action !== RETIRE_DECLARED_RESOURCE_ACTION
      || !item.resourceId || !item.source) continue;
    if (item.ownership?.kind === "managed-marker-file") {
      try {
        ...
        const resource = diagnoseRetiredResources({ home: ctx.home, project: ctx.project, installRoot: ctx.appHome, env: ctx.env, sourceRoot: ctx.sourceRoot, resourceId: item.resourceId }).resources
          .find((entry) => entry.evidence.paths.includes(item.source!));
