...
function boundedList(root: string, sourceRoot: string, selected: DiscoveredEntity[], allEntities: DiscoveredEntity[], limit: number | undefined, cursor: string | undefined, order: string, command: string, filter: JsonObject, format = "json", envelope: JsonObject = {}, decodedCursor?: JsonObject, selectorInput?: EntityListSelectorInput): JsonObject {
  ...
  const getCommand = command.includes("tasks") ? "agentera state plan tasks get --id ID --format json" : "agentera state plan get --id ID --format json";
  const syntax = command.includes("tasks") ? "agentera state plan tasks list [PLAN_ID] [--limit N] [--cursor TOKEN] [--ids-only|--fields FIELDS] --format json" : "agentera state plan list [--status open|complete|archived] [--limit N] [--cursor TOKEN] [--ids-only|--fields FIELDS] --format json";
  const projectionOptions = { artifact: ARTIFACT, boundary: command.includes("tasks") ? TASK : PLAN, format, maxUtf8Bytes: declared.maxUtf8Bytes, getCommand, syntax, selector: selectorInput };
  ...
  const response: JsonObject = { schemaVersion: "agentera.stateList.v1", command, status: remaining ? "degraded" : "ok", entries: page.map((entity) => entry(root, entity)), counts: { total: selected.length, returned: page.length, remaining }, order, filters: filter, snapshot: { id: snap, first_page: !cursor, has_more: Boolean(remaining), candidate_count: selected.length }, source: { artifact: ARTIFACT, authority: "canonical_entity_files", root: declared.entityRoot }, retrieval: { get: getCommand, ...(next ? { continue: `${command.replace("state ", "agentera state ")}${familyIdentifier}${filterFlags}${selectorFlags} --limit ${take} --cursor ${next} --format json` } : {}) }, ...(remaining ? { omitted: true, omitted_count: remaining, omission_reason: "page_limit", next_cursor: next } : {}), ...envelope };
