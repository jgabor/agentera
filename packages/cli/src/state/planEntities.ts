...
function contract(sourceRoot = resolveSourceRoot()): Contract {
  ...
  const number = (value: unknown, field: string): number => {
    ...
    if (!Number.isSafeInteger(result) || result < 1) throw new Error(`invalid plan entity ${field} authority`);
    return result;
  };
...
function failure(kind: StateFailureClass, message: string, recovery: string, id?: string, details?: JsonObject): StateRetrievalFailure {
  ...
  return new StateRetrievalFailure(
    { schemaVersion: "agentera.stateFailure.v1", status: "fail", error: { class: kind, message, syntax: "agentera state plan get --id ID", example: `agentera state plan get --id ${exampleId}`, recovery, artifact: ARTIFACT, ...(id ? { id } : {}), ...(details ? { details } : {}) } },
...
function all(root: string, sourceRoot: string, discovery?: EntityDiscoveryResult): DiscoveredEntity[] {
  try {
    ...
    throw failure("unsupported_state", `plan replacement recovery state is unavailable: ${(error as Error).message}`, "Preserve '.agentera/.entity-recovery/plan-replacement', restore its last valid journal bytes, then retry the exact plan replacement.");
  }
  const discovered = discovery ? validateEntityDiscovery(root, sourceRoot, discovery) : validateEntityState(root, sourceRoot);
...
function planStatus(entity: DiscoveredEntity): string {
  return String(mapping(entity.record?.header) ? (entity.record!.header.status ?? "") : (entity.record?.status ?? ""));
}
...
function multipleOpenPlanConflict(open: DiscoveredEntity[], sourceRoot: string): OpenPlanConflictDiagnostic {
  return openPlanConflictDiagnostic(
    open.map((entity) => entity.id!),
    sourceRoot,
  );
}
...
function preservedPlanEffects(plan: DiscoveredEntity): JsonObject {
  return { id: plan.id!, from_status: planStatus(plan), to_status: "archived", preserved: ["task_records", "task_evaluations", "task_completion"] };
}

function planLifecycleDecision(entities: DiscoveredEntity[], intent: { verb: "create"; force: boolean } | { verb: "archive"; force: boolean; plan?: string }, sourceRoot: string): PlanLifecycleDecision {
  const plans = entities.filter((entity) => entity.boundary === PLAN);
  ...
  if (intent.verb === "create") {
    if (open.length > 1) {
      const conflict = multipleOpenPlanConflict(open, sourceRoot);
      reject({ class: "conflict", message: `${conflict.message}; implicit plan create cannot choose a predecessor`, diagnosis: conflict.details, recovery: conflict.recovery });
    }
    if (open.length === 1) {
      const predecessor = open[0];
      if (!intent.force) {
        reject({ class: "conflict", message: `open plan '${predecessor.id}' blocks plan create`, recovery: `Use agentera state plan create --force --input PLAN.yaml only to archive '${predecessor.id}' unchanged and publish a successor.` });
      }
      return { predecessor, archivedRecord: archivedPlanRecord(predecessor), replay: false, effects: { lifecycle: "forced_replacement", force: true, archived_predecessor: preservedPlanEffects(predecessor), successor_lineage: { field: "previous_plan_archived", predecessor: predecessor.id! } } };
    }
    ...
    return {
      ...
      replay: false,
      effects: { lifecycle: predecessor ? "completed_predecessor_replacement" : "create", force: intent.force, ...(predecessor ? { archived_predecessor: preservedPlanEffects(predecessor), successor_lineage: { field: "previous_plan_archived", predecessor: predecessor.id! } } : {}) },
    };
  }
  ...
  if (status === "open" && !intent.force) {
    reject({ class: "conflict", message: `open plan '${target.id}' cannot be archived without --force`, recovery: `Complete '${target.id}', or use agentera state plan archive --plan ${target.id} --force to preserve unfinished task history without claiming completion.` });
  }
  ...
  return { target, archivedRecord, replay, effects: { lifecycle: status === "open" ? "forced_archive" : "archive", force: intent.force, archived_plan: preservedPlanEffects(target) } };
}
...
function entityPath(root: string, sourceRoot: string, boundary: string, id: string): string {
  return path.join(root, contract(sourceRoot).entityRoot, ARTIFACT, boundary, `${id}.yaml`);
}
...
function createPlanEntitiesUnderLock(req: StateWriteRequest, options: Options & { publicationContext: EntityPublicationContext }): StateWriteEnvelope {
  ...
  const planRecord = structuredClone(input) as JsonObject;
  ...
  if (lifecycle.replacementInputSha256) planRecord.replacement_input_sha256 = lifecycle.replacementInputSha256;
  const taskRecords = tasks.map((task, index): JsonObject => {
    const record = structuredClone(task) as JsonObject;
  ...
  const resultExtra: JsonObject = { tasks: taskRecords.map((record, index) => ({ id: taskIds[index], artifact: ARTIFACT, record })), effects: lifecycle.effects, ...(marker ? { initialization: { atomic: true, marker: { path: path.join(req.projectRoot, options.publicationContext.markerPath), record: marker } } } : {}) };
  ...
  if (command === "state plan replace" && lifecycle.predecessor && lifecycle.archivedRecord && lifecycle.replacementInputSha256) {
    const targets = [
      ...
      ...publications.map((item) => ({ path: relative(req.projectRoot, entityPath(req.projectRoot, sourceRoot, item.boundary, item.id)), before: null, after: canonicalEntityEnvelopeBytes({ id: item.id, artifact: ARTIFACT, record: item.record }) })),
    ];
  ...
  try {
    if (lifecycle.predecessor && lifecycle.archivedRecord) {
      const archived = replaceEntityUnderLock({
        ...
        artifact: ARTIFACT,
        boundary: PLAN,
        id: lifecycle.predecessor.id!,
        expectedRecord: lifecycle.predecessor.record!,
        expectedBytes: exactDiscoveredEntityBytes(lifecycle.predecessor),
        migrationProvenance: lifecycle.predecessor.migrationProvenance,
        record: lifecycle.archivedRecord,
      });
    ...
    if (predecessor) {
      try {
        ...
        recoveryFailures.push(`predecessor restoration failed because ownership changed or publication was unsafe: ${(restoreError as Error).message}`);
      }
    }
    if (markerIdentity) {
      try {
...
function replacementEffects(predecessor: DiscoveredEntity, successor?: DiscoveredEntity): JsonObject {
  return {
    ...
    successor: { ...(successor ? { id: successor.id! } : {}), created: !successor, lineage: { field: "previous_plan_archived", predecessor: predecessor.id! } },
  };
}
...
function successorForPredecessor(entities: DiscoveredEntity[], predecessor: DiscoveredEntity): DiscoveredEntity | undefined {
  ...
  if (matches.length > 1) {
    reject({ class: "conflict", message: `archived predecessor '${predecessor.id}' has multiple canonical successor plans`, recovery: "Repair canonical plan lineage so one archived predecessor has one successor before retrying targeted replacement." });
  }
  return matches[0];
}
...
function assertNoUnselectedOpenPlans(entities: DiscoveredEntity[], selected: DiscoveredEntity[], sourceRoot: string): void {
  ...
  if (unselected.length > 0) {
    ...
    reject({ class: "conflict", message: `${conflict.message}; targeted plan replacement would leave ${unselected.length} unnamed open plan${unselected.length === 1 ? "" : "s"}`, diagnosis: conflict.details, recovery: conflict.recovery });
  }
}

function existingReplacementDecision(entities: DiscoveredEntity[], predecessor: DiscoveredEntity, successor: DiscoveredEntity, sourceRoot: string): PlanReplacementDecision {
  ...
  if (planStatus(predecessor) === "archived") {
    if (!derived || derived.id !== successor.id) {
      reject({ class: "conflict", message: `archived predecessor '${predecessor.id}' is already replaced by '${derived?.id ?? "no canonical successor"}', not '${successor.id}'`, recovery: "Retry with the derived bare successor ID, or correct the divergent replacement request before any lifecycle mutation." });
    }
    return { predecessor, successor, successorRecord: structuredClone(successor.record!), replay: true, effects: replacementEffects(predecessor, successor) };
  }
...
function createReplacementLifecycle(entities: DiscoveredEntity[], predecessor: DiscoveredEntity, inputSha256: string, sourceRoot: string): { lifecycle?: PlanLifecycleDecision; replaySuccessor?: DiscoveredEntity } {
  ...
  if (planStatus(predecessor) === "archived") {
    if (!successor) {
      reject({ class: "conflict", message: `archived predecessor '${predecessor.id}' has no canonical successor to replay`, recovery: "Do not create a second successor for an archived predecessor. Select an open predecessor or repair canonical lineage first." });
    }
  ...
  return { lifecycle: { predecessor, archivedRecord: archivedPlanRecord(predecessor), replacementInputSha256: inputSha256, replay: false, effects: replacementEffects(predecessor) } };
}

function publishExistingPlanReplacement(req: StateWriteRequest, options: Options & { publicationContext: EntityPublicationContext }, sourceRoot: string, decision: PlanReplacementDecision): StateWriteEnvelope {
  ...
  if (!decision.inputSha256) throw new Error(`successor '${decision.successor.id}' has no immutable replacement input identity`);
  publishPlanReplacement(
    options.publicationContext,
    sourceRoot,
    { kind: "existing", predecessor: decision.predecessor.id!, successor: decision.successor.id!, inputSha256: decision.inputSha256 },
    [
      {
      ...
      { path: relative(req.projectRoot, decision.successor.path), before: exactDiscoveredEntityBytes(decision.successor), after: canonicalEntityEnvelopeBytes({ id: decision.successor.id!, artifact: ARTIFACT, record: decision.successorRecord, migrationProvenance: decision.successor.migrationProvenance ?? undefined }) },
    ],
    {
      validate: () => {
        const validation = validateEntityState(options.publicationContext.pinnedPath(), sourceRoot, { kind: "project", projectRoot: options.publicationContext.validatedRoot });
...
function replacePlanEntitiesUnderLock(req: StateWriteRequest, options: Options & { publicationContext: EntityPublicationContext }): StateWriteEnvelope {
  ...
  recoverPendingPlanReplacement(options.publicationContext, sourceRoot, typeof successorId === "string" ? { predecessor: predecessorId, successor: successorId } : { predecessor: predecessorId, inputSha256 }, {
    validate: () => {
      const validation = validateEntityState(options.publicationContext.pinnedPath(), sourceRoot, { kind: "project", projectRoot: options.publicationContext.validatedRoot });
      if (!validation.valid) throw new Error(`targeted plan replacement graph failed state validation: ${validation.issues.map(({ message }) => message).join("; ")}`);
    },
...
function taskRecord(req: StateWriteRequest, plan: string): JsonObject {
  ...
  for (const field of ["evidence", "blocked_reason"]) if (input[field] !== undefined) record[field] = structuredClone(input[field]) as never;
  return record;
}
function logicalTaskContent(record: JsonObject): JsonObject {
  const content: JsonObject = { name: record.name, depends_on: Array.isArray(record.depends_on) ? (structuredClone(record.depends_on) as never) : [], acceptance: Array.isArray(record.acceptance) ? (structuredClone(record.acceptance) as never) : [] };
  for (const field of ["evidence", "blocked_reason"]) if (record[field] !== undefined) content[field] = structuredClone(record[field]) as never;
  return content;
}
function appendReplayOrConflict(entities: DiscoveredEntity[], planId: string, record: JsonObject): DiscoveredEntity | null {
  ...
  if (identical.length > 1) reject({ class: "conflict", message: `plan '${planId}' contains multiple identical tasks named '${String(record.name)}'; retry cannot choose one existing task` });
...
function hasSupersededPredecessor(entities: DiscoveredEntity[], planId: string, taskId: string): boolean {
  return entities.some((entity) => entity.boundary === TASK && entity.record?.plan === planId && entity.record?.status === "superseded" && Array.isArray(entity.record.superseded_by) && entity.record.superseded_by.includes(taskId));
}
...
function assertProjectedTask(entities: DiscoveredEntity[], id: string, record: JsonObject): void {
  ...
  if (record.status !== "complete" && hasSupersededPredecessor(entities, planId, id)) reject({ class: "conflict", message: `task '${id}' must remain complete while it is referenced by a superseded task` });
...
function supersedeTask(entities: DiscoveredEntity[], task: DiscoveredEntity, taskId: string, planId: string, values: Record<string, unknown>): JsonObject {
  ...
  if (task.record?.status === "superseded") {
    ...
    if (canonicalRecordJson(existing) === canonicalRecordJson(normalized) && task.record.superseded_reason === reason) return structuredClone(task.record);
...
export function mutatePlanEntities(req: StateWriteRequest, options: Options = {}): StateWriteEnvelope {
  ...
  if (req.spec.verb === "supersede") {
    ...
    const result = replaceEntity({
      ...
      boundary: TASK,
      id: taskId,
      expectedRecord: task.record!,
      expectedBytes: exactDiscoveredEntityBytes(task),
      migrationProvenance: task.migrationProvenance,
      record: superseded,
    });
  ...
  if (req.spec.verb === "update") {
    ...
    if (input.surprise !== undefined) {
      ...
      const result = replaceEntity({
        ...
        boundary: PLAN,
        id: plan.id!,
        expectedRecord: plan.record!,
        expectedBytes: exactDiscoveredEntityBytes(plan),
        migrationProvenance: plan.migrationProvenance,
        record: planRecord,
      });
...
function snapshot(root: string, entities: DiscoveredEntity[]): string {
  return createHash("sha256")
    .update(canonicalRecordJson(entities.map(({ id, boundary, record, path: file }) => ({ id, boundary, record, path: relative(root, file) })).sort((a, b) => canonicalRecordJson(a).localeCompare(canonicalRecordJson(b)))))
    .digest("hex");
}
function secret(root: string, authorityPath: string): Buffer {
  return createHash("sha256").update(path.resolve(root)).update("\0").update(fs.readFileSync(authorityPath)).digest();
}
function encode(value: JsonObject, root: string, authorityPath: string): string {
  const bytes = Buffer.from(canonicalRecordJson(value));
  return `${bytes.toString("base64url")}.${createHmac("sha256", secret(root, authorityPath)).update(bytes).digest("base64url")}`;
}
function decode(token: string, root: string, authorityPath: string): JsonObject {
  try {
    ...
    if (!mapping(value)) throw new Error();
    return value;
  } catch {
...
function entry(root: string, entity: DiscoveredEntity): JsonObject {
  return { id: entity.id!, artifact: ARTIFACT, record: entity.record!, provenance: { storage: "canonical_entity_file", path: relative(root, entity.path) } };
}
function boundedList(
  ...
  format = "json",
  envelope: JsonObject = {},
  decodedCursor?: JsonObject,
  selectorInput?: EntityListSelectorInput,
): JsonObject {
  const declared = contract(sourceRoot);
  const take = limit ?? declared.defaultLimit;
  if (!Number.isSafeInteger(take) || take < 1 || take > declared.maximumLimit) throw failure("invalid_request", `plan list limit must be 1..${declared.maximumLimit}`, "Use a limit in the declared range.");
  ...
  const selector = resolveEntityListSelector(
    selectorInput,
    selected.map((entity) => entry(root, entity)),
    projectionOptions,
  );
  ...
  if (cursor) {
    ...
    if (value.selector !== selectorKey) throw failure("cursor_invalid", "plan cursor selectors do not match this request", "Repeat the original selector, or omit --cursor to restart from current state.");
    if (value.snapshot_id !== snap || value.order !== order || canonicalRecordJson(value.filter) !== canonicalRecordJson(filter)) throw failure("cursor_snapshot_unavailable", "plan state or filters changed after this cursor snapshot", "Repeat the original filters, or omit --cursor to restart from current state.");
    const found = selected.findIndex((entity) => entity.id === value.after);
    if (found < 0) throw failure("cursor_snapshot_unavailable", "plan cursor continuation is unavailable", "Omit --cursor to restart.");
    start = found + 1;
  }
  ...
  const response: JsonObject = {
    ...
    entries: page.map((entity) => entry(root, entity)),
...
export function getPlanEntity(root: string, id: string, sourceRoot = resolveSourceRoot()): JsonObject {
  const entities = all(root, sourceRoot);
  const plan = selectedPlan(entities, id, sourceRoot);
  return {
    schemaVersion: "agentera.stateGet.v1",
    command: "state plan get",
    status: "ok",
    entry: entry(root, plan),
    tasks: entities
      .filter((entity) => entity.boundary === TASK && entity.record?.plan === id)
      .sort((a, b) => a.id!.localeCompare(b.id!))
      .map((entity) => entry(root, entity)),
    source_contract: { authority: "references/artifacts/state-storage-authority.yaml", detail: "full_entities" },
  };
}
export function listPlanEntities(root: string, limit?: number, cursor?: string, options: { sourceRoot?: string; format?: string; statuses?: string[]; discovery?: EntityDiscoveryResult; selector?: EntityListSelectorInput } = {}): JsonObject {
  const sourceRoot = options.sourceRoot ?? resolveSourceRoot();
  const entities = all(root, sourceRoot, options.discovery);
  const statuses = options.statuses?.length ? new Set(options.statuses) : undefined;
  const plans = entities
    .filter((entity) => entity.boundary === PLAN && (!statuses || statuses.has(planStatus(entity))))
    .sort((a, b) => String(mapping(b.record?.header) ? (b.record!.header.created ?? "") : "").localeCompare(String(mapping(a.record?.header) ? (a.record!.header.created ?? "") : "")) || a.id!.localeCompare(b.id!));
...
export function getPlanTaskEntity(root: string, id: string, planId?: string, sourceRoot = resolveSourceRoot()): JsonObject {
  const entities = all(root, sourceRoot);
  const task = taskFor(entities, id, planId);
  return { schemaVersion: "agentera.stateGet.v1", command: "state plan tasks get", status: "ok", entry: entry(root, task), source_contract: { authority: "references/artifacts/state-storage-authority.yaml", detail: "full_entity" } };
}
export function listPlanTaskEntities(root: string, planId?: string, limit?: number, cursor?: string, options: { sourceRoot?: string; format?: string; discovery?: EntityDiscoveryResult; selector?: EntityListSelectorInput } = {}): JsonObject {
  const sourceRoot = options.sourceRoot ?? resolveSourceRoot();
  const entities = all(root, sourceRoot, options.discovery);
  const declared = contract(sourceRoot);
  const decodedCursor = cursor ? decode(cursor, root, declared.authorityPath) : undefined;
  const cursorFilter = mapping(decodedCursor?.filter) ? decodedCursor.filter : undefined;
  const cursorPlan = typeof cursorFilter?.plan === "string" && ID.test(cursorFilter.plan) ? cursorFilter.plan : undefined;
  if (decodedCursor && !cursorPlan) throw failure("cursor_snapshot_unavailable", "plan state changed after this cursor snapshot", "Omit --cursor to restart from current state.");
  const plan = selectedPlan(entities, planId ?? cursorPlan, sourceRoot);
  const tasks = entities.filter((entity) => entity.boundary === TASK && entity.record?.plan === plan.id).sort((a, b) => a.id!.localeCompare(b.id!));
