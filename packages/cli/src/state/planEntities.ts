...
export function currentPlanEntityView(root: string, limit?: number, cursor?: string, status?: string, options: { sourceRoot?: string; format?: string } = {}): JsonObject {
  ...
  const response = boundedList(root, sourceRoot, tasks, entities, limit, cursor, TASK_ORDER, "state plan", { plan: plan.id!, status: status ?? null }, options.format);
  response.plan = entry(root, plan);
  return response;
}
