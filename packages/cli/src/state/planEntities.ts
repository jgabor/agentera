...
function selectedPlan(entities: DiscoveredEntity[], requested?: string): DiscoveredEntity {
  ...
  if (open.length === 0) throw failure("not_found", "no open plan exists", "Create a plan, or pass --plan ID to address completed plan history explicitly.");
  throw failure("ambiguous", `multiple open plans require --plan ID: ${open.map(({ id }) => id).sort().join(", ")}`, "List plans and retry with one bare --plan ID.");
}
...
function boundedList(root: string, sourceRoot: string, selected: DiscoveredEntity[], allEntities: DiscoveredEntity[], limit: number | undefined, cursor: string | undefined, order: string, command: string, filter: JsonObject, format = "json"): JsonObject {
