...
import { parseTodoUpdateBatch, todoUpdateBatchEffectSha256 } from "./todoUpdateBatch.js"; import { parseTodoCreateBatch, resolveTodoCreateBatchRecords, todoCreateBatchEffectSha256 } from "./todoCreateBatch.js"; import { matchesTodoTransitionBatchPostState, parseTodoTransitionBatch, todoTransitionBatchPostStateSha256 } from "./todoTransitionBatch.js";
...
function contract(boundary: string, sourceRoot = resolveSourceRoot()): Contract {
  ...
  return new StateRetrievalFailure({ schemaVersion: "agentera.stateFailure.v1", status: "fail", error: { class: kind, message, syntax: family?.syntax ?? `agentera state ${artifact} get --id ID`, example: family?.example ?? `agentera state ${artifact} get --id ${id ?? "qjtrmnpvka"}`, recovery, artifact, ...(id ? { id } : {}) } }, exitCode);
}
...
function assertTodoReferences(id: string, record: JsonObject, entities: TodoEntityView[]): void {
  ...
  if (violations.length) reject({
    ...
    violations,
    recovery: "Use bare ten-letter IDs returned by `agentera state todo list`; reference existing TODO items only and remove self-references or cycles, then retry.",
  });
}
