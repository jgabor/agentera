...
export function parseTodoTransitionBatch(input: JsonObject | null, verb: string): TodoTransitionBatch | null {
  ...
  const entries = (Array.isArray(values) ? values : []).map((value, index) => {
    ...
    const id = String(item.id ?? ""); const reason = String(item.reason ?? "").trim(); const date = String(item.date ?? ""); const severity = item.severity === undefined ? undefined : String(item.severity);
    ...
    const timestamp = Date.parse(`${date}T00:00:00Z`); if (!DATE.test(date) || Number.isNaN(timestamp) || new Date(timestamp).toISOString().slice(0, 10) !== date) violations.push(`${member}[${index}].date must be a valid YYYY-MM-DD date`);
  ...
  for (const { id } of entries) { if (id && seen.has(id)) violations.push(`duplicate ${verb} target '${id}'`); seen.add(id); }
  if (violations.length) reject({ class: "schema_violation", message: `todo ${verb} batch input is invalid`, violations, recovery: `Correct the strict ${version} envelope, then preview it again; no state was changed.` });
  return { verb, entries };
}
