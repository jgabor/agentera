...
export function migrationEnrichmentContract(): Record<string, unknown> {
  return {
    ...
    apply: `${BACKFILL_COMMAND} --project PROJECT --artifact ARTIFACT --number N --apply --force --preview-token TOKEN --format json`,
...
export function migrationEnrichmentFollowUp(
  ...
  return {
    ...
    apply: `${BACKFILL_COMMAND} --project ${selectedProject} --artifact ${artifactId} --number ${entryNumber} --apply --force --preview-token TOKEN --format json`,
    remote_contact: false,
    recovery:
