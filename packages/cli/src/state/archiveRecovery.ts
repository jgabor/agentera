...
function archivePath(
  ...
  return path.join(
    ...
    artifactId,
    `${number}${contract.archiveExtension}`,
  );
}

function rejectionForEntry(
  rejectionList: ArchiveRejection[],
  ...
  return rejectionList.find((candidate) => {
    ...
    if (
      ...
      candidatePath === path.resolve(artifactDirectory) ||
      (targetFromCandidate !== "" &&
        !targetFromCandidate.startsWith(".." + path.sep) &&
        !path.isAbsolute(targetFromCandidate))
    ) {
      return true;
    }
    const relative = path.relative(artifactDirectory, candidatePath);
    if (!relative || relative.includes(path.sep)) return false;
    const stem = relative.endsWith(contract.archiveExtension)
      ? relative.slice(0, -contract.archiveExtension.length)
      : "";
...
function errorClassForRejection(rejection: ArchiveRejection): ProjectionRecoveryClass {
  return rejection.class === "unsupported_artifact" ||
    rejection.reason === "unsupported_artifact" ||
    (rejection.reason === "invalid_envelope" && rejection.message.startsWith("schemaVersion must be"))
    ? "unsupported_state"
    : "corrupt";
}
...
function failure(
  ...
  artifactId: string,
  number: number | null,
  contract: NumberedArchiveContract | undefined,
  recoveryClass: ProjectionRecoveryClass,
  message: string,
  details: Record<string, unknown> = {},
): ProjectionRecovery {
  ...
  const status: ProjectionRecoveryStatus =
    recoveryClass === "not_found"
      ? "degraded"
      : recoveryClass === "unsupported_state"
        ? "unsupported"
        : "blocked";
  return {
    stable_id: id,
...
function verifyWithDiscovery(
  projectRoot: string,
  artifactId: string,
  record: JsonObject,
  contract: NumberedArchiveContract,
  discovery: NumberedArchiveDiscovery,
): ProjectionRecovery {
  ...
  if (number === null) {
    return failure(
      ...
      contract,
      "unsupported_state",
      `full entry ${id} has no supported positive archive entry number`,
    );
  }
  ...
  if (candidate) {
    ...
    try {
      ...
      return failure(
        ...
        contract,
        "unsupported_state",
        `full entry ${id} cannot be represented as canonical archive content: ${(error as Error).message}`,
      );
    }
    ...
    return failure(
      ...
      contract,
      "immutable_conflict",
      `archive record ${id} is valid but conflicts with the full current entry; summary publication is refused`,
      {
  ...
  if (rejection) {
    ...
    return failure(
      ...
      contract,
      recoveryClass,
      `archive record ${id} cannot be verified: ${rejection.message}; summary publication is refused`,
      { archive_rejection: rejection.reason, archive_rejection_class: rejection.class },
    );
  }
  return failure(
    projectRoot,
    ...
    contract,
    "not_found",
    `validated archive record ${id} is absent; summary publication is refused`,
  );
}
...
export function verifyArchiveForProjection(
  projectRoot: string,
  artifactId: string,
  record: JsonObject,
  ...
  try {
    ...
    return unsupportedVerification(
      ...
      artifactId,
      record,
      `artifact '${artifactId}' is outside the numbered archive authority: ${(error as Error).message}`,
    );
  }
  try {
    return verifyWithDiscovery(
      projectRoot,
      artifactId,
      record,
      contract,
      discoverNumberedArchives(projectRoot, { sourceRoot: options.sourceRoot }),
    );
  } catch (error) {
    return failure(
      projectRoot,
      artifactId,
      entryNumber(record, contract),
      contract,
      "unsupported_state",
      `archive authority could not verify ${stableId(artifactId, entryNumber(record, contract))}: ${(error as Error).message}`,
    );
  }
}

export function gateProjectionEntries(
  projectRoot: string,
  artifactId: string,
  records: JsonObject[],
  options: { sourceRoot?: string } = {},
  ...
  if (records.length === 0) {
    return {
      verified: [],
      refused: [],
      recovery: { status: "complete", attempted: 0, verified: 0, retained_full: 0, refused_count: 0, refusals: [] },
    };
  }
  ...
  try {
    ...
    const refused = records.map((entry) => ({
      entry,
      recovery: unsupportedVerification(
        projectRoot,
        artifactId,
        entry,
        `artifact '${artifactId}' is outside the numbered archive authority: ${(error as Error).message}`,
  ...
  try {
    ...
    const refused = records.map((entry) => ({
      ...
      recovery: failure(
        projectRoot,
        artifactId,
        entryNumber(entry, contract),
        contract,
        "unsupported_state",
        `archive authority could not verify ${stableId(artifactId, entryNumber(entry, contract))}: ${(error as Error).message}`,
      ),
    }));
    return {
      verified: [],
