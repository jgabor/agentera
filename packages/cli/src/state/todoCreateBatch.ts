...
export interface TodoCreateBatchEntry { local_ref: string; record: JsonObject }
export interface TodoCreateBatchInspection { strictEnvelope: boolean; creates: TodoCreateBatchEntry[]; violations: string[] }
export interface TodoCreateBatchEffectTarget { path: string; before_sha256: string | null; after_sha256: string }
...
export function inspectTodoCreateBatch(input: Record<string, unknown> | null): TodoCreateBatchInspection | null {
  ...
  const creates = (Array.isArray(input.creates) ? input.creates : []).map((value, index) => {
    ...
    if (mapping(validationRecord.readiness) && Array.isArray(validationRecord.readiness.dependencies)) validationRecord.readiness.dependencies = validationRecord.readiness.dependencies.map((dependency) => mapping(dependency) && typeof dependency.local_ref === "string" ? { artifact: "todo", id: "aaaaaaaaaa" } : dependency);
  ...
  for (const { local_ref } of creates) { if (local_ref && refs.has(local_ref)) violations.push(`duplicate local reference '${local_ref}'`); refs.add(local_ref); }
  ...
  const cyclic = (ref: string): boolean => { if (visiting.has(ref)) return true; if (visited.has(ref)) return false; visiting.add(ref); const found = (edges.get(ref) ?? []).some(cyclic); visiting.delete(ref); visited.add(ref); return found; };
...
export function parseTodoCreateBatch(input: Record<string, unknown> | null): TodoCreateBatchEntry[] | null {
  const inspected = inspectTodoCreateBatch(input); if (!inspected) return null;
  if (inspected.violations.length) reject({ class: "schema_violation", message: "todo create batch input is invalid", violations: inspected.violations, recovery: "Correct the strict agentera.todoCreateBatch.v1 envelope, then preview it again; no state was changed." });
...
export function resolveTodoCreateBatchRecords(entries: TodoCreateBatchEntry[], ids: Map<string, string>): TodoCreateBatchEntry[] {
  return entries.map(({ local_ref, record }) => { const copy = structuredClone(record); if (mapping(copy.readiness) && Array.isArray(copy.readiness.dependencies)) copy.readiness.dependencies = copy.readiness.dependencies.map((value) => mapping(value) && "local_ref" in value ? { artifact: "todo", id: ids.get(String(value.local_ref))! } : value); return { local_ref, record: copy }; });
}
