...
function relevant(root: string, sourceRoot: string): DiscoveredEntity[] {
  const discovery = discoverEntities(root, sourceRoot); const entities = discovery.entities.filter(({ artifact, boundary }) => artifact === OBJECTIVE_ARTIFACT || artifact === EXPERIMENT_ARTIFACT || boundary === OBJECTIVE || boundary === EXPERIMENT);
...
export function listObjectiveEntities(root: string, limit?: number, cursor?: string, options: { sourceRoot?: string; format?: string } = {}): JsonObject { const sourceRoot = options.sourceRoot ?? resolveSourceRoot(); const entities = relevant(root, sourceRoot); const selected = entities.filter(({ boundary }) => boundary === OBJECTIVE).sort((a, b) => String(mapping(b.record?.header) ? b.record!.header.created ?? "" : "").localeCompare(String(mapping(a.record?.header) ? a.record!.header.created ?? "" : "")) || a.id!.localeCompare(b.id!)); return boundedList(root, sourceRoot, OBJECTIVE_ARTIFACT, OBJECTIVE, selected, entities, limit, cursor, OBJECTIVE_ORDER, {}, options.format); }
