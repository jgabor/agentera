...
function contract(boundary: string, sourceRoot = resolveSourceRoot()): Contract {
  ...
  const definition = (Array.isArray(target.entities) ? target.entities : []).find((value) => mapping(value) && value.boundary === boundary);
  ...
  return new StateRetrievalFailure(
    {
      ...
      error: {
        class: kind,
        message,
        syntax: `agentera state ${artifact} get --id ID`,
...
function relevant(root: string, sourceRoot: string, precomputed?: EntityDiscoveryResult): DiscoveredEntity[] {
  const discovery = precomputed ? validateEntityDiscovery(root, sourceRoot, precomputed) : validateEntityState(root, sourceRoot);
...
function objectiveStatus(entity: DiscoveredEntity): string {
  return String(mapping(entity.record?.header) ? (entity.record!.header["status"] ?? "") : "");
}
function selectObjective(entities: DiscoveredEntity[], requested?: string): DiscoveredEntity {
  ...
  if (requested) {
    const matches = objectives.filter(({ id }) => id === requested);
    if (matches.length !== 1) throw failure(matches.length ? "ambiguous" : "not_found", OBJECTIVE_ARTIFACT, matches.length ? `objective ID '${requested}' has conflicting ownership` : `objective ID '${requested}' was not found`, "Run agentera state objective list and select one canonical objective ID.", requested);
    return matches[0];
  }
  ...
  throw failure(
    "ambiguous",
    OBJECTIVE_ARTIFACT,
    `multiple active objectives require an explicit ID: ${active
      .map(({ id }) => id)
      .sort()
      .join(", ")}`,
    "Run agentera state objective list and retry with --id ID.",
  );
}
...
function objectiveViolations(record: JsonObject, sourceRoot?: string): string[] {
  ...
  const objective = mapping(record.objective) ? record.objective : {};
  const metric = mapping(record.metric) ? record.metric : {};
  const baseline = mapping(record.baseline) ? record.baseline : {};
  const scope = mapping(record.scope) ? record.scope : {};
  ...
  for (const [name, value] of [
    ...
    if (typeof value !== "string" || !value.trim()) violations.push(`${name} is required`);
...
function assertValidState(root: string, sourceRoot: string): void {
  ...
  if (!state.valid)
    reject({
      ...
      recovery: "Run agentera check validate state and resolve every identity, relationship, or baseline conflict before retrying; no state was changed.",
    });
}
function assertBaseline(entities: DiscoveredEntity[], objective: string, candidate?: JsonObject): void {
  const records = entities.filter((entity) => entity.boundary === EXPERIMENT && entity.record?.objective === objective).map((entity) => entity.record!);
  ...
  if (records.length && count !== 1)
    reject({
      class: "conflict",
      message: `objective '${objective}' requires exactly one experiment with status baseline; found ${count}`,
      recovery: count ? "Keep one immutable baseline and resolve competing baseline ownership before retrying; no state was changed." : "Publish the baseline experiment first with status baseline; no state was changed.",
    });
}
function envelope(command: string, entity: { id: string; path: string; replay: boolean }, artifact: string, record: JsonObject, dryRun: boolean): StateWriteEnvelope {
  return {
    ...
    operation: {
      ...
      dry_run: dryRun,
      idempotent_replay: entity.replay,
    },
    validation: { status: "pass", violations: [] },
  };
}
function entityPath(root: string, sourceRoot: string, artifact: string, boundary: string, id: string): string {
  return path.join(root, contract(boundary, sourceRoot).entityRoot, artifact, boundary, `${id}.yaml`);
}

export function mutateObjectiveEntity(req: StateWriteRequest, options: Options = {}): StateWriteEnvelope {
  const sourceRoot = options.sourceRoot ?? resolveSourceRoot();
  const input = structuredClone(req.input ?? {}) as JsonObject;
  const violations = objectiveViolations(input, sourceRoot);
  if (violations.length) reject({ class: "schema_violation", message: "objective entity input is invalid", violations });
  if (!options.publicationContext) {
    const binding = detectStateModeBinding(req.projectRoot, sourceRoot);
    if (binding.mode !== "entities") throw new Error("objective entity mutation requires the durable entity-mode marker");
    try {
      return mutateObjectiveEntity(req, {
        ...options,
  ...
  return withEntityWriterLock(context, () => {
    assertValidState(context.pinnedPath(), sourceRoot);
    const entities = relevant(context.pinnedPath(), sourceRoot);
    if (req.spec.verb === "create") {
      const id = allocateEntityId(context.pinnedPath(), options.candidate, sourceRoot);
      if (req.dryRun)
        return envelope(
          "state objective create",
          {
            id,
            path: entityPath(req.projectRoot, sourceRoot, OBJECTIVE_ARTIFACT, OBJECTIVE, id),
            replay: false,
          },
          OBJECTIVE_ARTIFACT,
          input,
      ...
      try {
        const result = publishEntityUnderLock({
        ...
        assertValidState(context.pinnedPath(), sourceRoot);
        context.assertValid();
        return envelope("state objective create", result, OBJECTIVE_ARTIFACT, input, false);
      } catch (error) {
    ...
    const request = {
      ...
      boundary: OBJECTIVE,
      id: objective.id!,
      expectedRecord: objective.record!,
      expectedBytes: exactDiscoveredEntityBytes(objective),
      migrationProvenance: objective.migrationProvenance,
      record: input,
    };
    ...
    try {
      ...
      assertValidState(context.pinnedPath(), sourceRoot);
      context.assertValid();
      return envelope("state objective update", result, OBJECTIVE_ARTIFACT, input, false);
    } catch (error) {
...
export function publishExperimentEntity(req: StateWriteRequest, options: Options = {}): StateWriteEnvelope {
  const sourceRoot = options.sourceRoot ?? resolveSourceRoot();
  const raw = structuredClone(req.input ?? {}) as JsonObject;
  const inputViolations = experimentViolations({ ...raw, objective: "qjtrmnpvka" });
  if (inputViolations.length)
    reject({
      class: "schema_violation",
      message: "experiment entity input is invalid",
      violations: inputViolations,
    });
  if (!options.publicationContext) {
    const binding = detectStateModeBinding(req.projectRoot, sourceRoot);
    if (binding.mode !== "entities") throw new Error("experiment entity publication requires the durable entity-mode marker");
    try {
      return publishExperimentEntity(req, {
        ...options,
        publicationContext: binding.publicationContext,
      });
    } finally {
  ...
  return withEntityWriterLock(context, () => {
    assertValidState(context.pinnedPath(), sourceRoot);
    const entities = relevant(context.pinnedPath(), sourceRoot);
    const objective = selectObjective(entities, String(req.values.objective ?? ""));
    const record = { ...raw, objective: objective.id! };
    const requested = req.values.id === undefined ? undefined : String(req.values.id);
    if (requested) {
      const existing = experimentById(entities, requested, objective.id!);
      if (canonicalRecordJson(existing.record) !== canonicalRecordJson(record))
        reject({
          class: "conflict",
          message: `divergent content for immutable experiment ID '${requested}'; no state was changed`,
        });
      return envelope("state experiments publish", { id: requested, path: existing.path, replay: true }, EXPERIMENT_ARTIFACT, record, req.dryRun);
    }
    assertBaseline(entities, objective.id!, record);
    const id = allocateEntityId(context.pinnedPath(), options.candidate, sourceRoot);
    if (req.dryRun)
      return envelope(
        "state experiments publish",
        {
          id,
          path: entityPath(req.projectRoot, sourceRoot, EXPERIMENT_ARTIFACT, EXPERIMENT, id),
          replay: false,
        },
        EXPERIMENT_ARTIFACT,
        record,
    ...
    try {
      ...
      published = result;
      assertValidState(context.pinnedPath(), sourceRoot);
      assertBaseline(relevant(context.pinnedPath(), sourceRoot), objective.id!);
      context.assertValid();
      return envelope("state experiments publish", result, EXPERIMENT_ARTIFACT, record, false);
    } catch (error) {
      if (published?.publishedIdentity) context.removeExact(relative(req.projectRoot, published.path), published.publishedIdentity, false);
      throw error;
    }
...
function entry(root: string, entity: DiscoveredEntity): JsonObject {
  return {
    ...
    provenance: {
      ...
      path: relative(root, entity.path),
      immutable: entity.boundary === EXPERIMENT,
    },
  };
}
...
function decode(token: string, root: string, authorityPath: string, artifact: string): JsonObject {
  try {
    ...
    throw failure("cursor_invalid", artifact, `${artifact} cursor is malformed or belongs to another project`, "Copy next_cursor exactly, or omit --cursor to restart.");
  }
}
function boundedList(root: string, sourceRoot: string, artifact: string, boundary: string, selected: DiscoveredEntity[], all: DiscoveredEntity[], limit: number | undefined, cursor: string | undefined, order: string, filter: JsonObject, format = "json", selectorInput?: EntityListSelectorInput): JsonObject {
  const declared = contract(boundary, sourceRoot);
  ...
  const projectionOptions = {
    family: artifact as "objective" | "experiments",
    artifact,
    boundary,
    format,
...
export function getObjectiveEntity(root: string, id: string, sourceRoot = resolveSourceRoot()): JsonObject {
  const entity = selectObjective(relevant(root, sourceRoot), id);
  return {
    schemaVersion: "agentera.stateGet.v1",
    command: "state objective get",
    status: "ok",
    entry: entry(root, entity),
    source_contract: {
      authority: "references/artifacts/state-storage-authority.yaml",
      detail: "full_entity",
    },
...
export function listObjectiveEntities(
  root: string,
  limit?: number,
  cursor?: string,
  options: {
  ...
  const sourceRoot = options.sourceRoot ?? resolveSourceRoot();
  const entities = relevant(root, sourceRoot, options.discovery);
  const statuses = options.statuses?.length ? new Set(options.statuses) : undefined;
  const selected = entities
    .filter((entity) => entity.boundary === OBJECTIVE && (!statuses || statuses.has(objectiveStatus(entity))))
  ...
  return boundedList(root, sourceRoot, OBJECTIVE_ARTIFACT, OBJECTIVE, selected, entities, limit, cursor, OBJECTIVE_ORDER, options.statuses ? { status: options.statuses } : {}, options.format, options.selector);
}
export function currentObjectiveEntity(root: string, sourceRoot = resolveSourceRoot()): JsonObject {
  const entity = selectObjective(relevant(root, sourceRoot));
  return {
    schemaVersion: "agentera.stateList.v1",
    command: "state objective",
    status: "ok",
    entries: [entry(root, entity)],
    counts: { total: 1, returned: 1, remaining: 0 },
    filters: { status: "active" },
    source: { artifact: OBJECTIVE_ARTIFACT, authority: "canonical_entity_files" },
  };
}
export function getExperimentEntity(root: string, id: string, sourceRoot = resolveSourceRoot()): JsonObject {
  const entity = experimentById(relevant(root, sourceRoot), id);
  return {
    schemaVersion: "agentera.stateGet.v1",
...
export function listExperimentEntities(
  root: string,
  objective: string,
  limit?: number,
  cursor?: string,
  ...
  const sourceRoot = options.sourceRoot ?? resolveSourceRoot();
  const entities = relevant(root, sourceRoot, options.discovery);
  const owner = selectObjective(entities, objective);
  assertBaseline(entities, owner.id!);
  const selected = entities.filter((entity) => entity.boundary === EXPERIMENT && entity.record?.objective === owner.id).sort((a, b) => String(b.record!.date).localeCompare(String(a.record!.date)) || a.id!.localeCompare(b.id!));
  return boundedList(root, sourceRoot, EXPERIMENT_ARTIFACT, EXPERIMENT, selected, entities, limit, cursor, EXPERIMENT_ORDER, { objective: owner.id! }, options.format, options.selector);
}
...
export function listCurrentExperimentEntities(root: string, objective: string | undefined, limit?: number, cursor?: string, filters: { topic?: string; status?: string } = {}, options: { sourceRoot?: string; format?: string } = {}): JsonObject {
  ...
  if (filters.topic) {
    ...
    selected = selected.filter((entity) => canonicalRecordJson(entity.record).toLowerCase().includes(needle));
  }
