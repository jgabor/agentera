...
import { operationSpec, mutationParityMatrix, writerOwnedFields, verbsForArtifact, type WritableArtifact, type WriteVerb, type OperationField } from "./operations.js";
...
function defaultVerb(artifact: WritableArtifact): Exclude<WriteVerb, "explain"> {
  ...
  if (!verb)
    reject({
      ...
      message: `artifact "${artifact}" has no public mutation`,
    });
...
function exposedFields(artifact: WritableArtifact, verb: string, spec: NonNullable<ReturnType<typeof operationSpec>>, validator: ArtifactSchemaValidator): Record<string, unknown>[] {
...
function pathFor(artifact: WritableArtifact, verb: string, projectRoot: string): string {
  ...
  const resolved = entityArtifact ? path.join(projectRoot, ".agentera", "entities", artifact, entityBoundary(artifact, verb), "<id>.yaml") : resolveArtifactPath(record, projectRoot, { strictWrite: true });
...
export function buildExplain(artifact: WritableArtifact, projectRoot: string, requestedVerb?: string | null): Record<string, unknown> {
  ...
  if (spec.inputMode === "structured") {
    ...
    if (artifact === "progress" && verb === "append")
      result.input_schema = {
        ...(result.input_schema as Record<string, unknown>),
        record_fields: ["timestamp", "type", "phase", "what", "inspiration", "discovered", "verified", "next", "context", "glossary_caveat"],
      };
    if (artifact === "decisions" && verb === "append")
      result.input_schema = {
        ...(result.input_schema as Record<string, unknown>),
        record_fields: ["date", "question", "context", "alternatives", "choice", "reasoning", "confidence", "feeds_into"],
      };
    if (artifact === "decisions" && verb === "amend")
      result.input_schema = {
        ...(result.input_schema as Record<string, unknown>),
        record_fields: ["question", "context", "alternatives", "choice", "reasoning", "confidence", "feeds_into"],
      };
    if (artifact === "plan" && verb === "append")
      result.input_schema = {
        ...(result.input_schema as Record<string, unknown>),
        record_fields: ["name", "depends_on", "acceptance", "evidence", "blocked_reason"],
      };
    if (artifact === "plan" && verb === "update")
      result.input_schema = {
        ...(result.input_schema as Record<string, unknown>),
        record_fields: ["name", "depends_on", "acceptance", "evidence", "blocked_reason", "surprise"],
        clearable_patch_fields: ["depends_on", "acceptance", "evidence", "blocked_reason"],
      };
...
function decisionsGuidance(artifact: WritableArtifact, verb: string, _entityHealth = false, entityArtifact = false): string[] {
  const base = ["do not embed commit hashes; evidence belongs in the commit message (Decision 66)", "fold the artifact write into the implementation commit per AGENTS.md"];
  if (artifact === "glossary")
    return ["build is the only publisher; audit and discuss remain mutation-free", "confirmation.confirmed_by must be user and must bind the exact proposal digest and timestamp", "the writer revalidates every cited project source line and publishes approval plus entry as one atomic document", ...base];
  ...
  if (entityArtifact && artifact === "plan" && verb === "archive")
    return ["archive a complete plan normally; --force archives an unfinished selected plan without changing task, evaluation, or completion history", "an implicit archive rejects multiple open candidates; select an exact historical plan with its bare --plan ID", ...base];
  ...
  if (entityArtifact && artifact === "plan")
    return ["the active plan entity is selected by lifecycle state", ...(verb === "set-plan-status" ? ["open-to-complete requires every superseded_by replacement to be complete with latest persisted PASS; historical unevaluated complete replacements may record their allowed first PASS, then retry"] : []), ...base];
  ...
  if (entityArtifact && artifact === "todo") return ["a bare ten-letter TODO item ID is assigned by the CLI; status starts open", "create requires a full typed TODO record; readiness is optional and absent means needs-triage", "the public marker is rendered as [kind:target] and is not stored in title prose", ...base];
