...
export interface TodoRepairPlan {
  records: Map<string, JsonObject>;
  retainedLegacyRows: string[];
  rendered: string;
  diagnosis: JsonObject;
}
export interface TodoOwnerCorrectionEvidence {
  owners: Array<{ id: string; source_line: number }>;
  sha256: string;
}
export interface TodoOwnerCorrectionPlan {
  records: Map<string, JsonObject>;
  rendered: string;
  diagnosis: JsonObject;
}
...
function severity(section: string): string | null {
  return ["critical", "degraded", "normal", "annoying"].includes(section) ? section : null;
}
...
function publicSnapshot(record: JsonObject): Snapshot {
  return {
    present: true,
...
function rowSnapshot(row: Row, record: JsonObject): Snapshot {
  return {
    ...row.snapshot,
    severity: row.section === "resolved" ? String(record.severity) : row.snapshot.severity,
  };
}
function samePublic(left: Snapshot, right: Snapshot): boolean {
  return ["present", "description", "severity", "status"].every((field) => left[field as keyof Snapshot] === right[field as keyof Snapshot]);
}
function withBaseline(record: JsonObject, value: Snapshot): JsonObject {
  return {
    ...record,
    reconciliation: {
      schema_version: RECONCILIATION_VERSION,
      public: Object.fromEntries(Object.entries(value).filter(([, field]) => field !== undefined)),
    },
  };
}
...
function rowFor(id: string, record: JsonObject): string {
  return `- [${record.status === "resolved" ? "x" : " "}] [id:${id}] ${renderTodoPublicRecord(record)}`;
}
...
export function todoOwnerCorrectionInputViolations(input: Record<string, unknown>): string[] {
  ...
  return violations.length <= DIAGNOSTIC_LIMIT ? violations : [...violations.slice(0, DIAGNOSTIC_LIMIT), `${violations.length - DIAGNOSTIC_LIMIT} additional input violations omitted`];
}
...
export function normalizeTodoOwnerCorrectionEvidence(input: Record<string, unknown>): TodoOwnerCorrectionEvidence {
  ...
  const owners = (input.owners as Array<Record<string, unknown>>).map((owner) => ({ id: String(owner.id), source_line: Number(owner.source_line) })).sort((left, right) => left.id.localeCompare(right.id));
  return {
    owners,
    sha256: createHash("sha256")
      .update(canonicalRecordJson({ schema_version: TODO_OWNER_CORRECTION_INPUT_VERSION, owners }))
      .digest("hex"),
  };
}
function scanRows(markdown: string): { managed: Map<string, ManagedRow[]>; legacy: Row[] } {
  assertTodoSeverityHeadingStructure(markdown);
  const managed = new Map<string, ManagedRow[]>();
  const legacy: Row[] = [];
  ...
  markdown.split(/\r?\n/).forEach((line, index) => {
    ...
    if (item.id && !rowSeverity && section !== "resolved")
      reject({
        class: "conflict",
        message: `TODO repair found managed ID '${item.id}' outside a managed section`,
        recovery: `Move '${item.id}' under one declared TODO severity or the resolved section, then preview repair again; no state was changed.`,
      });
    if (!section) return;
    ...
    const row = {
      ...
      snapshot: {
        ...
        ...(section === "resolved" ? {} : { severity: rowSeverity! }),
        status: item.status,
        order: publicOrder,
      },
    };
...
export function planTodoRepair(markdown: string, activation: TodoReconciliationActivation, entities: readonly Entity[]): TodoRepairPlan {
  ...
  const retainedRows: Row[] = [];
  const conflicts: string[] = [];
  for (const row of legacy) {
    const fingerprint = todoLegacyRowFingerprint(row.section, row.sourceLine);
    if (retained.has(fingerprint)) retainedRows.push(row);
    else conflicts.push(`line ${row.line + 1}: ID-less managed row has no activation provenance`);
  }
  for (const [id, rows] of managed) {
    if (rows.length > 1) conflicts.push(`ID '${id}' occurs at lines ${rows.map((row) => row.line + 1).join(", ")}`);
    if (!entities.some((entity) => entity.id === id)) conflicts.push(`ID '${id}' has no canonical entity`);
  }
  ...
  const entityKeys = (record: JsonObject): Set<string> => {
    const current = publicSnapshot(record);
    const keys = new Set([identityKey(current.description, current.severity)]);
    const prior = baseline(record);
    if (prior?.present) keys.add(identityKey(prior.description, prior.severity));
    return keys;
  };
    ...
    if (matches.length > 1) conflicts.push(`line ${row.line + 1}: retained row matches multiple canonical managed IDs`);
      ...
      if (items.length < DIAGNOSTIC_LIMIT)
        items.push({
          ...
          id: unproven.length === 1 ? unproven[0]!.id : null,
          source_line: row.line + 1,
        });
  ...
  for (const entity of entities) {
    ...
    if (managedRows.length > 1) continue;
    ...
    if (managedRow && legacyRow) {
      ...
      if (entity.record.status === "open" || prior?.present) conflicts.push(`ID '${entity.id}' has no one-to-one public row`);
  ...
  if (conflicts.length)
    reject({
      class: "conflict",
      message: `TODO repair requires complete one-to-one evidence; found ${conflicts.length} conflicting decision${conflicts.length === 1 ? "" : "s"}`,
      ...
      diagnosis: {
        ...
        items,
        omitted_count: Math.max(0, duplicate + matched + retainedCount - items.length),
  ...
  for (const [id, row] of selected) replacements.set(row.line, rowFor(id, records.get(id)!));
  const rendered = `${markdown
    .split(/\r?\n/)
    .flatMap((line, index) => (remove.has(index) ? [] : [replacements.get(index) ?? line]))
    .join("\n")
    .replace(/\n*$/, "")}\n`;
  ...
  const retainedLegacyRows = retainedRows
    .filter((row) => !claimedLegacy.has(row.line))
    .map((row) => todoLegacyRowFingerprint(row.section, row.sourceLine))
    .sort();
  return {
    records,
    retainedLegacyRows,
    rendered,
    diagnosis: {
      counts: { duplicate, stale, matched, retained: retainedLegacyRows.length, conflicting: 0 },
      items,
      omitted_count: Math.max(0, duplicate + matched + retainedLegacyRows.length - items.length),
    },
  };
}
...
export function planTodoOwnerCorrection(markdown: string, entities: readonly Entity[], evidence: TodoOwnerCorrectionEvidence): TodoOwnerCorrectionPlan {
  const { managed, legacy } = scanRows(markdown);
