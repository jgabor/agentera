...
function mapping(value: unknown): Mapping {
  return value !== null && typeof value === "object" && !Array.isArray(value) ? (value as Mapping) : {};
}
...
export function decisionRevisionContract(sourceRoot: string = resolveSourceRoot()): DecisionRevisionContract {
  ...
  for (const [field, paths] of [
    ["amendable_paths", amendablePaths],
    ["legacy_amendable_paths", legacyAmendablePaths],
    ["migration_amendable_paths", migrationAmendablePaths],
  ] as const) {
...
export function legacyLabelCoexistence(sourceRoot: string = resolveSourceRoot()): LegacyLabelCoexistence {
  const authority = readAuthority(sourceRoot);
...
export function classifyConfidenceLabel(contract: LegacyLabelCoexistence, value: string, touched: boolean): ConfidenceLabelClassification {
...
export function decisionRevisionViolations(document: unknown, contract: DecisionRevisionContract = decisionRevisionContract()): string[] {
    ...
    if (key === "alternatives") {
      ...
      const malformed =
        ...
        alternatives.some((entry) => {
          ...
          return typeof candidate.name !== "string" || candidate.name.length === 0 || !["chosen", "rejected"].includes(String(candidate.status));
        }) ||
...
export function decisionRevisionEntityViolations(record: JsonObject, contract: DecisionRevisionContract = decisionRevisionContract(), allowLegacyConfidence = false): string[] {
