...
export function readTodoMarkdown(target: string): { bytes: Buffer; text: string } {
  ...
  try {
    ...
    reject({
      class: "conflict",
      message: "managed TODO Markdown is not valid UTF-8",
      recovery: "Restore valid UTF-8 TODO Markdown and retry; no state was changed.",
    });
  }
  return { bytes, text: text! };
}

function sectionFor(record: JsonObject): string {
  return record.status === "resolved" ? "resolved" : String(record.severity);
}
function headingFor(section: string): string {
  return TODO_SEVERITY_HEADINGS.find((entry) => entry.section === section)!.heading;
}
...
export function renderManagedMarkdown(markdown: string, records: Map<string, JsonObject>, existing: Map<string, ExistingRow>): string {
  ...
  for (const { section } of TODO_SEVERITY_HEADINGS) {
    const ids = [...records.entries()]
      .filter(([id, record]) => !retained.has(id) && sectionFor(record) === section)
      .sort(([left], [right]) => {
    ...
    while (insert < lines.length && !/^##\s+/.test(lines[insert]!.trim())) insert += 1;
