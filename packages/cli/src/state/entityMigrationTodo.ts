...
function migrationTodoRows(text: string): MigrationTodoRow[] {
  ...
  text.split(/\r?\n/).forEach((line, index) => {
    ...
    const record: JsonObject = item.kind && item.kind !== "x"
      ...
      : { severity, status: item.status, description: item.public_description ?? item.description };
...
export function todoMigrationObservations(
  root: string,
  sourceRoot: string,
  todoPath: string,
  ...
  if (activation?.kind === "file") observations.push({ key: "todo:activation:stale", artifact: "todo", boundary: "todo_item", path: TODO_RECONCILIATION_ACTIVATION_PATH, provenance: "preexisting_activation", record: null, detail: "corrupt", relationships: [], message: "TODO reconciliation activation exists before entity cutover" });
  if (todo?.kind === "file" && todo.bytes) {
    ...
    if (existing.length > 0) {
      ...
      if (invalid) observations.push({ key: `todo:entity:invalid:${invalid.relativePath}`, artifact: "todo", boundary: "todo_item", path: invalid.relativePath, provenance: "precreated_entity", record: null, detail: "corrupt", relationships: [], message: "pre-created TODO entity evidence is not canonical" });
      ...
      for (const row of rows) {
        ...
        if (matches.length !== 1) {
          observations.push({ key: `todo:row:${row.index + 1}`, artifact: "todo", boundary: "todo_item", path: todoPath, provenance: "precreated_entity_match", record: null, detail: "corrupt", relationships: [], message: matches.length === 0 ? `TODO.md line ${row.index + 1} has no one-to-one canonical entity match` : `TODO.md line ${row.index + 1} has multiple canonical entity matches` });
          continue;
        }
        ...
        observations.push({ key: `${todoPath}:line:${row.index + 1}`, artifact: "todo", boundary: "todo_item", path: todoPath, provenance: "precreated_entity_match", record, detail: violations.length ? "corrupt" : "full", relationships: [], message: violations.length ? violations.join("; ") : undefined, explicitId: entity.id! });
      }
      for (const entity of existing) if (entity.id && !claimed.has(entity.id)) observations.push({ key: `todo:entity:unmatched:${entity.id}`, artifact: "todo", boundary: "todo_item", path: entity.relativePath, provenance: "precreated_entity_match", record: null, detail: "corrupt", relationships: [], message: `canonical TODO entity '${entity.id}' has no one-to-one TODO.md row match` });
    } else {
      for (const row of rows) {
        const violations = todoDocsRecordViolations("todo_item", row.record, sourceRoot);
        observations.push({ key: `${todoPath}:line:${row.index + 1}`, artifact: "todo", boundary: "todo_item", path: todoPath, provenance: "current_canonical", record: row.record, detail: violations.length ? "corrupt" : "full", relationships: [], message: violations.length ? violations.join("; ") : undefined });
      }
    }
  } else if (todo?.kind === "unsafe") observations.push({ key: "todo:document", artifact: "todo", boundary: "todo_item", path: todoPath, provenance: "current_canonical", record: null, detail: "corrupt", relationships: [], message: `source path '${todoPath}' is a symbolic link, non-file, or unreadable; replace it with a readable regular file inside the project` });
  return observations;
}

export function todoReconciliationMigrationPlan(
  todoPath: string,
  files: readonly MigrationSourceFile[],
  entries: TodoMigrationEntry[],
): TodoReconciliationMigrationPlan | undefined {
  const todo = files.find((source) => source.relative === todoPath);
  if (todo?.kind !== "file" || !todo.bytes) return undefined;
  const rows = migrationTodoRows(todo.bytes.toString("utf8"));
  const byLine = new Map(entries.filter((entry) => entry.boundary === "todo_item" && entry.proposed_target).flatMap((entry) => {
  ...
  for (const row of rows) {
    ...
    record.reconciliation = { schema_version: "agentera.todoReconciliation.v1", public: { present: true, description: renderTodoPublicRecord(record), severity: String(record.severity), status: String(record.status), order: row.order } };
    entry.record = record;
