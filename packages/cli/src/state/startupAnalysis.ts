...
function distribution(values: number[]): Dict {
  ...
  return {
    ...
    mean: pyRound(ordered.reduce((a, b) => a + b, 0) / ordered.length, 2),
    p50: percentile(0.5),
    p75: percentile(0.75),
    histogram,
  };
...
export function aggregateStartupMetrics(intermediateInput: Dict): Dict {
  ...
  const result: Dict = {
    ...
    raw_after_cli_sequence_rate: totalSequences ? pyRound(sequencesWithRaw / totalSequences, 4) : 0,
    redundant_raw_sequence_rate: totalSequences ? pyRound(sequencesWithRedundant / totalSequences, 4) : 0,
