...
The **spec** is the knowledge protocol. Artifact contracts define how agents share state. Shared primitives give them a common language. The spec is what turns a collection of disconnected skills into a graph where each one checks the others' work and compounds understanding over time.
...
He finds the spec. Artifact contracts tell his skill exactly what to produce and what it can consume. Shared primitives mean his confidence scores and severity levels mean the same thing as everyone else's. He builds one skill that meshes with hundreds, not one skill that works alone. The spec is the leverage.
...
- **Coherence over features.** The suite is the product, not the individual skills. Skills that mesh perfectly beat more skills that don't talk to each other. Resist the temptation to add surface area. Every new skill must strengthen the graph, not just extend it.
...
- **Openness over lock-in.** The spec is an open standard. The artifact contracts are platform-agnostic. Skills that follow the spec work across any runtime that speaks the same language. The reference implementation proves the model; it doesn't own it.

## Direction

The spec is the foundation. The skills are the proof. The next layer is adoption.

Today agentera runs on Claude Code. Skills as plugins, hooks as lifecycle events, agents spawned through the host's primitives. That proved the model works. The spec and the artifact contracts are already platform-agnostic. The adoption arc takes that portability seriously.

The spec becomes a gravity well. OpenCode, Gemini CLI, Codex CLI, any agent runtime that speaks the protocol can run the portable core of agentera. The artifact contracts are the API: VISION.md, TODO.md, HEALTH.md, PROGRESS.md, the whole set. Skills that depend only on those contracts travel cleanly; skills that depend on host-specific session exhaust declare that dependency explicitly until a wider contract exists. The reference implementation proves the contracts are sufficient, then others implement against them.

The runtime question answers itself. Claude Code today, any agent CLI tomorrow. The artifacts don't change. The portable core of the spec doesn't change. What changes is how many platforms they run on and which adapter-specific extensions each runtime chooses to implement. An opinionated standalone runtime is one answer. A thin adapter for each platform is another. The protocol doesn't prescribe the mechanism; it prescribes the contract.
