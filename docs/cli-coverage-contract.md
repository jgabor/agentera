...
| `packages/cli/src/setup/sharedSkill.ts`; `packages/cli/src/setup/hostSkillLifecycle.ts`; `packages/cli/src/setup/hostSkillConversion.ts`; `packages/cli/src/upgrade/productV1Reset.ts`; `references/adapters/product-v1-reset.yaml`; `UPGRADE.md` | `upgrade --explain` → `--operation install`, `refresh`, `migrate`, `reset`, `cleanup`; `doctor --explain`. `upgrade --shared-skill` owns fresh one-file publication, owned refresh and token-bound legacy conversion. The upgrade guide covers Linux apply, the 4096-entry inventory, same-filesystem retention, retry, stale approval, ownership and scope. Reset/refresh/upgrade never recreate a full-tree host link. | E8, E9 |
