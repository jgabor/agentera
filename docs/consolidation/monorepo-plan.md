...
| **Editor-runtime surface** | **Composite delivery surface** (Decision 69): `skills/agentera/SKILL.md` (discoverability + routing stub) + runtime-native hooks (lifecycle injection) + `agentera` CLI binary (engine). Same twelve-capability workflow in OpenCode, Codex, Cursor, and Copilot — not user extensibility; hooks delegate validation to CLI. Documented under README **Internals** and [`packages/cli/README.md`](../../packages/cli/README.md). |
