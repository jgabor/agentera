...
| `.mutation-lock` | Concurrent callers wait for an atomically-created owner record. A dead or PID-reused owner is atomically claimed and reclaimed only after its token is rechecked; a losing reclaimer retries. Live ownership times out actionably, and unknown identity is preserved. |
