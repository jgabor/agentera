...
- [x] ~~[opencode-session-events] Replace the SESSION.md bookmark wiring using OpenCode's real `event` mechanism.~~ · Resolved 2026-04-27 by 1.20.1: OpenCode now handles `session.idle` through generic `event`, writes SESSION.md bookmarks for modified artifacts, rejects direct phantom hook keys, and documents session-start preload as deferred.
