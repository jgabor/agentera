...
- [ ] [awesome-copilot-pr] Submit PR to `github/awesome-copilot` aggregator listing agentera. Filed 2026-04-27 by Cross-Runtime Parity Completion T7.
- [ ] [awesome-codex-plugins-pr] Submit PR to `hashgraph-online/awesome-codex-plugins` aggregator listing agentera. Filed 2026-04-27 by Cross-Runtime Parity Completion T7.
- [ ] [copilot-hook-event-name-validator] Add a small lint rule to `scripts/validate_lifecycle_adapters.py` (or a new validator) that rejects Copilot hook event names not in the documented event allowlist (`sessionStart`, `sessionEnd`, `userPromptSubmitted`, `preToolUse`, `postToolUse`, `errorOccurred`). Prevents future dead-hook regressions like the original `stop.json` typo. Filed 2026-04-27 by Cross-Runtime Parity Completion T7.
...
- [ ] [live-host-smoke-mjs-doc-row] `.agentera/DOCS.md` Index and README Scripts section omit a row for `scripts/smoke_opencode_bootstrap.mjs`. Live-Host Verification Task 5 acceptance criterion 3 referenced it as the existing peer of `smoke_setup_helpers.py`, but the row was never added during prior cycles. Add an Index row at the next dokumentera audit and an entry to README Scripts when the next docs cycle touches that section.
