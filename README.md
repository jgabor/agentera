...
Copilot plugin management supports `copilot plugin install`, `copilot plugin list`, and `copilot plugin marketplace add/list/browse/remove`. Prefer verified marketplace installs when a source is available; `<plugin>@<marketplace>` is Copilot syntax, not evidence that Agentera is published in a marketplace. Direct repo, Git URL, and local path installs currently work but Copilot warns they are deprecated. The current-checkout plugin manifest is `plugin.json`, so `copilot --plugin-dir <repo>` loads shared `skills/` without escaping the plugin root.
...
After a successful aggregate install, `copilot plugin list` should show the `agentera` plugin. Older per-skill entries such as `hej@agentera` may also appear if they were installed through earlier metadata.
