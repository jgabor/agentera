...
Hook event coverage is implemented where host payloads expose enough evidence. Copilot enforcement happens at `preToolUse` only for reconstructable artifact edits; Copilot `postToolUse` output is logging-only. Codex routes Write/Edit interception through `apply_patch` with compatibility aliases (`apply_patch`, `Write`, `Edit`) for hook matchers.
