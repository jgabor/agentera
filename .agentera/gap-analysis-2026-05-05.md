...
<!-- Date: 2026-05-05 | Status: repaired/current | Scope: Agentera 2.0 main worktree plus v2.0.2 legacy cleanup correction -->
...
Post-release transcript review of OpenCode session `ses_2080d5c94ffeaOL51P6gQ0CwtF` found one additional v2.0.0 upgrade gap: after `npx skills update`, `/agentera` was treated as prompt text and the runtime loaded the stale v1 `/hej` skill from `~/.agents/skills/hej`. The first v2.0.1 correction added a legacy `skills/hej` bridge. The refined v2.0.2 correction keeps `/hej` as the only compatibility trigger, then makes the approved package upgrade remove legacy v1 package-managed skill entries before installing `/agentera`.

Host-specific note: the local `npx skills update` command also reported "up to date" while `gh auth token` returned an invalid GitHub token and GitHub's tree API returned 401. Hiding that broken `gh` token made the skills CLI detect the remote `/hej` update. That is not an Agentera package metadata gap, but it can mask successful publication during local smoke checks.

Current verification:

| Check | Result |
|---|---|
| `uv run --with pytest --with pyyaml pytest -q` | PASS: 540 passed, 1 skipped |
...
| `uv run scripts/measure_token_payload.py` | PASS: v2 315,983 bytes vs v1 352,213 bytes, -10.3%; revised -10% release target exceeded |
...
| `uv run scripts/agentera upgrade --only packages --runtime opencode --json` | PASS: package phase lists skipped legacy removal plus `/agentera` install commands |
| `env -u AGENTERA_HOME uvx --from . agentera upgrade --only bundle --home <tmp> --install-root <tmp>/.agents/agentera --yes --json` | PASS: built wheel installs a durable bundle with active `/agentera` and temporary `/hej` bridge only |
| `uv run --with pytest --with pyyaml pytest -q tests/test_runtime_adapters.py tests/test_upgrade_cli.py tests/test_detect_stale_v1.py` | PASS: 90 passed |
| `npx skills add . --list --full-depth` | PASS: repo exposes active `agentera` and temporary legacy `hej` bridge |
...
| Keep one bundled skill | Complete with temporary legacy bridge | `registry.json` keeps only `agentera` as the active bundled skill; `skills/hej/SKILL.md` exists only so old `/hej` installs can hand users to `/agentera` plus `agentera upgrade`, and package upgrade removes legacy v1 skills | None |
...
| Port tests | Complete for current v2 suite | `540 passed, 1 skipped` | The retired v1 helper tests are intentionally gone; no claim should cite 577 as current |
...
| Claim | Status | Evidence | Gap |
|---|---|---|---|
| Full test suite green | Complete | `540 passed, 1 skipped` | None |
| Semantic eval port | Complete | Semantic eval fixtures/tests use v2 YAML paths and pass | None |
| Token consumption benchmark | Complete | Decision 41 and `scripts/measure_token_payload.py` record v1 352,213 bytes vs current v2 315,983 bytes, -10.3%; revised -10% release target exceeded | Further optimization deferred to future versions |
| Version bump to 2.0.x | Complete | Runtime package validators pass; v2.0.2 keeps `/hej` as the temporary legacy bridge and removes package-managed v1 skills during approved upgrade | None |
...
No release-blocking gaps remain after the 2026-05-05 scope decisions and the v2.0.2 legacy cleanup correction.

Non-blocking follow-ups remain:

1. Additional token optimization can continue in future versions.
   The final remeasurement after the v2.0.2 bridge correction is v2 315,983 bytes vs v1 352,213 bytes, a 10.3% reduction. The v2.0 release target is revised to -10%, and the current payload exceeds it.
...
- Legacy v1 `npx skills update` interception: OpenCode session `ses_2080d5c94ffeaOL51P6gQ0CwtF` showed `/agentera` loading old v1 `hej` after `npx skills update`. v2.0.2 keeps `/hej` as the temporary bridge and package-refresh commands now remove legacy v1 skills before installing `/agentera`.

## Closed Questions

- Token target: remeasured at -10.3% after the v2.0.2 bridge correction; revised -10% release target exceeded, with follow-up optimization deferred to future versions.
...
- Decision 41 now includes the 2026-05-05 remeasurement: v2 315,983 bytes vs v1 352,213 bytes, -10.3%, exceeding the revised -10% release target.
...
- `SKILL.md` and legacy bridge guards now instruct agents to detect v1 artifacts, preview the no-clone `uvx --from git+https://github.com/jgabor/agentera agentera upgrade` command, and ask before applying `--yes`.
...
1. Commit the v2.0.2 compatibility correction once final validation passes.
   The worktree now keeps only the `/hej` bridge, removes legacy package-managed skills during approved package refresh, and records the npx-update-derived gap.

2. Publish the patch release after approval.
   After integration, create and push `v2.0.2` only when explicitly requested. Do not move existing `v2.0.0` or `v2.0.1` tags.
...
The final release-candidate validation passed on 2026-05-05, and the transcript-derived legacy entry-point gap is now closed in the v2.0.2 patch work. Remaining release work is procedural: commit, integrate to main, and only create/push `v2.0.2` when explicitly approved.
