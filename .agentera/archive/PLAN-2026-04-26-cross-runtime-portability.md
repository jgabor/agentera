...
- **Task 2 (Cycle 170)**: The `session.created` hook in legacy agentera.js was carrying a context-preload block that called `client.session.create({ message: ... })` to inject a `Last progress / grades / critical issues` briefing at session start. Since the hook key was phantom, that briefing never reached the agent — under OpenCode the context preload was already vapor before this task. Dropping the phantom key removed dead code rather than removing functionality. Genuine OpenCode session-start context preload (and the SESSION.md bookmark Out-of-scope item) should be reconsidered together when adopting OpenCode's real `event` hook for session lifecycle. Logged as a TODO follow-up alongside SESSION.md replacement during Task 6. Empirical lifecycle observation in the smoke harness: the Agentera plugin function fires once per explicit invocation (5 invocations in the harness produced exactly 5 `lifecycle.initCount` increments). The plan's "once per plugin load" assumption holds at the unit level; real OpenCode-session cadence still requires manual verification (the install steps in README will instrument this when a real OpenCode run is available). The implementation is robust to multi-fire either way: bootstrapCommands is idempotent against the version marker, shell.env honors pre-set values, and setProfileDir is gated on existing env.
- **Task 4 (Cycle 172)**: Cycle 169's documented Section-renumbering drift map listed SKILL.md prose, validator code, and two test files as the affected surfaces. Six additional surfaces also carried hardcoded Section 21 references to the Session Corpus Contract (now Section 22) that the original Surprise didn't enumerate: `.codex-plugin/plugin.json` (`invocationHint`), `scripts/validate_lifecycle_adapters.py` (`CODEX_PROFILERA_INVOCATION_TERMS` plus the matching error string), `agents/openai.yaml` and `skills/profilera/agents/openai.yaml` (Codex metadata descriptions), `skills/profilera/scripts/extract_all.py` (5 comments and docstrings), `scripts/usage_stats.py` (6 comments and docstrings), and `references/adapters/opencode.md` (3 references including a heading). The plugin.json + validator + test triplet was tightly coupled to the explicit Surprise scope (the test asserts the validator's error string which mentions the section number that flows from the plugin metadata it checks) so all three had to move together; the four documentation surfaces moved with them for consistency. No new test budget added for the documentation shifts — existing test coverage (lifecycle adapter validation, spec validation, contract freshness) all stayed green. The drift remained mechanical and within the spirit of the documented mapping; no scope creep beyond the same Session Corpus Contract semantic anchor.
