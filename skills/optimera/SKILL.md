...
When writing a new experiment entry to EXPERIMENTS.md, check entry count. If >8 full-detail entries exist, collapse the oldest to one-line format under `## Archived Experiments` (one line per experiment: `EXP-N: ≤15-word result summary`). If >22 one-line entries exist, drop the oldest. See ecosystem-spec.md Section 4 compaction thresholds.
