...
- The same invalid-input envelope is now wired across the remaining CLI surfaces (`lint`, `state`, `query`, `compact`, `schema`, `doctor`, `upgrade`, `report`, `verify`, `capability`, `help`, `usage`, `prime`, `gate`, `hook`) in `packages/cli/src/cli/dispatch.ts`. 20 `run*` functions route their parse-error and catch paths through `emitInvalidInput`; existing text-mode assertions in `help.test.ts`, `hook.test.ts`, and `schema.test.ts` were updated to assert the four-question template. `check validate`, `check verify`, `agentera schema`, and the `check *` not-yet-ported branch use the canonical `unsupported_target` class with optional `valid_values` lists of supported options.
...
- Frozen parity oracle for the `source_contract` field set emitted by the 3.0 TypeScript CLI at `packages/cli/test/cli/fixtures/oracle/source-contract.json` (`schemaVersion: agentera.sourceContractOracle.v1`). The oracle pins the field set, value types, and required keys for every CLI surface that emits a `source_contract`: `agentera prime` (top-level `payload.source_contract.capability_startup.*`), `agentera prime --context <capability>` (per-capability bespoke `capability_context.context.<bespoke>.source_contract` for the five BESPOKE capabilities `orkestrera`, `realisera`, `inspektera`, `optimera`, `dokumentera`; the seven non-bespoke capabilities carry no bespoke source_contract), `agentera state plan` (PLAN-artifact-shaped contract with `complete_state` and `raw_artifact_access_boundary` sub-objects), and `agentera state query --list-artifacts` (discovery-mode contract with `raw_artifact_reads_required_for_discovery` and `allowed_raw_artifact_uses`). The `state todo` exclusion is documented. A shared cross-cutting structural pattern (boolean 'is raw reads required' field, string policy field, caveats/fallbacks array) is asserted across all emissions. Field content that legitimately varies by caller (`cli_fallback`, `confidence_caveats`, `caveats`, `fallback_commands`, `owns`, `deferred`) is pinned at the value-type level only per AC4. The fail path and the cross-cutting npm exit-code matrix are owned by the existing `invalid-input-envelope.json` and `npm-cli-surface.json` oracles and are referenced, not duplicated. Enforced by `packages/cli/test/cli/sourceContractOracles.test.ts` with 28 parity tests covering the five source_contract emissions, seven non-bespoke capability exclusions, the `state todo` exclusion, and a `assertExactKeys` drift detector that fails with a named diff when an undeclared key is added. The full CLI test suite passes 672/672 (75 files, +28 vs cycle 628's 644/644).
